...
Partial Class Corp_DeMergerFinalPosting
    ...
    Sub PostMerge()
        If DivNoExists() = False Then
            msgbox("DeMerger instruction no longer at Final Authorization stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_PostDeMerge"
            ...
            End Using
        End Using
        Response.Write("<script>alert('DeMerger Final Authorization successful') ; location.href='DeMergerFinalPosting.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from DeMerger_instr where Company=@Company and div_no=@div_no and Authorize=3"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
