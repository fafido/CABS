...
Partial Class _Default
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Session("end") = "Yes" Then
                Session("end") = ""
                msgbox("Pledge Transfer Approved")
            Else
    ...
    Public Sub getlst()
        Try
            ...
            cmd = New SqlCommand("select CONVERT(varchar(10), id)+' '+ company+' '+ securitytype+' '+ CONVERT(varchar(10), Quantity ) as pleadgeinf, id from pledges_recording where Pledge_Status='A' and released is null and transferred is not null and id not in (select pledge_id from pledges_transfer where transfer_approval='Yes')", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(dspledgee1, "Accounts_Clients3")
    ...
    Public Sub getrelease()
        Try
            ...
            cmd = New SqlCommand("select transfer_reason  from pledges_transfer where pledge_id='" + lstpledgeSearch.Value.ToString + "'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(dsrel, "trans")
            If (dsrel.Tables(0).Rows.Count > 0) Then
                ' cmbcompany.DataSource = dscomps.Tables(0)
                txtPledgeReason.Text = dsrel.Tables(0).Rows(0).Item("transfer_reason")
    ...
    Protected Sub ASPxButton10_Click(sender As Object, e As EventArgs) Handles ASPxButton10.Click
        Try
            cmd = New SqlCommand("Update Pledges_transfer set transfer_approval='Yes' where pledge_id='" + lstpledgeSearch.Value.ToString + "'", cn)
            ...
            cmd = New SqlCommand("Update trans set pledge_shares=" & txtquantity.Text & " where company='" & txtcompany.Text & "' and cds_number='" & txtpledgor_Id.Text & "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
