...
Partial Class transferapproval_UT
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("select s.*, a1.Surname+' '+a1.Forenames as [FromName],a2.Surname+' '+a2.Forenames as [ToName] from transfer_UT s, Accounts_Clients a1, Accounts_Clients a2 where s.transferor=a1.CDS_Number and a2.CDS_Number=s.transferee and s.[Status]='PENDING'", cn)
    ...
    Public Sub loadnames()
        Dim ds As New DataSet
        '  cmd = New SqlCommand("select * from transfer_UT where id='" + cmbpending.SelectedItem.Value.ToString + "'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "Accounts_Audit2")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub approvetransaction(transkey As String)
        Try
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number , Date_Created, Trans_Time, shares, Update_Type, Created_By, Batch_Ref, source, Pledge_shares, reference, Instrument, Custodian) select company, transferor  , [date], getdate(), amount_to_transfer*-1 , 'Unit Trust Transfer', '" + Session("Username") + "', id , 'D', '0', id, 'Unit Trust', 'CABS' from transfer_UT WHERE id='" + transkey + "'      insert into trans (company, cds_number , Date_Created, Trans_Time, shares, Update_Type, Created_By, Batch_Ref, source, Pledge_shares, reference, Instrument, Custodian) select company, transferee  , [date], getdate(), amount_to_transfer , 'Unit Trust Transfer', '" + Session("Username") + "', id , 'D', '0', id, 'Unit Trust', 'CABS' from transfer_UT WHERE id='" + transkey + "'    Update transfer_UT set status='APPROVED' where id='" + transkey + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
    ...
    Public Sub DECLINE(transkey As String)
        Try
            ...
            cmd = New SqlCommand(" Update transfer_UT set status='REJECTED' where id='" + transkey + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
