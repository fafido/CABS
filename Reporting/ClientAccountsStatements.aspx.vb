Partial Class Reporting_ClientAccountsStatements
    ...
    Public Sub SaveClientStatementView(ByVal cdsnu As String, ByVal comp As String)
        ...
        cmd = New SqlCommand(" delete from tbl_accountstatement where cds_number='" + cdsnu + "' and account_type<>'DEPOSIT'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
        ...
        If (ri.Tables(0).Rows.Count > 0) Then
            cmd = New SqlCommand("insert into tbl_ClientStatementsView (trans_date,ref,trans_type,details,debit,credit,balance,cds_number,company) values ('" & txtDateFrom.Text & "','B/F','B/FWD','B/FWD',0.00,0.00," & ri.Tables(0).Rows(0).Item("Deposit_Amount").ToString & ",'" & cdsnu & "','OMZL')", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
            ...
            cn.Close()
        Else
            cmd = New SqlCommand("insert into tbl_ClientStatementsView (trans_date,ref,trans_type,details,debit,credit,balance,cds_number,company) values ('" & txtDateFrom.Text & "','B/F','B/FWD','B/FWD',0.00,0.00,0,'" & cdsnu & "','OMZL')", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
        ...
        cmd = New SqlCommand("select Tbl_MatchedOrders .Trade as [TradeDate] , 'P/'+convert(varchar ,deal) as [Deal],'PAYMENT' as [Type],  convert(varchar,Tbl_MatchedOrders.Quantity )+' '+'OMZL @'+  convert(varchar,Tbl_MatchedOrders.DealPrice) as [Details], Tbl_MatchedOrders.Quantity * Tbl_MatchedOrders .DealPrice as [Debit] ,  0.0 as [Credit]  from Tbl_MatchedOrders where Buyercdsno ='" & cdsnu & "'", cn)
        adp = New SqlDataAdapter(cmd)
        ...
        If (dsr.Tables(0).Rows.Count > 0) Then
            ...
            Next
        End If
        cmd = New SqlCommand("insert into tbl_accountstatement SELECT company, cds_number, trans_type as [Account_type], details as [description], trans_date as transdate, Debit as dr, Credit as cr, ref as reference   FROM TBL_ClientStatementsView where cds_number='" + cdsnu + "' and trans_date>='" & txtDateFrom.Text & "' and trans_date <='" + txtDateTo.Text + "' and trans_type<>'B/FWD'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
    ...
    Protected Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        SaveClientStatementView(lblCDsNumber.Text, "OMZL")
        Dim strscript As String
        strscript = "<script langauage=JavaScript>"
        strscript += "window.open('ClientAccountStatementReport.aspx?cdsno=" & lblCDsNumber.Text & "&Datefrom=" & txtDateFrom.Text & "&Dateto=" & txtDateTo.Text & "');"
        strscript += "</script>"
