...
Partial Class Reporting_ewr_newrpt
    ...
    Public Sub getstatus()
        Try
            ...
            'cmdStatus.Items.Insert(1, New DevExpress.Web.ASPxEditors.ListEditItem("Open", "Open"))
            'cmdStatus.Items.Insert(0, New DevExpress.Web.ASPxEditors.ListEditItem("Traded", "Traded"))
            'cmdStatus.Items.Insert(0, New DevExpress.Web.ASPxEditors.ListEditItem("Pending PMEX Fetch", "Pending PMEX Fetch"))
        Catch ex As Exception
            msgbox(ex.Message)
        End Try
    End Sub
    ...
    Public Sub Getproduce()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select distinct(commodity) from wr where status = 'AFT' and isnull (commodity,'') <>''", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds)
    ...
    Public Sub getEWRs()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select distinct(ReceiptNo) from wr where status = 'AFT'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds)
    ...
    Protected Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Try
            ...
            strscript += "window.open('rptpmex.aspx?Datefrom=" & txtDateFrom.Text & "&Dateto=" & txtDateTo.Text & "&commodity=" & cmdProduce.Value & "&status=" & cmdStatus.Value & "&ewr=" & cmbEWRs.Value & "&code=" & cmbParticipantCode.Value & "');"
            strscript += "</script>"
