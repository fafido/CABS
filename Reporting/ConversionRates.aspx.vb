...
Partial Class _Default
    ...
    Public Function CheckRateOverlap() As Boolean
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand("SELECT * from para_CurrencyRates Where CurrencyCode='" & ddCurrency.DataTextField & "' and  DateFrom <= '" & dtpFrom.Text & "' AND DateTo >= '" & dtpTo.Text & "'", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub ASPxButton1_Click(sender As Object, e As EventArgs) Handles ASPxButton1.Click
        ...
        Else
            ...
            Else
                If dtpTo.Text = "" Then
                    msgbox("Please enter the date to!")
                    Exit Sub
                End If
            End If
        End If
        ...
        If ASPxButton1.Text = "Save Rate" Then
            Dim cmd1 = New SqlCommand("insert into para_CurrencyRates (CurrencyCode,RateToBase,DateFrom,DateTo) values (@currencycode,@ratetobase,@datefrom,@dateto)", cn)
        ...
        Else
            ...
            cmd1 = New SqlCommand("update para_CurrencyRates set currencycode=@currencycode, RateToBase=@RateToBase, DateFrom=@DateFrom, DateTo=@DateTo where RateID = @SelectedID", cn)
            cmd1.Parameters.AddWithValue("@currencycode", ddCurrency.Text)
            cmd1.Parameters.AddWithValue("@RateToBase", txtRate.Text)
            cmd1.Parameters.AddWithValue("@DateFrom", dtpFrom.Text)
            cmd1.Parameters.AddWithValue("@DateTo", dtpTo.Text)
    ...
    Public Sub FillGrid()
        Dim ds As New DataSet
        cmd = New SqlCommand("SELECT RateID,CurrencyCode,RateToBase,DateFrom,DateTo from  para_CurrencyRates", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "para_CurrencyRates")
