...
Partial Class Reporting_cashbalances
    ...
    Public Sub loadcomboforassetmanagers()
        ...
        cmd = New SqlCommand("select * from (select 1 as rankz, 'All' as AssetMananger, 'All' as AssetManagerCode union select 2 as rankz,AssetMananger+' | '+ pb.AccountNo +' | '+ pb.Currency as AssetMananger, p.AssetManagerCode+' | '+ pb.AccountNo +' | '+ pb.Currency as AssetManagerCode  from para_AssetManager p, para_assetManager_Banks pb where pb.AssetManagerCode=p.AssetManagerCode) j order by j.rankz", cn)
