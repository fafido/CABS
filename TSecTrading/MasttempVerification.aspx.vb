...
Partial Class TSecTrading_MasttempVerification
    ...
    Public Sub getCompany()
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                ...
                cmbBatch.DataBind()
            Else
                cmbBatch.Text = "No Companies Present"
            End If
        ...
        End Try
    End Sub
    Public Sub getMasttempData()
        Try
            Dim TShares As Integer
            Dim ds As New DataSet
            cmd = New SqlCommand("Select sum(shares) as Shares from trans where company='" & cmbBatch.Text & "'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "trans")
            If (CInt(ds.Tables(0).Rows.Count) > 0) Then
                TShares = CInt(ds.Tables(0).Rows(0).Item("Shares").ToString)
                LblBatchDate.Text = CStr(TShares)
                MastDetails()
            Else
                LblBatchDate.Text = ""
            End If
        ...
        End Try
    End Sub
    Public Sub MastDetails()
        Try
            Dim ds1 As New DataSet
            Dim ds As New DataSet
            cmd = New SqlCommand("select distinct (tran_type),sum (shares) from mast where company='" & cmbBatch.SelectedValue & "' group by tran_type", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "masttemp")
            If (ds.Tables(0).Rows.Count > 0) Then
        ...
        End Try
    End Sub
    Public Sub getCurrentShareCapital()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select sum(Shares) as Shares from masttemp where company='" & cmbBatch.SelectedIndex & "'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "Masttemp")
            If (ds.Tables(0).Rows.Count > 0) Then
                Dim TotalShares As Integer
                lblBatchTotal.Text = CInt(ds.Tables(0).Rows(0).Item("Shares").ToString)
                TotalShares = CInt((lblBatchTotal.Text) + (LblBatchDate.Text))
                lblbatchref.Text = TotalShares
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Public Sub getBatch()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select * from TSec_batch_header where batch_ref=" & cmbBatch.Text & " and status='F'", cn)
            adp = New SqlDataAdapter(cmd)
