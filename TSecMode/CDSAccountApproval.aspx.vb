...
Partial Class TSecMode_CDSAccountApproval
    ...
    Public Sub GetHolderData()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select * from pre_names_Creation where approved=1 and RecType='NEW'", cn)
    ...
    Public Sub getHolderDetails()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select * from pre_names_creation where cds_number='" & cmbCdsNumber.Text & "' and Approved=1", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "pre_names_creation")
            If (ds.Tables(0).Rows.Count > 0) Then
        ...
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                If (Session("Username").ToString = ds.Tables(0).Rows(0).Item("updated_by").ToString) Then
                    msgbox("User can not authorize own transactions")
                    Exit Sub
                End If
