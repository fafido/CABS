...
Partial Class TSecMode_NewAccountCreation
    ...
    Protected Sub Unnamed13_Click(sender As Object, e As EventArgs)
        ...
        ElseIf accType.SelectedIndex = 2 Then
            ...
            If (Len(certCompany.Text) < 1) Then
        ...
        ElseIf accType0.SelectedIndex = 0 Then
            Using Cony As New SqlConnection(constr)
                Cony.Open()
                Using Comy As New SqlCommand("SELECT * from [CDS].[dbo].[CertMaster] cm join [CDS].[dbo].[CertNames] cn on cm.Shareholder=cn.Shareholder where cm.CertNumber='" & certNumberTxt.Text & "' and cm.Shareholder='" & holderTxt.Text & "' and cm.Company='" & certCompany.Text & "'", Cony)
                    Using RDR = Comy.ExecuteReader()
                        If RDR.HasRows Then
                            ...
                            cmd = New SqlCommand("SELECT [Company], [Shareholder], [CertNumber], [Shares] FROM [CDS].[dbo].[CertMaster] where CertNumber='" & certNumberTxt.Text & "' and Shareholder='" & holderTxt.Text & "' and Company='" & certCompany.Text & "'", cn)
                            adp = New SqlDataAdapter(cmd)
                            adp.Fill(ds, "APP")
                            ...
                            For Each dr As DataRow In ds.Tables(0).Rows
                                ...
                                Dim holder = dr.Item("Shareholder").ToString()
                                                ...
                                                Using conn As New SqlConnection(constr)
                                                    Using comm As New SqlCommand()
                                                        ...
                                                        Try
                                                            ...
                                                            If accType.SelectedIndex = 0 Then
                                                                cmd = New SqlCommand("SELECT * FROM [CDS].[dbo].[AccountNewCertificates] where [IdNumber]='" + idNumberTxt.Text + "' order by id desc", cn)
                                                            ElseIf accType.SelectedIndex = 1 Then
    ...
    Protected Sub Unnamed12_Click(sender As Object, e As EventArgs)
        ...
        If Not acccreated Then
            ...
            End If
        Else
            msgbox("You have alreadey created account")
        End If
