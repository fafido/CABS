...
Partial Class CDSMode_InstrumentsSetUpnew1
    ...
    Protected Sub btnSaveCompany_Click(sender As Object, e As EventArgs) Handles btnSaveCompany.Click
        ...
        If txtprice.Text = "" Then
            msgbox("Please enter the initial price for the company!")
            Exit Sub

        End If

        If isnumeric(txtprice.Text) = False Then
            msgbox("Please enter initial price in decimal format!")
            Exit Sub
        End If
        ...
        Else
            ...
            Dim stmnt As String = "insert into para_company([Company],[Fnam],[Date_created],[Issued_shares],[registrar],[Add_1],[Add_2],[Add_3],[Add_4],[City],[Country],[Contact_Person],[Telephone],[Cellphone],[Fax],[Comments],[sec_type],[board],[index_type],[ticker],[ISIN],[year_listed],[comp_secretary],[email],[website],[issued_capital],[nominal_value] ,[Currency],[Issuer_code], [series],[InitialPrice], [TradingStatus], [Category]) values (''" & removeSpecialCharacter(txtTicker.Text) & "'',''" & removeSpecialCharacter(txtSecName.Text) & "'',''" & dtYear.Text & "'',''" & txtIssuedCapital.Text & "'',(select company_code from client_companies where company_name=''" & removeSpecialCharacter(txtcompsecretary.Text) & "''),''" & removeSpecialCharacter(txtRegdOffice1.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice2.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice3.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice4.Text) & "'','''',''" & cmbCountryListed.SelectedItem.Value & "'',''" & removeSpecialCharacter(txtcompsecretary.Text) & "'',''" & txtPhoneNo.Text & "'','''',''" & txtFaxNo.Text & "'','''',''EQUITY'',''" & removeSpecialCharacter(txtboard.Text) & "'','''',''" & removeSpecialCharacter(txtTicker.Text) & "'',''" & txtISIN.Text & "'',''" & dtYear.Text & "'',(select company_code from client_companies where company_name=''" & removeSpecialCharacter(txtcompsecretary.Text) & "''),''" & txtEmail.Text & "'',''" & txtWebsite.Text & "'',''" & txtIssuedCapital.Text & "'',''" & txtNominalValue.Text & "'',''" + cmbcurrency.SelectedItem.Value.ToString + "'',''" + removeSpecialCharacter(txtTicker.Text) + "'',''" + removeSpecialCharacter(txtTicker.Text) + "'',''" + txtprice.Text + "'',''" + cmbIndexType0.SelectedItem.Text + "'',''" + cmbIndexType.SelectedItem.Text + "'')    insert into Market_Data ([Ticker]      ,[ISIN]      ,[Best_Ask]      ,[Best_bid]      ,[Current_price]      ,[Ask_Volume]      ,[Bid_Volume]      ,[Prev_Price]      ,[counter_type]) values (''" + txtTicker.Text + "'',''" + txtTicker.Text + "'',''0'',''0'',''" + txtprice.Text + "'',''0'',''0'',''0'',''equity'')   insert into Market_data_History ([Ticker]      ,[ISIN]      ,[Best_Ask]      ,[Best_bid]      ,[Current_price]      ,[Ask_Volume]      ,[Bid_Volume]      ,[Prev_Price]      ,[counter_type],[Date],[CaptureDate] ) values (''" + txtTicker.Text + "'',''" + txtTicker.Text + "'',''0'',''0'',''" + txtprice.Text + "'',''0'',''0'',''0'',''equity'', getdate(),getdate())"
            Dim descr As String = "Added a new Security with the following details. Company Ticker: " + removeSpecialCharacter(txtTicker.Text) + "   Full Company Name=" + txtSecName.Text + " Issued Shares:" + txtIssuedCapital.Text + " Registrar:" + sec + " Address:" + txtRegdOffice1.Text + " " + txtRegdOffice2.Text + " " + txtRegdOffice3.Text + " " + txtRegdOffice4.Text + " Country:" + countr + " Security Type:EQUITY  ISIN: " + txtISIN.Text + "  Year Listed: " + dtYear.Text + " Nominal Value: " + txtNominalValue.Text + " Status:" + dealallowed + ""
    ...
    Protected Sub loadIndexType()
        Try
            cmd = New SqlCommand("select * from (select 2 as nr, 'UNQUOTED' as code, 'UNQUOTED' as fname union select 1 as nr,  code, fname from para_type) k order by k.nr", con)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "sec")
            If ds.Tables(0).Rows.Count > 0 Then
