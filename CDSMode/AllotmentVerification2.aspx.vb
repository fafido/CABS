...
Partial Class CDSMode_AllotmentVerification2
    ...
    Public Sub getCompany()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select batch_ref from broker_batch_header where status='F' order by batch_ref", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "broker_Batch_Header")
    ...
    Public Sub getBatch()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select * from broker_batch_header where batch_ref=" & txtSearch.Text & " and status='F'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "Broker_batch_header")
            If (ds.Tables(0).Rows.Count > 0) Then
                ...
                cmd = New SqlCommand("Select Company,Batch_Type,Shareholder as [Old Holder Number],Shares,Certificate,CD,CDS_Number,HolderNames from broker_Batch_Ref where batch_ref=" & lblbatchref.Text & " AND status='F'", cn)
                adp = New SqlDataAdapter(cmd)
