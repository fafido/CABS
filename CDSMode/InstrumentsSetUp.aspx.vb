...
Partial Class CDSMode_InstrumentsSetUp
    ...
    Protected Sub btnSaveCompany_Click(sender As Object, e As EventArgs) Handles btnSaveCompany.Click
        ...
        If rdbType.SelectedValue = "Update" Then
            ...
            End If
        Else
            Dim stmnt As String = "insert into para_company([Company],[Fnam],[Date_created],[Issued_shares],[registrar],[Add_1],[Add_2],[Add_3],[Add_4],[City],[Country],[Contact_Person],[Telephone],[Cellphone],[Fax],[Comments],[sec_type],[board],[index_type],[ticker],[ISIN],[year_listed],[comp_secretary],[email],[website],[issued_capital],[nominal_value]) values (''" & removeSpecialCharacter(txtSecName.Text) & "'',''" & removeSpecialCharacter(txtSecName.Text) & "'',''" & dtYear.Text & "'',''" & txtIssuedCapital.Text & "'',(select company_code from client_companies where company_name=''" & removeSpecialCharacter(txtcompsecretary.Text) & "''),''" & removeSpecialCharacter(txtRegdOffice1.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice2.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice3.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice4.Text) & "'','''',''" & cmbCountryListed.SelectedItem.Value & "'',''" & removeSpecialCharacter(txtcompsecretary.Text) & "'',''" & txtPhoneNo.Text & "'','''',''" & txtFaxNo.Text & "'','''',''" & cmbSecType.SelectedItem.Value & "'',''" & removeSpecialCharacter(txtBoard.Text) & "'',''" & cmbIndexType.SelectedItem.Value & "'',''" & removeSpecialCharacter(txtTicker.Text) & "'',''" & txtISIN.Text & "'',''" & dtYear.Text & "'',(select company_code from client_companies where company_name=''" & removeSpecialCharacter(txtcompsecretary.Text) & "''),''" & txtEmail.Text & "'',''" & txtWebsite.Text & "'',''" & txtIssuedCapital.Text & "'',''" & txtNominalValue.Text & "'')"
            Dim descr As String = "Added a new Instrument with the following details. Company=" + txtSecName.Text + "   Full Company Name=" + txtSecName.Text + " Issued Shares:" + txtIssuedCapital.Text + " Registrar:" + txtcompsecretary.SelectedItem.Text + " Address:" + txtRegdOffice1.Text + " " + txtRegdOffice2.Text + " " + txtRegdOffice3.Text + " " + txtRegdOffice4.Text + " Country:" + cmbCountryListed.SelectedItem.Text + " Security Type:" + cmbSecType.SelectedItem.Text + " Board:" + txtBoard.Text + "  Industry Type" + cmbIndexType.SelectedItem.Text + " Ticker:" + txtTicker.Text + " ISIN" + txtISIN.Text + "  Year Listed " + dtYear.Text + " Nominal Value:" + txtNominalValue.Text + " Status:" + cmbIndexType0.SelectedItem.Text + " Term:" + txtNominalValue0.Text + ""
    ...
    Protected Sub loadcurrencies()
        Try
            cmd = New SqlCommand("select * para_currencies where CurrencyStatus<>'Base'", con)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "sec")
            If ds.Tables(0).Rows.Count > 0 Then
                cmbcurrency.DataSource = ds.Tables(0)
                cmbcurrency.ValueField = "Currency"
                cmbcurrency.TextField = "Currency"
            Else
                cmbSecType.DataSource = Nothing
            End If
            cmbSecType.DataBind()
    ...
    Protected Sub txtNominalValue1_TextChanged(sender As Object, e As EventArgs) Handles txtNominalValue1.TextChanged

    End Sub

    Protected Sub txtSearchSecName_TextChanged(sender As Object, e As EventArgs) Handles txtSearchSecName.TextChanged
