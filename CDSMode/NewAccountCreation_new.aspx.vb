...
Partial Class TSecMode_NewAccountCreation
    ...
    Dim cn2 As New SqlConnection(constr)
    ...
    Protected Sub Unnamed13_Click(sender As Object, e As EventArgs)
        ...
        cmd = New SqlCommand("select * from AccountNewDetails", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsi, "Accounts_Audit")
        If (dsi.Tables(0).Rows.Count > 0) Then
            Dim dsh As New DataSet
            cmd = New SqlCommand("select max(ID) as ID from AccountNewDetails", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(dsh, "Accounts_Audit")
            password = 4
            password = CreateRandomPassword(Integer.Parse(password))
            cdsNo = (CInt(dsh.Tables(0).Rows(0).Item("ID").ToString + 1)).ToString.PadLeft(9, "0") & clientcompanyid(Session("BrokerCode")).ToString & password
        Else
        ...
        Using Cony As New SqlConnection(constr)
            Cony.Open()
            Using Comy As New SqlCommand("SELECT * from [dbo].[AccountNewCertificates] where CertificateNumber='" & certNumberTxt.Text & "' and Holder='" & holderTxt.Text & "' and Company='" & certCompany.SelectedValue &"'", Cony)
                Using RDR = Comy.ExecuteReader()
                    If RDR.HasRows Then
                        Do While RDR.Read
                            certAdded = True
        ...
        ElseIf accType0.SelectedIndex = 0 Then
            ...
            If Not userAdded Then
                ...
                Using Cony As New SqlConnection(constr2)
                    ...
                    '                replace(replace(cn.IdNumber,' ',''), '-', '')

                    If accType.SelectedIndex = 0 Then
                        styInd = "SELECT * from mast cm join names cn on cm.Shareholder=cn.Shareholder where (cm.Cert='" & certNumberTxt.Text & "' and cm.Shareholder='" & holderTxt.Text & "' and cm.Company='" & certCompany.Text & "' and replace(replace(cn.IDRNO,' ',''), '-', '')='" & idNumberTxt.Text.Replace(" ", "").Replace("-", "") & "') or (cm.Cert='" & certNumberTxt.Text & "' and cn.old_id='" & holderTxt.Text & "' and cm.Company='" & certCompany.Text & "' and replace(replace(cn.IDRNO,' ',''), '-', '')='" & idNumberTxt.Text.Replace(" ", "").Replace("-", "") & "')"
                    Else
                        styInd = "SELECT * from mast cm join names cn on cm.Shareholder=cn.Shareholder where (cm.Cert='" & certNumberTxt.Text & "' and cm.Shareholder='" & holderTxt.Text & "' and cm.Company='" & certCompany.Text & "') or (cm.Cert='" & certNumberTxt.Text & "' and cn.old_id='" & holderTxt.Text & "' and cm.Company='" & certCompany.Text & "')"
                    End If
                        ...
                        Using RDR = Comy.ExecuteReader()
                            ...
                            If RDR.HasRows Then
                                ...
                                For Each dr As DataRow In ds.Tables(0).Rows
                                    ...
                                    '                                If String.IsNullOrEmpty(dr.Item("OldId").ToString()) Then
                                    '
                                    '                                End If
                                    '
                                    '
                                    '
                                    '                                If dr.Item("CertNumber").ToString().Equals(dr.Item("OldId").ToString()) Then
                                    '
                                    '                                End If
                                    ...
                                    Using Conz As New SqlConnection(constr)
                                        Conz.Open()
                                        Using Comz As New SqlCommand("SELECT * from [dbo].[AccountNewCertificates] where CertificateNumber='" & cert & "'", Conz)
    ...
    Protected Sub Unnamed12_Click(sender As Object, e As EventArgs)
        ...
        If Not acccreated Then
            If accType0.SelectedIndex = 1 Then
                If Not userAdded Then
                    Dim queryQ = "INSERT INTO [dbo].[AccountNewDetails] ([CdsNumber], [IdNumber], [Surname], [Forename], [Date], [CapturedBy], [CompanyCode], [ISIN], [DateOfInc], AccountType, [Status]) VALUES (@CdsNumber, @IdNumber, @Surname, @Forename, @Date, @CapturedBy, @CompanyCode, @ISIN, @DateOfInc, @AccountType, @Status)"
                    Using con As New SqlConnection(constr)
                        Using com As New SqlCommand()
                            With com
                                .Connection = con
            ...
            ElseIf accType0.SelectedIndex = 0 Then
                ...
                '                msgbox(Session("totalShares").ToString())

                If String.IsNullOrEmpty(Session("totalShares").ToString()) Then
                    msgbox("Add shareholder certificates")
                    Exit Sub
                End If

                Dim alreadyAdd = False


                Try
                    Using Cony As New SqlConnection(constr)
                        Cony.Open()
                        Using Comy As New SqlCommand("SELECT * from [dbo].[AccountNewDetails] where IdNumber='" & ind & "' and Status='0'", Cony)
                            Using RDR = Comy.ExecuteReader()
                                If RDR.HasRows Then
                                    Do While RDR.Read
                                        alreadyAdd = True
                ...
                If Not alreadyAdd Then
                    ...
                    If Not String.IsNullOrEmpty(Session("totalShares").ToString()) Then
                        If Integer.TryParse(Session("totalShares").ToString(), trading) Then
                            If Integer.TryParse(Session("totalShares").ToString(), custody) Then
                                    ...
                                    Using Cony As New SqlConnection(constr)
                                        ...
                                        Using Comy As New SqlCommand("SELECT * from [dbo].[AccountNewDetails] where IdNumber='" & ind & "'", Cony)
                                            Using RDR = Comy.ExecuteReader()
                                                If RDR.HasRows Then
                                                    Do While RDR.Read
                                                        Session("CreatedNumber") = RDR.Item("CdsNumber").ToString()
                                                        '                                                    userAdded = True
                                                        '                                    msgbox("1")
                                                    Loop
                                                End If
                                ...
                                cmnd = New SqlCommand("Update [CDS].[dbo].[AccountNewDetails] SET [Custody] = '" & custody & "', [Trading] = '" & trading & "', [Total] = '" & Integer.Parse(Session("totalShares").ToString()) & "', AccountType='" & accType.SelectedValue & "', Status='0' Where IdNumber='" & ind & "'", cn)
                                ...
                                Try
                                    ...
                                    lastCreatedCds.Text = "Last Created ATP Number is " + cdsNo


                                    Session("CreatedNumber") = cdsNo
                                    msgbox("Account added succesfully. Your ATP number is " + cdsNo)
                    ...
                    End If
                Else
                    msgbox("Account Already initiated")
                End If
            End If
            '            End If
        Else
