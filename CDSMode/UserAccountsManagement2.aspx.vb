...
Partial Class CDSMode_UserAccountsManagement2
    ...
    Protected Sub ASPxButton1_Click(sender As Object, e As EventArgs) Handles ASPxButton1.Click
        ...
        Try
            ...
            'Dim reg As New Regex("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$")
            'If reg.IsMatch(txtEmail.Text) = True Then
        ...
        End If
            ...
            Dim subject As String = "New User Account Created"
            Dim body As String = "Your Account has been successfully created on Central Depository System. Your username is " + txtUsername.Text + " and your password is Resetpassword123 . Upon first Login you must change your password."
            Dim stmnt As String = "insert into systemUsers (company,companyCode,CompanyType,Username,Department,Password,Activation,Trail,PasswordExpireyDate,AccountType,Forenames,Surname,email) values (''" & cmbParticipants.SelectedItem.Text & "'',''" & txtParticipant.Text & "'',''" & txtParticipantType.Text & "'',''" & txtUsername.Text & "'',''" & txtDepartment.Text & "'',''" + base64Encode("Resetpassword123") + "'',''" + lock.ToString + "'',0,getdate(),''" & typedat & "'',''" & txtOthernames.Text & "'',''" & txtSurname.Text & "'',''" & txtEmail.Text & "'')"
            cmd = New SqlCommand(" insert into tbl_uncommitted ([description], script, [status], sender, date_inserted, comment, email_body, email_subject, email_to, category) values ('Created a new Broker user with First Name: " + txtOthernames.Text + ". Surname: " + txtSurname.Text + " under Participant: " + txtParticipant.Text + ". User email is " + txtEmail.Text + ". ID Number is " + txtIDno.Text + ". Department is " + txtDepartment.Text + ". Role is  " + txtRole.Text + "' , '" + stmnt + "', '0','" + Session("Username") + "', getdate(), '','" + body + "','" + subject + "', '" + txtEmail.Text + "','New User Created')", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
            ...
            Try
                ...
                Else
                    '    sendmail(txtEmail.Text, "New User Account Added", "Your Account has been successfully created on Central Depository System. Your username is " + txtUsername.Text + " and your password is Resetpassword123 . Upon first Login you must change your password.")
                End If
            ...
            msgbox("Account Sent for Authorization")
        txtDepartment.Text = ""
        txtEmail.Text = ""
        txtExpiryDate.Text = ""
        txtIDno.Text = ""
    ...
    Public Sub sendmail(emailAdd As String, subject As String, emailbody As String)
        Try
            ...
            Mail.From = New MailAddress("mAkibakenya@gmail.com")
            Mail.Body = emailbody
            ' Dim SMTP As New SmtpClient("smtp.googlemail.com")
            ...
            SMTP.Credentials = New System.Net.NetworkCredential("mAkibakenya@gmail.com", "pavilion$")
            SMTP.Port = "587"
            ' SMTP.Port = SMTPport
            SMTP.Send(Mail)
        Catch ex As Exception
