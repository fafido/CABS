...
Partial Class TSecMode_NewAccountCreation
    ...
    Protected Sub Unnamed13_Click(sender As Object, e As EventArgs)
        ...
        If accType0.SelectedIndex = 1 Then
            If Not userAdded Then
                Dim queryQ = "INSERT INTO [dbo].[AccountNewDetails] ([CdsNumber], [IdNumber], [Surname], [Forename], [Date], [CapturedBy], [CompanyCode], [ISIN], [DateOfInc], [custodian]) VALUES (@CdsNumber, @IdNumber, @Surname, @Forename, @Date, @CapturedBy, @CompanyCode, @ISIN, @DateOfInc,@custodian)"
                Using con As New SqlConnection(constr)
                    Using com As New SqlCommand()
                        With com
                            ...
                            .Parameters.AddWithValue("@Custodian", Session("BrokerCode").ToString())
                        End With
                        Try
                            con.Open()
                            com.ExecuteNonQuery()
    ...
    Protected Sub Unnamed12_Click(sender As Object, e As EventArgs)
        ...
        If Not acccreated Then
            If accType0.SelectedIndex = 1 Then
                If Not userAdded Then
                    Dim queryQ = "INSERT INTO [dbo].[AccountNewDetails] ([CdsNumber], [IdNumber], [Surname], [Forename], [Date], [CapturedBy], [CompanyCode], [ISIN], [DateOfInc], AccountType, [Status],[Custodian]) VALUES (@CdsNumber, @IdNumber, @Surname, @Forename, @Date, @CapturedBy, @CompanyCode, @ISIN, @DateOfInc, @AccountType, @Status,@custodian)"
                    Using con As New SqlConnection(constr)
                        Using com As New SqlCommand()
                            With com
                                .Connection = con
