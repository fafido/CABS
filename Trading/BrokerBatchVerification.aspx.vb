...
Partial Class Trading_BrokerBatchVerification
    ...
    Public Sub getCompany()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select batch_ref from broker_batch_header where status='B' order by batch_ref", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "broker_Batch_Header")
    ...
    Public Sub getBatch()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select * from broker_batch_header where batch_ref=" & txtSearch.Text & " and status='B'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "Broker_batch_header")
            If (ds.Tables(0).Rows.Count > 0) Then
                LblBatchDate.Text = FormatDateTime(ds.Tables(0).Rows(0).Item("Created_On").ToString, DateFormat.LongDate)
                ...
                cmd = New SqlCommand("Select Company,Batch_Type as [Batch Type],Shares as [Shares],Certificate ,CDS_Number as [CDS Number],HolderNames as [Holder] from broker_Batch_Ref where batch_ref=" & lblbatchref.Text & "", cn)
                adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub btnForward_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnForward.Click
        Try
            If (Session("Username").ToString = lblBatchBy.Text) Then
    ...
    Public Sub UpdateDecliningBatch()
        Try
            cmd = New SqlCommand("Update broker_batch_header set status='X',RejectionReason='" & txtRejectionReason.Text & "' where batch_ref=" & lblbatchref.Text & "", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
