...
Partial Class Trading_OrdersCodeApproval 
    ...
    Public Sub GetPendingOrders()
        Try
            ...
            cmd = New SqlCommand("select distinct (OrderNumber) from OrdersSummary where ApprovalFlag=0 and ActivationCodeApp = '0' AND  PurchasingBroker='" & Session("BrokerCode").ToString & "'", cn)
    ...
    Public Sub GetSelectedOrderData()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select OrdersSummary .OrderNumber , OrdersSummary .Company , OrdersSummary .CDS_Number , OrdersSummary .Order_Quantity,OrdersSummary .OrderValue  , case OrdersSummary.OrderType  when 'PUR' then 'Purchase Order' when 'SAL' then 'Sell Order' end as OrderType , names.Surname , names.Forenames , OrdersSummary .OrderDate , OrdersSummary .PurchasingBroker , OrdersSummary .CapturedBy, OrdersSummary .BasePrice , OrdersSummary .TargetDate , OrdersSummary .ExpiryDate , OrdersSummary .OrderAttribute, case names.accountstate  when 1 then 'Account active' when 0 then 'Account inactive' end as AccountState  from OrdersSummary , names where OrdersSummary .ApprovalFlag = 0 and ActivationCodeApp = '0' AND OrdersSummary .CDS_Number = names.CDS_Number and ordersSummary.PurchasingBroker='" & Session("BrokerCode") & "' and ordersSummary.orderNumber='" & cmbOrders.Text & "'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "OrdersSummary")
            If (ds.Tables(0).Rows.Count > 0) Then
                lblOrderNum.Text = ds.Tables(0).Rows(0).Item("OrderNumber").ToString
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not IsPostBack) Then
            ...
            End If
            cmd = New SqlCommand("insert into UserActivityLog (UserName,Activity,TimeOfActivity,DateDone,CompanyCode) values ('" & Session("Username") & "','Accessed Orders Code Verification Form','" & Date.Now & "','" & Now.Date & "','" & Session("BrokerCode") & "')", cn)
    ...
    Protected Sub Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Save.Click
        Try
            ...
            cmd = New SqlCommand("select ActivationCode from ordersSummary where OrderNumber='" & lblOrderNum.Text & "'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(Dss, "ordersSummary")
            rdApprove.Checked = True

            If (Trim(Dss.Tables(0).Rows(0).Item("ActivationCode").ToString) = Trim(txtCodeActivation.Text)) Then
                If (rdApprove.Checked = True) Then
                    ...
                    If (lblBuyer0.Text = "Account active") Then
                        cmd = New SqlCommand("Update ordersSummary set ActivationCodeApp = '1' where PurchasingBroker='" & Session("BrokerCode") & "' and OrderNumber='" & lblOrderNum.Text & "'", cn)
                        If (cn.State = ConnectionState.Open) Then
                            cn.Close()
                        End If
                        cn.Open()
                ...
                cmd = New SqlCommand("insert into UserActivityLog (UserName,Activity,TimeOfActivity,DateDone,CompanyCode) values ('" & Session("Username") & "','Saved an order code verification','" & Date.Now & "','" & Now.Date & "','" & Session("BrokerCode") & "')", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
