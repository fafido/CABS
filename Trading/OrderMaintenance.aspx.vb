...
Partial Class Trading_OrderMaintenance
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not IsPostBack) Then
            ...
            GetSecurities()
            getCompany()
            txtPortfolioBalance.ReadOnly = True
            txtDateCreated.SelectedDate = Now.Date
            txtTargetDate.SelectedDate = Now.Date
            txtExpiryDate.SelectedDate = Today.AddDays(30)
            ...
            cmbMarketboard.SelectedValue = "Normal Board"
        End If
    End Sub
    Public Sub GetSecurities()
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                cmbSecurity.DataSource = ds.Tables(0)
                cmbSecurity.DataValueField = "SecCode"
    ...
    Public Sub getHolder()
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                ...
                If (rdSell.Checked = True) Then
                    cmd = New SqlCommand("select * from trans where cds_number='" & txtHolder.Text & "' and company='" & cmbCompany.SelectedItem.Text & "'", cn)
                    adp = New SqlDataAdapter(cmd)
                    Dim dsi As New DataSet
                    adp.Fill(dsi, "trans")
                    If (dsi.Tables(0).Rows.Count > 0) Then
                        cmd = New SqlCommand("select sum(shares) as shares from trans where company='" & cmbCompany.SelectedItem.Text & "' and cds_number='" & txtHolder.Text & "'", cn)
                        Dim rix As New DataSet
                        adp = New SqlDataAdapter(cmd)
                        adp.Fill(rix, "trans")
                        If (rix.Tables(0).Rows.Count > 0) Then
                            txtPortfolioBalance.Text = rix.Tables(0).Rows(0).Item("shares").ToString
                        End If
                    Else
                        txtPortfolioBalance.Text = 0.0
                        'msgbox("Shareholder does not hold shares in selected company")
                        'Exit Sub
                    End If
                End If
            Else
    ...
    Protected Sub txtOrderShares_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOrderShares.TextChanged
        Try
            If (IsNumeric(txtOrderShares.Text)) Then
                If (cmbOrderPref.SelectedItem.Text = "L") Then
                    Dim orderPrice As Double
                    If (txtSharePrice.Text <> "") Then
                    Else
                        txtSharePrice.Text = 0.01
                    End If
    ...
    Public Sub SaveNewMain()
        Try
            ...
            Else
                ...
                Exit Sub
            End If
            OrderValue = CDbl(txtOrderAmount.Text)
            If (rdBuy.Checked = True) Then
                OrderType = "BUY"
            End If
            If (rdSell.Checked = True) Then
                OrderType = "SELL"
            ...
            If (rdSell.Checked = True) Then
                ...
                cmd = New SqlCommand("select * from trans where company='" & cmbCompany.SelectedItem.Text & "' and cds_number='" & txtHolder.Text & "'", cn)
                adp = New SqlDataAdapter(cmd)
                adp.Fill(rosk, "trans")
                ...
                If (ds.Tables(0).Rows.Count > 0) Then
                    ...
                    AvailableShares = CInt((ds.Tables(0).Rows(0).Item("Shares").ToString))

                    If (AvailableShares >= CInt(txtOrderShares.Text)) Then
                    Else
                        msgbox("Entered shares are greater than portfolio shares")
                        Exit Sub
                    End If
                End If
    ...
    Protected Sub btnPrintStatement_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrintStatement.Click
        Try
            ...
            If (txtTargetDate.Text = "") Then
                msgbox("Enter a valid Target Date")
                Exit Sub
            End If
            If (txtExpiryDate.Text = "") Then
                ...
                Exit Sub
            End If
            If (rdBuy.Checked = False And rdSell.Checked = False) Then
                msgbox("Select a buy or sell option")
                Exit Sub
            End If
            'saveOrderMain()
            SaveNewMain()
            ...
            Mail.To.Add("" + email + "")
            Mail.From = New MailAddress("cdspresentation@gmail.com")
            Mail.Body = " Your order has been successfully placed for " & txtOrderShares.Text & " shares - " & cmbCompany.SelectedItem.Text & " .Your activation code " & Code & ". Thank you for using CDS. Enjoy our services "
            ...
            txtExpiryDate.SelectedDate = Today.AddDays(30)
            rdBuy.Checked = False
            rdSell.Checked = False
            txtNames.Text = ""
            txtPortfolioBalance.Text = ""
            txtOrderAmount.Text = ""
    ...
    Protected Sub btnSearchName_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchName.Click
        Try
            ...
            End If
            Dim DS As New DataSet
            cmd = New SqlCommand("Select surname+' '+forenames+' '+cds_number as namess from Accounts_Clients where surname like '" & txtSearch.Text & "%'", cn)
            adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub cmbOrderPref_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbOrderPref.SelectedIndexChanged
        If (cmbOrderPref.SelectedItem.Text = "M") Then
            txtSharePrice.Text = 0.0
            txtSharePrice.ReadOnly = True
        Else
            txtSharePrice.ReadOnly = False
            getSharePrice()
    ...
    Protected Sub txtSharePrice_TextChanged(sender As Object, e As EventArgs) Handles txtSharePrice.TextChanged
        Try
            If (txtSharePrice.Text <> "") Then
                If (cmbOrderPref.SelectedItem.Text = "L") Then
                    If (IsNumeric(txtSharePrice.Text)) Then
                        If (txtOrderShares.Text <> "") Then
                            txtOrderAmount.Text = CDbl(CInt(txtOrderShares.Text) * CDbl(txtSharePrice.Text))
                        Else
                            txtOrderAmount.Text = 0.0
                        End If
                    End If
                Else
                    txtOrderAmount.Text = 0.0
    ...
    Protected Sub rdSell_CheckedChanged(sender As Object, e As EventArgs) Handles rdSell.CheckedChanged
        Try
            If (rdSell.Checked = True) Then
                cmd = New SqlCommand("select * from trans where cds_number='" & txtHolder.Text & "' and company='" & cmbCompany.SelectedItem.Text & "'", cn)
                adp = New SqlDataAdapter(cmd)
