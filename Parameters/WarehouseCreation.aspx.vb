...
Partial Class Custodian_WarehouseCreation
    ...
    Public Sub getCity()
        Try
            ...
            cmd = New SqlCommand("select distinct (city) as cityy from para_city where country='" & cmbCountry.Text & "' order by cityy", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "para_city")
            If (ds.Tables(0).Rows.Count > 0) Then
                cmbCity.DataSource = ds.Tables(0)
                cmbCity.TextField = "cityy"
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs) Handles btnsave.Click
        Try
            ...
            End If
            If txtWarehouseName.Text = "" Then
                msgbox("Please Enter Warehouse Name")
                Exit Sub
            End If
            If txtWarehouseCode.Text = "" Then
                msgbox("Enter Accreditation Number")
            ...
            If txtRepresentative.Text = "" Then
                msgbox("Please Enter Representative")
                Exit Sub
            End If
            ...
            If btnsave.Text = "Save and Submit" Then
                ...
                Try
                    cmd = New SqlCommand("insert into [WarehouseCreation_temp] (Country,City,AgreementDate,[CreatedBy],[TotalCapacityMetricTons],[compoundwall],[Weighingscalespresent],[Dateposted] ,[WarehouseName],[WarehouseCode] ,[WarehouseAddresses],[Representative] ,[Indicapacityperunit] ,[Phone],[Mobileno] ,[Email], [Designation] ,[Googlelocation] ,[contactpersonPhoneno] ,[contactpersonMobileno], commoditystored,[noStorageUnits] ,[DimOfGodown] ,[DurationLease] ,[PoliceStation] ,[noEntry] ,[NoExit] ,[SecurityArragements] , [RegulatoryActions] ,[Nostoragefacilities] ,Onwed_Lease,tel,[Onwername] ,[weighbridgeaddress] ,[distance] ,[datecalibration] ,[wegcapacity] ,[wegcalibration] ,[duedate] ,[dateInst] ,[make_model],[OPERATOR],[Status]) values ('" & cmbCountry.SelectedItem.Text & "','" & cmbCity.SelectedItem.Text & "','" & txtAgreementDate.Text & "','" & Session("Username") & "','" & txtTotalCapacityMetricTons.Text.Replace(",", "") & "','" & txtbountwithfance.Text & "','" & weighbridge & "',getdate(), '" & txtWarehouseName.Text & "','" & txtWarehouseCode.Text & "','" & txtAddresses.Text & "','" & txtRepresentative.Text & "','" & txtindivcapacity.Text & "','" & txtPhone.Text & "', '" & txtMobile.Text & "','" & txtEmail.Text & "','" & txtDesignation.Text & "','" & txtgooglelocation.Text & "', '" & txtcontactpersonPhoneno.Text & "','" & txtcontactpersonMobileno.Text & "','', '" & txtNoStorageunits.Text & "','" & txtdeminsionsunits.Text & "','" & txtDurationoflease.Text & "','" & txtpolice.Text & "', '" & txtentry.Text & "','" & txtExit.Text & "','" & txtsecurityguards.Text & "','" & txtpastRegulatory.Text & "','" & txtNomberofStorage.Text & "','" & lease_onwed & "', '" & txtPhone.Text & "','" & txtOnwername.Text & "','" & txtweighbridgeaddress.Text & "' ,'" & txtdistance.Text & "','" & txtdatecalibration.Text & "' ,'" & txtwegcapacity.Text & "' ,'" & cmbwegcalibration.SelectedValue & "' ,'" & validateDate(txtduedate.Text) & "','" & validateDate(txtdateInst.Text) & "' ,'" & txtmake_model.Text & "','" + cmboperator.SelectedValue + "','Authorisation')", cn)
                    If cn.State = ConnectionState.Open Then
                        cn.Close()
                    End If
                    cn.Open()
            ...
            Else
                ...
                Try
                    cmd = New SqlCommand("UPDATE [WarehouseCreation_temp] SET Country='" & cmbCountry.Text & "',City='" & cmbCity.Text & "',AgreementDate='" & txtAgreementDate.Text & "',[TotalCapacityMetricTons]='" & txtTotalCapacityMetricTons.Text.Replace(",", "") & "',[compoundwall]='" & txtbountwithfance.Text & "',[Weighingscalespresent]='" & weighbridge & "',[WarehouseName]='" & txtWarehouseName.Text & "',[WarehouseAddresses]='" & txtAddresses.Text & "',[Representative]='" & txtRepresentative.Text & "',[Indicapacityperunit]='" & txtindivcapacity.Text & "',[Phone]='" & txtPhone.Text & "' ,[Mobileno] ='" & txtMobile.Text & "',[Email]='" & txtEmail.Text & "',[Designation]='" & txtDesignation.Text & "' ,[Googlelocation] ='" & txtgooglelocation.Text & "',[contactpersonPhoneno]='" & txtcontactpersonPhoneno.Text & "' ,[contactpersonMobileno]='" & txtcontactpersonMobileno.Text & "', commoditystored='',[noStorageUnits]='" & txtNoStorageunits.Text & "' ,[DimOfGodown]='" & txtdeminsionsunits.Text & "' ,[DurationLease]='" & txtDurationoflease.Text & "' ,[PoliceStation]='" & txtpolice.Text & "' ,[noEntry]='" & txtentry.Text & "' ,[NoExit]='" & txtExit.Text & "' ,[SecurityArragements]='" & txtsecurityguards.Text & "', [RegulatoryActions]='" & txtpastRegulatory.Text & "' ,[Nostoragefacilities]='" & txtNomberofStorage.Text & "' ,Onwed_Lease='" & lease_onwed & "',tel='" & txtPhone.Text & "',[Onwername]='" & txtOnwername.Text & "' ,[weighbridgeaddress]='" & txtweighbridgeaddress.Text & "' ,[distance]='" & txtdistance.Text & "' ,[datecalibration]='" & txtdatecalibration.Text & "' ,[wegcapacity]='" & txtwegcapacity.Text & "' ,[wegcalibration]='" & cmbwegcalibration.SelectedValue & "' ,[duedate]='" & validateDate(txtduedate.Text) & "' ,[dateInst]='" & validateDate(txtdateInst.Text) & "',[status]='Work in Progress' ,[make_model]='" & txtmake_model.Text & "' " & "  WHERE [WarehouseCode]='" & txtWarehouseCode.Text & "' AND Operator='" + cmboperator.SelectedValue + "')", cn)
                    If cn.State = ConnectionState.Open Then
                        cn.Close()
                    End If
                    cn.Open()
    ...
    Public Sub loadall()
        Dim ds As New DataSet
        cmd = New SqlCommand("  Select ID,[WarehouseName],[WarehouseCode],[WarehouseAddresses],[WarehouseCode] as [Accreditation Number],[Representative],[Type],[Insurance],[Phone],Coverage  from WarehouseCreation order by id desc", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Public Sub getphonecodes()
        Dim ds As New DataSet
        cmd = New SqlCommand("select phonecode from para_country where country='" & cmbCountry.Text & "'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "para_city")
        If (ds.Tables(0).Rows.Count > 0) Then
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("conpath").ConnectionString)
            Using cmd As New SqlCommand(sql_str, con)
                ...
                If dss.Tables(0).Rows.Count > 0 Then
                    ...
                    Try
                        cmboperator.SelectedValue = dr.Item("Operator").ToString
                    Catch ex As Exception
                    End Try
                    Try
                        cmbCountry.Value = dr.Item("Country").ToString
                    Catch ex As Exception
                        cmbCountry.SelectedIndex = -1
                    End Try
                    getCity()
                    Try
                        cmbCity.Value = dr.Item("City").ToString
                    Catch ex As Exception
                        cmbCity.SelectedIndex = -1
                    End Try
                    ...
                    Try
                        cmbwegcalibration.SelectedValue = dr.Item("wegcalibration").ToString
                    Catch ex As Exception
                    End Try
                    Try
    ...
    Sub getNExtWarehouseCode()
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("conpath").ConnectionString)
            Using cmd As New SqlCommand(sql_str, con)
                cmd.Parameters.AddWithValue("@recID", cmboperator.SelectedValue)
                Dim dss As New DataSet
                Dim adp = New SqlDataAdapter(cmd)
                adp.Fill(dss, "WarehouseCreation")
                If dss.Tables(0).Rows.Count > 0 Then
                    Dim nextNumber As Long = CLng(dss.Tables(0).Rows(0).Item("cntRec")) + 1
                    txtWarehouseCode.Text = cmboperator.SelectedValue & "-" & nextNumber.ToString.PadLeft(3, "0")
                End If
            End Using
        End Using
    End Sub
    ...
    Protected Sub btnsaveWIP_Click(sender As Object, e As EventArgs) Handles btnsaveWIP.Click
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("conpath").ConnectionString)
            Using cmd As SqlCommand = New SqlCommand
                ...
                cmd.Parameters.AddWithValue("@wegcalibration", cmbwegcalibration.SelectedValue)
                ...
                cmd.Parameters.AddWithValue("@OPERATOR", cmboperator.SelectedValue)
    ...
    Protected Sub grdReject_RowCommand(sender As Object, e As DevExpress.Web.ASPxGridView.ASPxGridViewRowCommandEventArgs)
        Dim myID As String = e.KeyValue.ToString
        'getExistingWIP(myID)
    End Sub
    Public Sub loadallReject()
        Dim ds As New DataSet
        cmd = New SqlCommand("  Select ID,[WarehouseName],[WarehouseCode],[WarehouseAddresses],[WarehouseCode] as [Accreditation Number],[Representative],[Type],[Insurance],[Phone],Coverage,comments as Reason  from WarehouseCreation_Temp where status =  'Rejected' and  CreatedBy='" & Session("Username") & "' and Closed=0 order by id desc", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "WarehouseCreation")
        If (ds.Tables(0).Rows.Count > 0) Then
            grdReject.DataSource = ds
            grdReject.DataBind()
        End If
    End Sub
