Partial Class Parameters_withdrawalreversal
    ...
    Public Sub getdetails()
        cmd = New SqlCommand("select *,(select top 1 surname+' '+forenames from cds_router.dbo.Accounts_clients_web where cds_number=[CDSC].[dbo].[CashTrans].cds_Number) as [CDS Number] from  [CDSC].[dbo].[CashTrans] where TransType='Withdraw' and convert(date, datecreated)>='" + cmbfrom.Text + "' and convert(date, datecreated)<='" + cmbto.Text + "' and id not in (select reversal_id from cdsc.dbo.pending_reversals)", cn)
