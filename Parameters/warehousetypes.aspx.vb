Partial Class Parameters_warehousetypes
    ...
    Protected Sub ASPxButton7_Click(sender As Object, e As EventArgs) Handles ASPxButton7.Click
        If txtType.Text <> "" And txtDesc.Text <> "" Then
            cmd = New SqlCommand("select * from Para_warehouse_Type where Commodity_Type='" & txtType.Text & "'", cn)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "Para_Security_Type")
            If ds.Tables(0).Rows.Count <= 0 Then
                cmd = New SqlCommand("insert into Para_warehouse_Type([Warehouse_Type],[Description])values('" & txtType.Text & "','" & txtDesc.Text & "')", cn)
    ...
    Protected Sub getSecurities_Types()
        Try
            cmd = New SqlCommand("select ID,Warehouse_Type as [TYPE],[Description] from Para_Warehouse_Type", cn)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "Para_Security_Type")
            If ds.Tables(0).Rows.Count > 0 Then
