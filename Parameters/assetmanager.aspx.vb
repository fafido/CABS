...
Partial Class Parameters_AssetManager
    ...
    Protected Sub ASPxButton7_Click(sender As Object, e As EventArgs) Handles ASPxButton7.Click
        If ASPxButton7.Text = "Save" Then
            If txtCode.Text <> "" And txtFname.Text <> "" Then
                cmd = New SqlCommand("select * from Para_AssetManager where AssetManagerCode='" & txtCode.Text & "'", cn)
                Dim ds As New DataSet
                adp = New SqlDataAdapter(cmd)
                adp.Fill(ds, "para_Country")
                If ds.Tables(0).Rows.Count <= 0 Then
                    cmd = New SqlCommand("insert into para_AssetManager(AssetManagerCode,AssetMananger) values ('" & txtCode.Text & "','" & txtFname.Text & "')", cn)
                    ...
                    clearall()
                Else
                    msgbox("Asset Manager Already Exists")
                    Exit Sub
                End If
            Else
        ...
        Else
            If txtCode.Text <> "" And txtFname.Text <> "" Then
                cmd = New SqlCommand("update para_AssetManager set AssetManagerCode='" & txtCode.Text & "',AssetMananger='" & txtFname.Text & "' where id='" + Label2.Text + "'", cn)
                If cn.State = ConnectionState.Open Then
                    cn.Close()
                End If
                cn.Open()
    ...
    Public Sub getinfortoedit(id As String)
        cmd = New SqlCommand("select * from para_AssetManager where id='" + id.ToString + "'", cn)
        Dim ds As New DataSet
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "info")
        If ds.Tables(0).Rows.Count > 0 Then
            txtCode.Text = ds.Tables(0).Rows(0).Item("AssetManagerCode").ToString
            txtFname.Text = ds.Tables(0).Rows(0).Item("AssetMananger").ToString
    ...
    Private Sub ASPxGridView2_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles ASPxGridView2.RowCommand
        ...
        ElseIf e.CommandArgs.CommandName.ToString = "Delete" Then
            cmd = New SqlCommand("DELETE FROM para_AssetManager where id='" + e.KeyValue.ToString + "' ", cn)
            If cn.State = ConnectionState.Open Then
                cn.Close()
            End If
            cn.Open()
