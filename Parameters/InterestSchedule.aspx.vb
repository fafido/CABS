Partial Class Parameters_InterestSchedule
    ...
    Public Sub getcompanies()
        cmd = New SqlCommand("select issuer+' Payment('+convert(nvarchar(50),payment_no)+')' as pay, id from interest_instr where Authorized=1", cn)
    ...
    Protected Sub ASPxButton7_Click(sender As Object, e As EventArgs) Handles ASPxButton7.Click
        ...
        If txtinterest.Text <> "" And txtDesc.Text <> "" Then
            ...
            strscript += "window.open('rptinterestpayment.aspx?issuer=" & isser().ToString & "&payNo=" & payno().ToString & "');"
            strscript += "</script>"
    ...
    Public Function isser() As String
        Try
            cmd = New SqlCommand("select issuer from interest_instr where id='" + txtid.Text + "'", cn)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
    ...
    Public Function payno() As String
        Try
            cmd = New SqlCommand("select payment_no from interest_instr where id='" + txtid.Text + "'", cn)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "Para_Security_Type")
            If ds.Tables(0).Rows.Count > 0 Then
