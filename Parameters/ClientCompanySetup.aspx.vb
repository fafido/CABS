...
Partial Class Parameters_ClientCompanySetup
    ...
    Public Function loadnomencle() As String
        Dim ds As New DataSet
        cmd = New SqlCommand("SELECT RIGHT('0000'+CAST(isnull(count(id),0)+(select isnull(count(*),0)+1 from client_companies_audit where approvedby is NULL) AS VARCHAR(3)),5) AS max1 from client_companies", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "tbl_ParticipantsType")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    End Sub
    Public Function NullExpert(fieldval As String, replacewith As String) As String
    ...
    Protected Sub ASPXButton6_click(sender As Object, e As EventArgs) Handles ASPxButton6.Click
        ...
        Else
            ownershiptype = ""
        End If
        Dim stmnt As String = "insert into Client_Companies_Audit_Temp (company_name,company_code,company_type,AccountManager,Account_Pass,Adress_1,Adress_2,Adress_3,adress_4,adress_5,contact_email,contact_phone,contact_person,status,[main_branch],[main_account],[trading_branch],[trading_account],[main_account_name],[trading_account_name],[trading_bank],[main_bank],[settlement_cycle],[main_branch_new], [trading_branch_new],[key_letter], [County],[NTN],[SECP],[fax],[website],[state],[OwnershipType],[MaxUsers],[CapturedBy],UpdateType)values ('" & txtCompanyName.Text & "','" & txtCode.Text & "','" & cmbType.Text & "','" & txtContact.Text & "','password','" & txtAdd1.Text & "','" & txtAdd2.Text & "','" & txtAdd3.Text & "','" & cmbCity.Text & "','" & cmbCountry.Text & "','" & txtEmail.Text & "','" & txtMobile.Text & "','" & txtContact.Text & "',1,'" + txtmainbic.Text + "','" + txtmainaccount.Text + "','','','" + txtmainaccountname.Text + "','','','" + txtmainbankname.Text + "','','" + txtmainbranch.Text + "','','','','" + txtNTN.Text + "','" + txtsecp.Text + "','" + txtfax.Text + "','" + txtwebsite.Text + "','" + cmbstate.Text + "','" + ownershiptype + "','" + txtmaxusers.Text + "','" + Session("Username") + "','" + updatetype + "') "
        ...
        msgbox("Record has been saved as work in progress")
    ...
    Protected Sub ASPxButton1_Click(sender As Object, e As EventArgs) Handles ASPxButton1.Click
        ...
        If txtmainbranch.Text = "" Then
            msgbox("Please Enter Main Branch Name!")
            Exit Sub
        End If
        ...
        Dim stmnt As String = "insert into Client_Companies_Audit (company_name,company_code,company_type,AccountManager,Account_Pass,Adress_1,Adress_2,Adress_3,adress_4,adress_5,contact_email,contact_phone,contact_person,status,[main_branch],[main_account],[trading_branch],[trading_account],[main_account_name],[trading_account_name],[trading_bank],[main_bank],[settlement_cycle],[main_branch_new], [trading_branch_new],[key_letter], [County],[NTN],[SECP],[fax],[website],[state],[OwnershipType],[MaxUsers],[CapturedBy],UpdateType)values ('" & txtCompanyName.Text & "','" & txtCode.Text & "','" & cmbType.SelectedValue.ToString & "','" & txtContact.Text & "','password','" & txtAdd1.Text.ToUpper & "','" & txtAdd2.Text & "','" & txtAdd3.Text & "','" & cmbCity.Text & "','" & cmbCountry.SelectedItem.Text & "','" & txtEmail.Text.ToLower & "','" & txtMobile.Text & "','" & txtContact.Text & "',1,'" + txtmainbic.Text + "','" + txtmainaccount.Text + "','','','" + txtmainaccountname.Text + "','','','" + txtmainbankname.Text + "','','" + txtmainbranch.Text.ToString + "','','','','" + txtNTN.Text + "','" + txtsecp.Text + "','" + txtfax.Text + "','" + txtwebsite.Text + "','" + cmbstate.Text + "','" + ownershiptype + "','" + txtmaxusers.Text + "','" + Session("Username") + "','" + updatetype + "') "
        stmnt = stmnt & " UPDATE Client_Companies_Audit_Temp SET Closed=1 where CapturedBy='" & Session("Username") & "' and Closed=0 "
        cmd = New SqlCommand(stmnt, cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
    ...
    Public Sub GetParticipants()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select distinct (Company_name) as company_name from client_companies where company_code not like '%-%'", cn)
            adp = New SqlDataAdapter(cmd)
    ...
    Public Sub GetParticipantsDetials()
        ...
        Dim ds As New DataSet
            ...
            cmbCountry.SelectedValue = ds.Tables(0).Rows(0).Item("adress_5").ToString

            GetCity()
            Getstates()

            If (cmbCity.Items.Count > 0) Then
                cmbCity.SelectedValue = ds.Tables(0).Rows(0).Item("adress_4").ToString
            End If
            cmbType.SelectedValue = ds.Tables(0).Rows(0).Item("Company_type").ToString
    ...
    Public Sub FillDetails()
        If cmbParticipants.SelectedItem.Text <> "NAYMAT" Then
        ...
        Else
            msgbox("You cannot edit CMC Participant")
    ...
    Public Sub loadcontactsgrid(code As String)
        ...
        cmd = New SqlCommand("SELECT [ID] AS [Entry ID],[Name]      ,[PhoneNo]    AS [Telephone]  ,[MobileNo]      ,[Designation]      ,[Email]      ,[Other]      ,[DateAdded]      ,[ParticipantCode]  FROM [CDS].[dbo].[Client_Companies_Contacts] where participantcode='" + code + "'", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub ASPxButton5_Click(sender As Object, e As EventArgs) Handles ASPxButton5.Click
        ...
        If txtphoneNo.Text <> "" Then
            If txtphoneNo.Text.Length <> 10 Then
                msgbox("Please enter 10 digits for phone number!")
                Exit Sub
            End If
    ...
    Public Sub loadallWIP()
        Dim ds As New DataSet
        cmd = New SqlCommand("  select ID, Company_Code, Company_Name,Company_Type from client_companies_Audit_Temp where CapturedBy='" & Session("Username") & "' and Closed=0 order by id desc", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Public Sub getExistingWIP(ByVal recID As String)
        '  Try
        Dim ds As New DataSet
        cmd = New SqlCommand("select * from Client_Companies_Audit_Temp where ID = '" & recID & "'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "client_companies")
        If (ds.Tables(0).Rows.Count > 0) Then
            txtAdd1.Text = ds.Tables(0).Rows(0).Item("Adress_1").ToString
            ...
            End Try
            Try
                txtmainbranch.text = ds.Tables(0).Rows(0).Item("main_branch_NEW").ToString
            Catch ex As Exception
            End Try
