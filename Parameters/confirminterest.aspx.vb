Partial Class Parameters_confirminterest
    ...
    Public Sub getcompanies()
        cmd = New SqlCommand("select issuer+' Payment('+convert(nvarchar(50),payment_no)+')' as pay, id from interest_instr where Authorized=0", cn)
    ...
    Protected Sub ASPxButton7_Click(sender As Object, e As EventArgs) Handles ASPxButton7.Click
        ...
        If txtinterest.Text <> "" And txtDesc.Text <> "" Then
            Dim per As Decimal = txtinterest.Text
            Dim rate As Decimal = per / 100
            cmd = New SqlCommand("declare @company nvarchar(50)=(select issuer from interest_instr where id='" + txtid.Text + "') declare @rate numeric(18,4)='" + per.ToString + "' declare @datefrom date='" + cmbdatedeclared.Text + "' declare @dateto date='" + cmbpaymentsdate.Text + "' insert into interest_payments  select @company, 1, (select payment_no from interest_instr where id='" + txtid.Text + "') as paymentno, cds_number, surname+' '+forenames, ADD_1, ADD_2, ADD_3, add_4, country, div_bank, div_branch, div_branch, Div_AccountNo, (select sum(shares) from trans where cds_number=Accounts_Clients.CDS_Number and company=@company) as bonds, getdate(), getdate(), 0, (select sum(shares) from trans where cds_number=Accounts_Clients.CDS_Number and company=@company and Date_Created<=@dateto)*@rate/100-(" + txttaxrate.Text + "/100*(select sum(shares) from trans where cds_number=Accounts_Clients.CDS_Number and company=@company and Date_Created<=@dateto)*@rate/100)*100 as interest,convert(numeric(18,2),(select sum(shares) from trans where cds_number=Accounts_Clients.CDS_Number and company=@company and Date_Created<=@dateto)*@rate/100-(" + txttaxrate.Text + "/100*(select sum(shares) from trans where cds_number=Accounts_Clients.CDS_Number and company=@company and Date_Created<=@dateto)*@rate/100))*100 as interestrounded FROM Accounts_Clients where cds_number in (select cds_number from trans where company=@company and Date_Created<=@dateto  group by cds_number having sum(shares)>0)  update interest_instr set Authorized='1', Authorizedby='" + Session("Username") + "', Tax=" + txttaxrate.Text + "/100*(select sum(shares) from trans where company=@company and Date_Created<=@dateto) where id='" + txtid.Text + "'", cn)
    ...
    Public Sub getdetails()
        cmd = New SqlCommand("select * from interest_instr where id='" + txtid.Text + "'", cn)
        Dim ds As New DataSet
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "Para")
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            txttaxrate.Text = ds.Tables(0).Rows(0).Item("tax_new")
        End If
    End Sub
End Class
