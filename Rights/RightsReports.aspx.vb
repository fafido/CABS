...
Partial Class Corp_RightsReports
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT company FROM Rights_instr order by company", cn)
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT div_no FROM Rights_instr where company=@company order by div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Sub activateLinks()
        If IsNumeric(cmbIssueNo.Text) = True Then
            getAssetManagers()
            HyperLink1.Text = "<a href='RightsReportsPreview.aspx?Company=" & cmbCompany.Value & "&IssueNo=" & cmbIssueNo.Value & "&RepType=CS&AssetManager=ALL' target='blank'>Control Summary</a>"
            HyperLink2.Text = "<a href='RightsReportsPreview.aspx?Company=" & cmbCompany.Value & "&IssueNo=" & cmbIssueNo.Value & "&RepType=DS&AssetManager=ALL' target='blank'>Rights Issue Schedule</a>"
            HyperLink3.Text = "<a href='RightsReportsPreview.aspx?Company=" & cmbCompany.Value & "&IssueNo=" & cmbIssueNo.Value & "&RepType=AS&AssetManager=ALL' target='blank'>Acceptance Schedule</a>"
    ...
    Sub getAssetManagers()
        Dim strSQL As String = ""
        strSQL = "SELECT * FROM (SELECT 'ALL' AS AssetManager,1 AS sn UNION SELECT DISTINCT AssetManager,2 as sn FROM rights_dets where company=@company AND Issue_no=@DivNo)H order by H.sn, H.AssetManager"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Protected Sub cmbAssetManager_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbAssetManager.SelectedIndexChanged
        HyperLink1.Text = "<a href='RightsReportsPreview.aspx?Company=" & cmbCompany.Value & "&IssueNo=" & cmbIssueNo.Value & "&RepType=CS&AssetManager=" & cmbAssetManager.Value & "' target='blank'>Control Summary</a>"
        HyperLink2.Text = "<a href='RightsReportsPreview.aspx?Company=" & cmbCompany.Value & "&IssueNo=" & cmbIssueNo.Value & "&RepType=DS&AssetManager=" & cmbAssetManager.Value & "' target='blank'>Rights Issue Schedule</a>"
        HyperLink3.Text = "<a href='RightsReportsPreview.aspx?Company=" & cmbCompany.Value & "&IssueNo=" & cmbIssueNo.Value & "&RepType=AS&AssetManager=" & cmbAssetManager.Value & "' target='blank'>Acceptance Schedule</a>"
    End Sub
End Class
