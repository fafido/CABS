...
Partial Class Corp_RightsFinalPosting
    ...
    Private Function ISSelectedData() As Boolean
        ...
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"RightsId"})
        ...
        End If
    End Function
    Private Function SomeSelectedDataMisMatchAmountToPayRIGHTS() As Boolean
        Dim myGridView As New ASPxGridView
        myGridView = grdPaymentsRights
    ...
    Sub PostRights()
        If ISSelectedData() = False Then
            ...
            Exit Sub
        End If
        If SomeSelectedDataMisMatchAmountToPayRIGHTS() = True Then
            msgbox("Rights to Post + Write-off should not exceed remaining Rights")
            Exit Sub
        End If
        PostTempData()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_PostRights"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Rights  Posted successfully, awaiting authorization') ; location.href='RightsPosting.aspx'</script>")
    End Sub
    ...
    Sub getRightsData()
        Dim strSQL As String = " "
        strSQL = "SELECT a.*,a.AcceptedRights-ISNULL(c.PaidShares,0) as RemainingShares FROM rights_dets a left outer join (SELECT b.company,b.Div_No,b.Shareholder,b.AssetManager,sum(b.Shares) as PaidShares FROM RightsPayments b WHERE ISNULL(b.Authorized,0) in (0,1) group by b.company,b.Div_No,b.Shareholder,b.AssetManager) c On a.company=c.Company and a.Issue_no=c.Div_No and a.Origin=c.Shareholder and a.AssetManager=c.AssetManager where a.company=@Company and a.Issue_no=@IssueNo and isnull(a.AcceptedRights,0)>0 AND (a.AcceptedRights-isnull(c.PaidShares,0)>0) Order by RemainingShares desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Protected Sub btnCalc_Click(sender As Object, e As EventArgs) Handles btnCalc.Click
        ...
        Dim totalwriteoffRights As Long = 0
        Dim myGridView As New ASPxGridView
        myGridView = grdPaymentsRights
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"RightsId"})
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                Dim AmountToPay As Double = 0, PostShares As Long = 0, writeoffRights As Long = 0
                Try
                    AmountToPay = CDbl(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("AmountPaidc"), GridViewDataColumn), "lblAmountPaid"), ASPxLabel).Text)
                Catch ex As Exception
                End Try
                Try
                ...
                End Try
                Try
                    writeoffRights = CLng(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("SharesWriteoffc"), GridViewDataColumn), "txtSharesWriteoff"), ASPxTextBox).Text)
                Catch ex As Exception
                End Try
                totalToPay += AmountToPay
                totalPostShares += PostShares
                totalwriteoffRights += writeoffRights
            End If
        Next
        lblTotalCashSelected.Text = "Total Amount Paid: " & totalToPay.ToString("#,#.##") & " . Rights to Post: " & totalPostShares.ToString("#,#") & " . Write-off Rights: " & totalwriteoffRights.ToString("#,#") & ""
    End Sub
    Sub PostTempData()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "DELETE FROM RightsPayments_temp WHERE PaidBy=@PaidBy"
            Using cmd As New SqlCommand(stmnt, cn)
        ...
        sharesPostDesc = "Rights"
        sharesWriteoffDesc = "Write-off Rights"
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"RightsId"})
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                Dim Shareholder As String = TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Origin"), GridViewDataColumn), "lblshareholder"), ASPxLabel).Text
                ...
                Dim PostShares As Long = 0, writeoffRights As Long = 0
                Try
                    PostShares = CLng(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("SharesToPayc"), GridViewDataColumn), "txtSharesToPay"), ASPxTextBox).Text)
                Catch ex As Exception
                End Try
                ...
                If PostShares > 0 Then
                    stmnt = " INSERT INTO RightsPayments_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [PaidBy], [PaymentType],[Shares])VALUES(@Company,@IssueNo,@Shareholder,@AssetManager,0,@PostedBy,'" & sharesPostDesc & "','" & PostShares & "') "
                End If
                If writeoffRights > 0 Then
                    stmnt = stmnt & " INSERT INTO RightsPayments_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [PaidBy], [PaymentType],[Shares])VALUES(@Company,@IssueNo,@Shareholder,@AssetManager,0,@PostedBy,'" & sharesWriteoffDesc & "','" & writeoffRights & "') "
                End If
                Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
                    Using cmd As New SqlCommand(stmnt, cn)
                        cmd.CommandType = CommandType.Text
    ...
    End Sub
    Function getRightsDataDSET() As DataSet
        Dim strSQL As String = " "
        strSQL = "SELECT a.*,a.AcceptedRights-ISNULL(c.PaidShares,0) as RemainingShares FROM rights_dets a left outer join (SELECT b.company,b.Div_No,b.Shareholder,b.AssetManager,sum(b.Shares) as PaidShares FROM RightsPayments b WHERE ISNULL(b.Authorized,0) in (0,1) group by b.company,b.Div_No,b.Shareholder,b.AssetManager) c On a.company=c.Company and a.Issue_no=c.Div_No and a.Origin=c.Shareholder and a.AssetManager=c.AssetManager where a.company=@Company and a.Issue_no=@IssueNo and isnull(a.AcceptedRights,0)>0 AND (a.AcceptedRights-isnull(c.PaidShares,0)>0) Order by RemainingShares desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
