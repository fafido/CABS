...
Partial Class Reporting_Clientmoney
    ...
    Protected Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Try
            ...
            strscript += "window.open('" + rep + ".aspx?Datefrom=" & txtDateFrom.Text & "&Dateto=" & txtDateTo.Text & "&Summarized=" + m202 + "&AssetManager=" + cmbassetmanager.SelectedItem.Value.ToString + "');"
            strscript += "</script>"
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        Try
            ...
            cmd = New SqlCommand("select cds_Number+' '+Name  as name, cds_number from (select forenames+' '+surname as [Name], CDS_Number, Add_1 as add1, Add_2 as add2 from Accounts_Clients where cds_Number in (select clientno from Client_AssetManagers where AssetManager='" + Session("BrokerCode") + "') ) j where j.name+''+j.cds_number like '%" + txtSeachName.Text + "%' order by cds_number", cn)
