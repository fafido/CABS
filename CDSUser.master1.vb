Partial Class CDSUser
    ...
    Public Sub setloggeduser()
        Dim ds As New DataSet
        cmd = New SqlCommand("select company, Companytype+' '+Accounttype as [AccountType], isnull(Surname,'')+' '+isnull(forenames,'') as [Names] from cds.dbo.SystemUsers where id='" + Session("newid") + "' ", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "tbl_SettlementSummary2")
        If (ds.Tables(0).Rows.Count > 0) Then
            label1.Text = "User: " + StrConv(ds.Tables(0).Rows(0).Item("Names"), vbProperCase)
