...
Partial Class Corp_MMCompute
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT a.company FROM MM_instr a where a.Authorize=2 order by a.company", cn)
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT a.div_no, format(a.date_payment,'MMMM-yyyy','en-us') + ' ' + a.div_type +' No. ' + convert(nvarchar(100),a.div_no) AS displayname FROM MM_instr a WHERE a.Authorize=2 and a.company=@company order by a.div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Sub ComputeDividend()
        If DivNoExists() = False Then
            msgbox("Instruction no longer at Computation stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_Create_MM"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction Computed successfully') ; location.href='Computation.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from MM_instr where Company=@Company and div_no=@div_no and Authorize=2"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Sub getDivDetails()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                PanelSAVE.Visible = True
            Else
                msgbox("Instruction not on Computation stage")
                clearFields()
            End If
        End Using
    End Sub
