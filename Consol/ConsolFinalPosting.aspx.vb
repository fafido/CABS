...
Partial Class Corp_ConsolFinalPosting
    ...
    Sub PostRights()
        If DivNoExists() = False Then
            msgbox("Consolidation instruction no longer at Final Authorization stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Consolidation Final Authorization successful') ; location.href='ConsolFinalPosting.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Consol_instr where Company=@Company and div_no=@div_no and Authorize=3"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
