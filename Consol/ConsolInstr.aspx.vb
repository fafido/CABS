...
Partial Class Corp_ConsolInstr
    ...
    Sub getNextDivnumber()
        ...
        strSQL = "SELECT ISNULL(MAX(div_no),0)+1 as NextDivNo FROM Consol_instr WHERE company=@Company"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Protected Sub btnDivsearch_Click(sender As Object, e As EventArgs) Handles btnDivsearch.Click
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                ElseIf Auth = 1 Then
                    msgbox("Consolidation Issue " & txtIssueNo.Text & " at Authorization stage,proceed to reject it or contact Admin for reflagging to be able to update it")
                    clearFields()
                ElseIf Auth = 2 Then
                    msgbox("Consolidation Issue " & txtIssueNo.Text & " at Computation stage, contact Admin for reflagging")
                    clearFields()
                ElseIf Auth = 3 Then
                    msgbox("Consolidation Issue " & txtIssueNo.Text & " already Computed, contact Admin for reflagging")
                    clearFields()
                ElseIf Auth = 4 Then
                    msgbox("Consolidation Issue " & txtIssueNo.Text & " already Computed/Posted, contact Admin for reflagging")
                    clearFields()
                End If
            Else
                msgbox("Invalid Consolidation Issue " & txtIssueNo.Text)
                clearFields()
            End If
        End Using
    End Sub
    ...
    Sub saveInstruction()
        If IsDate(txtDateClose.Text) = False Then
            ...
            Exit Sub
        End If
        If IsDate(txtDatePayment.Text) = False Then
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Using cmd As New SqlCommand(stmnt & stmntAUDIT, cn)
                ...
                cmd.Parameters.AddWithValue("@date_payment", validateDate(txtDatePayment.Text))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction saved, pending authorization') ; location.href='ConsolInstr.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Consol_instr where Company=@Company and div_no=@div_no"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Sub UpdateInstruction()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction updated, pending authorization') ; location.href='ConsolInstr.aspx'</script>")
    End Sub
End Class
