...
Partial Class Corp_ConsolPostingAuth
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT a.company,b.Fnam FROM ConsolPayments a join para_company b on a.company=b.Company where ISNULL(a.Authorized,0)=0 and a.PaidBy<>@PaidBy order by a.company", cn)
            cmd.Parameters.AddWithValue("@PaidBy", Session("Username"))
            Dim adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "ConsolPayments")
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT a.div_no FROM Consol_Instr a join (SELECT DISTINCT company,div_no FROM ConsolPayments where ISNULL(Authorized,0)=0 and company=@company and PaidBy<>@PaidBy) b on a.company=b.Company and a.div_no=b.Div_No WHERE a.company=@company order by a.div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Sub PostConsol()
        ...
        PostTempDataAuth()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_PostConsolAuth"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Consol. Issue Authorized/Rejected successfully') ; location.href='ConsolPostingAuth.aspx'</script>")
    End Sub
    ...
    Sub getConsolData()
        ...
        'clear all grids
        Dim strSQL As String = " "
        strSQL = "SELECT a.*,c.ID,c.AmountPaid,c.PaymentType,c.DatePosted,c.PaidBy,c.PaidShares FROM Consol_issue a join (SELECT b.ID,b.company,b.Div_No,b.Shareholder,b.AssetManager,b.AmountPaid,b.PaymentType,b.PaidBy,format(b.DatePaid,'dd-MMMM-yyyy hh:mm:ss','en-us') as DatePosted,b.Shares as PaidShares FROM ConsolPayments b WHERE ISNULL(b.Authorized,0) in (0) and b.PaidBy<>@PaidBy) c On a.company=c.Company AND a.Consol_No=c.div_no and a.Shareholder=c.Shareholder and a.AssetManager=c.AssetManager where a.company=@Company and a.Consol_No=@divNo Order by a.Shareholder"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Sub PostTempDataAuth()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "DELETE FROM ConsolPaymentsAuth_temp WHERE PaidBy=@PaidBy"
            Using cmd As New SqlCommand(stmnt, cn)
        ...
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                Dim stmnt As String = " "
                stmnt = " IF NOT EXISTS(SELECT TOP 1 H.* FROM ConsolPaymentsAuth_temp H where H.RecordID=@RecordID AND H.PaidBy=@PaidBy) begin INSERT INTO ConsolPaymentsAuth_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [DatePaid], [PaidBy], [PaymentType], [RecordID])SELECT [Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [DatePaid], @PaidBy, [PaymentType], @RecordID FROM ConsolPayments s WHERE s.ID=@RecordID AND ISNULL(s.Authorized,0)=0 end "
    ...
    Protected Sub btnCalc_Click(sender As Object, e As EventArgs) Handles btnCalc.Click
        ...
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                ...
                Catch ex As Exception
                End Try
                If PayType = "Consolidation Shares" Then
                    totalSharesToPay += sharesPay
                ElseIf PayType = "Write-off Consolidation Shares" Then
                    totalSharesWriteOff += sharesPay
                End If
            End If
        Next
        lblTotalCashSelected.Text = "Consolidation Shares to Post: " & totalSharesToPay.ToString("#,#") & " . Write-off Consolidation Shares: " & totalSharesWriteOff.ToString("#,#")
    End Sub
