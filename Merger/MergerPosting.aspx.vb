...
Partial Class Corp_MergerFinalPosting
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT company FROM Merger_instr where Authorize=3 order by company", cn)
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT div_no FROM Merger_instr where Authorize=3 and company=@company order by div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Sub PostMerger()
        ...
        If SomeSelectedDataMisMatchAmountToPayMerger() = True Then
            msgbox("Merger Shares to Post + Write-off should not exceed remaining Merger Shares")
            Exit Sub
        End If
        PostTempData()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_PostMerger"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Merger Shares Posted successfully, awaiting authorization') ; location.href='MergerPosting.aspx'</script>")
    End Sub
    ...
    Protected Sub btnCalc_Click(sender As Object, e As EventArgs) Handles btnCalc.Click
        ...
        Dim totalwriteoffMergerShares As Long = 0
        Dim myGridView As New ASPxGridView
        myGridView = grdPaymentsMerger
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"SEQ"})
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                Dim PostShares As Long = 0, writeofMergerShares As Long = 0
                Try
                ...
                End Try
                Try
                    writeofMergerShares = CLng(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("SharesWriteoffc"), GridViewDataColumn), "txtSharesWriteoff"), ASPxTextBox).Text)
                Catch ex As Exception
                End Try
                totalPostShares += PostShares
                totalwriteoffMergerShares += writeofMergerShares
            End If
        Next
        lblTotalCashSelected.Text = "Merger Shares to Post: " & totalPostShares.ToString("#,#") & " . Write-off Merger Shares: " & totalwriteoffMergerShares.ToString("#,#") & ""
    End Sub
    Sub PostTempData()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "DELETE FROM MergerPayments_temp WHERE PaidBy=@PaidBy"
            Using cmd As New SqlCommand(stmnt, cn)
        ...
        sharesPostDesc = "Merger Shares"
        sharesWriteoffDesc = "Write-off Merger Shares"
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"SEQ"})
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                ...
                Dim PostShares As Long = 0, writeoffMergerShares As Long = 0
                Try
                    PostShares = CLng(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("SharesToPayc"), GridViewDataColumn), "txtSharesToPay"), ASPxTextBox).Text)
                Catch ex As Exception
                End Try
                Try
                    writeoffMergerShares = CLng(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("SharesWriteoffc"), GridViewDataColumn), "txtSharesWriteoff"), ASPxTextBox).Text)
                Catch ex As Exception
                End Try
                Dim stmnt As String = " "
                If PostShares > 0 Then
                    stmnt = " INSERT INTO MergerPayments_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [PaidBy], [PaymentType],[Shares])VALUES(@Company,@IssueNo,@Shareholder,@AssetManager,0,@PostedBy,'" & sharesPostDesc & "','" & PostShares & "') "
                End If
                If writeoffMergerShares > 0 Then
                    stmnt = stmnt & " INSERT INTO MergerPayments_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [PaidBy], [PaymentType],[Shares])VALUES(@Company,@IssueNo,@Shareholder,@AssetManager,0,@PostedBy,'" & sharesWriteoffDesc & "','" & writeoffMergerShares & "') "
                End If
                Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
                    Using cmd As New SqlCommand(stmnt, cn)
                        cmd.CommandType = CommandType.Text
