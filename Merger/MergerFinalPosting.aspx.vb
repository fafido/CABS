...
Partial Class Corp_MergerFinalPosting
    ...
    Sub PostMerge()
        If DivNoExists() = False Then
            msgbox("Merge instruction no longer at Final Authorization stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_PostMerge"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Merge Final Authorization successful') ; location.href='MergerFinalPosting.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Merger_instr where Company=@Company and div_no=@div_no and Authorize=3"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
