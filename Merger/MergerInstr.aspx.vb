...
Partial Class Corp_MergerInstr
    ...
    Sub getNextDivnumber()
        ...
        strSQL = "SELECT ISNULL(MAX(div_no),0)+1 as NextDivNo FROM Merger_instr WHERE company=@Company"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Protected Sub btnDivsearch_Click(sender As Object, e As EventArgs) Handles btnDivsearch.Click
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                ElseIf Auth = 1 Then
                    msgbox("Merger Instruction No. " & txtIssueNo.Text & " at Authorization stage,proceed to reject it or contact Admin for reflagging to be able to update it")
                    clearFields()
                ElseIf Auth = 2 Then
                    msgbox("Merger Instruction No. " & txtIssueNo.Text & " at Computation stage, contact Admin for reflagging")
                    clearFields()
                ElseIf Auth = 3 Then
                    msgbox("Merger Instruction No. " & txtIssueNo.Text & " already Computed, contact Admin for reflagging")
                    clearFields()
                ElseIf Auth = 4 Then
                    msgbox("Merger Instruction No. " & txtIssueNo.Text & " already Computed/Posted, contact Admin for reflagging")
                    clearFields()
                End If
            Else
                msgbox("Invalid Merger Instruction No. " & txtIssueNo.Text)
                clearFields()
            End If
        End Using
    End Sub
    ...
    Sub saveInstruction()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction saved, pending authorization') ; location.href='MergerInstr.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Merger_instr where Company=@Company and div_no=@div_no"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Sub UpdateInstruction()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction updated, pending authorization') ; location.href='MergerInstr.aspx'</script>")
    End Sub
End Class
