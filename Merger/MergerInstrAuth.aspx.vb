...
Partial Class Corp_MergerInstrAUTH
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT company FROM Merger_instr where Authorize=1 and createdBy<>@createdBy order by company", cn)
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT div_no FROM Merger_instr where Authorize=1 and company=@company and createdBy<>@createdBy order by div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Sub getDivDetails()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Else
                msgbox("Merger Instruction not on authorization stage")
                clearFields()
            End If
        End Using
    End Sub
    ...
    Sub AuthorizeInstruction()
        If DivNoExists() = False Then
            msgbox("Merger Instruction longer at authorization stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = " DECLARE @LastAuditID numeric(18,0)=(select max(ID) FROM Merger_instrAuth WHERE company=@Company and div_no=@div_no) "
            stmnt = stmnt & "  UPDATE Merger_instr SET Authorize=2,AuthorizedBy=@createdBy WHERE company=@Company and div_no=@div_no and Authorize=1 and createdBy<>@createdBy "
            stmnt = stmnt & " UPDATE Merger_instrAuth SET AuthBy=@createdBy,AuthDate=GETDATE(),Authorize=2,AuthorizedBy=@createdBy WHERE ID =  @LastAuditID "
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction authorized successfully') ; location.href='MergerInstrAuth.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Merger_instr where Company=@Company and div_no=@div_no and Authorize=1 and createdBy<>@createdBy"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Sub RejectInstruction()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = " DECLARE @LastAuditID numeric(18,0)=(select max(ID) FROM Merger_instrAuth WHERE company=@Company and div_no=@div_no) "
            stmnt = stmnt & "  UPDATE Merger_instr SET Authorize=0,AuthorizedBy=@createdBy WHERE company=@Company and div_no=@div_no and Authorize=1 and createdBy<>@createdBy "
            stmnt = stmnt & " UPDATE Merger_instrAuth SET AuthBy=@createdBy,AuthDate=GETDATE(),Authorize=0,AuthorizedBy=@createdBy WHERE ID =  @LastAuditID "
            Using cmd As New SqlCommand(stmnt, cn)
                cmd.Parameters.AddWithValue("@company", cmbCompany.Value)
                cmd.Parameters.AddWithValue("@div_no", cmbIssueNo.Value)
                cmd.Parameters.AddWithValue("@createdBy", Session("Username"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction has been rejected') ; location.href='MergerInstrAuth.aspx'</script>")
    End Sub
End Class
