...
Partial Class Corp_MergerCompute
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT company FROM Merger_instr where Authorize=2 order by company", cn)
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT div_no FROM Merger_instr where Authorize=2 and company=@company order by div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Sub ComputeMerger()
        If DivNoExists() = False Then
            msgbox("Merger Instruction no longer at Computation stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_Create_Merger_New"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Merger Instruction Computed successfully') ; location.href='MergerComputation.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Merger_instr where Company=@Company and div_no=@div_no and Authorize=2"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
