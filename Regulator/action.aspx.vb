...
Partial Class TransferSec_action
    ...
    Public Sub Bindnw()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("use testcds select orderno,clientname, broker_code as [Broker], company as [Counter], case baseprice when '0.00' then (select initialprice from para_company where company=Audit_Order_Posted_Log.company) else baseprice end as [Price], quantity as [Quantity], case baseprice when '0.00'  then Quantity*(select initialprice from para_company where company=testcds.dbo.Audit_Order_Posted_Log.Company) else Quantity*BasePrice   end as [Value]  from testcds.dbo.Audit_Order_Posted_Log  where Inserted_datetime between '" + ASPxDateEdit1.Text + "' and '" + ASPxDateEdit2.Text + "' and company='" + DropDownList2.SelectedItem.Text + "' and broker_code='" + DropDownList1.SelectedValue.ToString + "' order by orderno desc", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "ordersSummary")
            If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Protected Sub grdEvent0_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdEvent0.SelectedIndexChanged
        cmd = New SqlCommand("delete from Audit_Order_Posted_Log where orderno='" + grdEvent0.SelectedValue.ToString + "'", cn)
