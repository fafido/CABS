...
Partial Class TransferSec_ApproveTransfer
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand(" select t.id,t.Narration, af.Surname+' '+af.Forenames as [Transfer From], ato.Surname+' '+ato.Forenames as [Transfer To], t.Amount, t.DateCreated, t.ValueDate, t.Currency , t.BankAccount,t.AssetManager    from cashtransfer  t,  Accounts_Clients af, Accounts_clients ato where t.FromAccount=af.CDS_Number and t.ReAllocateTo=ato.CDS_Number and t.ApprovedBy is NULL", cn)
    ...
    Public Sub approvetrans(transid As String)
        Try
            ...
            cmd = New SqlCommand(" insert into cashtrans ( [Description]      ,[TransType]      ,[Amount]      ,[DateCreated]      ,[TransStatus]      ,[CDS_Number]      ,[Paid]      ,[Reference]      ,[ChargeCode]      ,[AssetManager]      ,[BankName]      ,[BankAccount]      ,[Ref2]      ,[PostedBy]      ,[Currency])    select Narration,Narration, Amount*-1, ValueDate , '1', FromAccount, '1', id, '', AssetManager, (select top 1 Bank from para_assetManager_Banks  where AccountNo=cashtransfer.BankAccount) , BankAccount, id, '" + Session("Username") + "', Currency   from cashtransfer  where id='" + transid + "' and Approvedby is NULL   insert into cashtrans ( [Description]      ,[TransType]      ,[Amount]      ,[DateCreated]      ,[TransStatus]      ,[CDS_Number]      ,[Paid]      ,[Reference]      ,[ChargeCode]      ,[AssetManager]      ,[BankName]      ,[BankAccount]      ,[Ref2]      ,[PostedBy]      ,[Currency])    select Narration, Narration, Amount, ValueDate , '1', ReAllocateTo, '1', id, '', AssetManager, (select top 1 Bank from para_assetManager_Banks  where AccountNo=cashtransfer.BankAccount) , BankAccount, id, '" + Session("Username") + "', Currency   from cashtransfer  where id='" + transid + "' and Approvedby is NULL update cashtransfer set ApprovedBy='" + Session("Username") + "', ApprovedDate=getdate() where id='" + transid + "'", cn)
    ...
    Public Sub rejecttrans(transid As String)
        Try
            cmd = New SqlCommand("update cashtransfer set rejected='1', ApprovedDate=getdate(),ApprovedBy='" + Session("Username") + "' where id='" + transid + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
