...
Partial Class TransferSec_AccountsCreations2
    ...
    Public Sub getbrokers()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select * from client_companies where company_type='Broker'", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "tbl_")
            If (ds.Tables(0).Rows.Count > 0) Then
                cmbCustodian0.DataSource = ds.Tables(0)
                cmbCustodian0.TextField = "Company_name"
                cmbCustodian0.DataBind()
            End If
    ...
    Public Sub SaveNewAccount()
        Try
            ...
            If (dsi.Tables(0).Rows.Count > 0) Then
                ...
                cdsNo = (CInt(dsh.Tables(0).Rows(0).Item("ID").ToString + 1)).ToString.PadLeft(10, "0") & password
            ...
            If (Len(txtEmail.Text) > 1) Then
                ...
                '***14/08/2014 SJ commenting out activation code notification
                ' Mail.Body = " Your account  has been successfully created. Your account activation number: " & code & " Enjoy our services "
                Mail.Body = " Your account  has been successfully created. Enjoy our services "
