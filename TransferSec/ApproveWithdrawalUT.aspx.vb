...
Partial Class TransferSec_ApproveWithdrawalUT
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("  select *,'Redemption' as [Type] from UT_Redemption where [status]='PENDING'", cn)
    ...
    Public Sub approvetrans(id As String)
        Try
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number, date_created, trans_time, shares, Update_Type, Created_By, Batch_Ref, source, Pledge_shares, reference) select company, cds_number, getdate(), getdate(), quantity*-1, 'Fund Redemption', '" + Session("Username") + "', '" + id + "', 'D', 0, '0' from UT_Redemption where id='" + id + "' insert into CashTrans_Audit ([Description],TransType, Amount, DateCreated, TransStatus , cds_Number, PAID, Reference, ChargeCode, [Type], [CapturedBy],BeneficiaryBank, BeneficiaryBranch, BeneficiaryAccount, BeneficiaryAccountName)  select 'Fund Redemption','Fund Redemption', Amount  , getdate(), '0' , cds_Number, NULL, ID, '', 'PAYMENT', '" + Session("Username") + "',Bank , '', BankAccount , CDS_Number  from UT_Redemption where id='" + id + "' update UT_Redemption set [status]='Approved' where id='" + id + "'", cn)
        ...
        End Try
    End Sub
    Public Sub rejecttrans(id As String)
        Try
            cmd = New SqlCommand("update UT_Redemption set [status]='REJECTED' where id='" + id + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
