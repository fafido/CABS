...
Partial Class TransferSec_ApproveAllocation
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("  select *,'Allocation' as [Type], a.Surname+' '+a.forenames as [Names] from bond_Allocation b, Accounts_clients a where b.[status]='PENDING' and a.cds_number=b.cds_number", cn)
    ...
    Public Sub approvetrans(transid As String)
        Try
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number, date_created, trans_time, shares, Update_Type, Created_By, Batch_Ref, source, Pledge_shares, reference) select company, cds_number, getdate(), getdate(), quantity, 'Trustee Allocation', '" + Session("Username") + "', Certificate, 'D', 0, '0' from bond_Allocation where id='" + transid + "' update Bond_Allocation set [status]='Approved' where id='" + transid + "'", cn)
    ...
    Public Sub rejecttrans(transid As String)
        Try
            cmd = New SqlCommand("update Bond_Allocation set [status]='Rejected' where id='" + transid + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
