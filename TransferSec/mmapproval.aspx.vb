...
Partial Class mmapproval
    ...
    Public Function pending(assetmanager As String, dat As String) As DataSet
        ...
        cmd = New SqlCommand("select * from Recon_AssetManager_Fixed_Approvals where [Approved] is NULL and convert(date, ForDate)='" + dat + "' and AssetManager='" + assetmanager + "' and Rejected is NULL", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Public Sub approvetransaction(transkey As String)
        Try
            ...
            cmd = New SqlCommand("UPDATE Recon_AssetManager_Fixed_Approvals set Approved='1', ApprovedBy='" + Session("Username") + "', DateApproved= getdate() where   id='" + transkey + "' AND Approved is NULL", cn)
    ...
    Public Sub DECLINE(transkey As String)
        Try
            ...
            cmd = New SqlCommand("UPDATE Recon_AssetManager_Fixed_Approvals set [Rejected]='1', ApprovedBy='" + Session("Username") + "', DateApproved= getdate()  where  id='" + transkey + "' AND currentStatus='Pending'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
