...
Partial Class Assetmanbond
    ...
    Public Function correctcompany(company As String) As Boolean
        Dim dsport As New DataSet
        cmd = New SqlCommand("select code from bond where code='" + company + "'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
            Return True
    ...
    Public Sub deleteprev(assetmanager As String, company As String, accountnumber As String)
        cmd = New SqlCommand("delete from recon_AssetManager_Bond where AssetManager='" + assetmanager + "' and Company='" + company + "' and AccountNumber='" + accountnumber + "' ", cn)
        ...
        If fileupload1.HasFile Then
            ...
            For x = 1 To dtExcelRecords.Rows.Count - 1                                                                                                                                                                                      '                                                                                                                                                                                                                                                                                                                               ImportID,Company,Date_trade,CDS_Ref,Date_Settlement,Client_Id,Other_Names,Surname,Buy_Sell,Quantity,Price,UploadDate)
                ...
                If correctaccounts(dr.Item(1).ToString) = False Then
                    ...
                    Exit Sub
                End If
                If correctassetmanager(dr.Item(3).ToString) = False Then
                    msgbox("There is an invalid Asset Manager in the system. Invalid Asset Manager is " + dr.Item(3).ToString + "")
                    Exit Sub
                End If

                If correctcompany(dr.Item(4).ToString) = False Then
                    msgbox("There is an invalid Bond in the system. Invalid Bond is " + dr.Item(4).ToString + "")
                    Exit Sub
                End If


                Dim cmd2 As New SqlCommand
                Dim cmdStr As String = "insert into Recon_AssetManager_Bond ([DateUploaded] ,[ForDate] ,[AccountNumber] ,[CSDAccount] ,[Name] ,[AssetManager],[Company] ,[Units] ,[MarketValue],[UploadedBy],[SystemUploadBy], [RecordDate]) values (@DateUploaded ,@ForDate ,@AccountNumber ,@CSDAccount ,@Name ,@AssetManager,@Company ,@Units ,@MarketValue,@UploadedBy,@SystemUploadBy, getdate())"
                ...
                cmd2.Parameters.AddWithValue("@CSDAccount", "")
                cmd2.Parameters.AddWithValue("@Name", dr.Item(2).ToString)
                cmd2.Parameters.AddWithValue("@AssetManager", dr.Item(3).ToString)
                cmd2.Parameters.AddWithValue("@Company", dr.Item(4).ToString)
                cmd2.Parameters.AddWithValue("@Units", dr.Item(5).ToString.Replace(",", ""))
                cmd2.Parameters.AddWithValue("@MarketValue", dr.Item(6).ToString.Replace(",", ""))
    ...
    Public Sub getview()
        ...
        Try
            ...
            If cmbtype.SelectedItem.Text = "Exceptions" Then
                cmd = New SqlCommand(" declare @company nvarchar(50)='ALL';  declare @date date='" + dtdateview.Date.ToString + "' select *, [C-Trade Value]-[Asset Manager Value] as [Value Variance], case when [C-Trade Value]-[Asset Manager Value]>0 then 'Less Value at Asset Manager' when  [C-Trade Value]-[Asset Manager Value]<0  then 'More Value at Asset Manager' when [C-Trade Value]-[Asset Manager Value]=0 then 'Balancing' end as [Value Status] from (select *, [Account No]+' '+ [Security]+' '+Assetmanager+''+convert(nvarchar(50),@date) as [ID], custodial.dbo.moneymarketgain(IssueDate,Rate,DayCountBasis,[C-Trade Holding] , 0 , @date)+[C-Trade Holding] as [C-Trade Value], MarketValue as [Asset Manager Value]  from (select *, case when j.variance>0 then 'Less units at Asset Manager' when j.Variance<0 then 'More units at Asset Manager' else 'Balancing' end as [Status] from ( select t.cds_number as [Account No], a.Surname+' '+a.Forenames as [Names],   t.Company as [Security], AssetManager, isnull(sum(t.shares),0) as [C-Trade Holding] , (select isnull(sum(units),0) from Recon_AssetManager_Bond where company=t.Company and AssetManager=t.AssetManager and AccountNumber=t.CDS_Number and convert(date, ForDate)=@date  ) as [Asset Manager Holding],(select isnull(sum(MarketValue),0) from Recon_AssetManager_Bond where company=t.Company and AssetManager=t.AssetManager and AccountNumber=t.CDS_Number and convert(date, ForDate)=@date  )  as [MarketValue], b.IssueDate, b.Rate,   b.DayCountBasis  ,isnull(sum(t.shares),0)-  (select isnull(sum(units),0) from Recon_AssetManager_Bond where company=t.Company and AssetManager=t.AssetManager and AccountNumber=t.CDS_Number and convert(date, ForDate)=@date) as [Variance]   from trans_recon t, Accounts_Clients a, Bond b where a.CDS_Number = t.CDS_Number and t.company = b.Code     and convert(date, t.date_created)<=@date  group by t.CDS_Number, a.Surname, a.Forenames, t.Company,t.AssetManager, b.IssueDate, b.Rate , b.DayCountBasis ) j) z) zz  where  [Account No]+' '+ [Security]+' '+Assetmanager+''+convert(nvarchar(50),@date) NOT IN (select Reference from Recon_AssetManager_Approvals_Bond where CurrentStatus in ('Pending','APPROVED'))  order by [Account No]", cn)
            End If
    ...
    Public Function view_auth() As DataSet
        Try
            ...
            cmd = New SqlCommand("select * from Recon_AssetManager_Approvals_Bond where CurrentStatus='Approved' and convert(date, ReconDate)='" + dtdateview.Date.ToString + "' order by [Account No]", cn)
    ...
    Public Sub getview_authorized()
        ...
        Try
            ...
            If cmbtype.SelectedItem.Text = "Exceptions" Then
                cmd = New SqlCommand("select *,[CtradeValue] as [C-Trade Value]     ,[AssetManagerValue] as [Asset Manager Value]      ,[ValueVariance]    as [Value Variance]   ,[ValueStatus] as [Value Status] from Recon_AssetManager_Approvals_Bond where CurrentStatus='Approved' and convert(date, ReconDate)='" + dtdateview.Date.ToString + "' order by [Account No]", cn)
    ...
    Public Sub sendforauthorization(id As String, comment As String)
        ...
        cmd = New SqlCommand("declare @company nvarchar(50)='ALL';  declare @date date='" + dtdateview.Date.ToString + "' INSERT INTO Recon_AssetManager_Approvals_Bond  ([Account No]      ,[Names]      ,[Security]      ,[AssetManager]      ,[C-Trade Holding]      ,[Asset Manager Holding] ,    [Variance] ,[AssetManagerValue]       ,[Status]      ,[Reference]      ,[DateAuthorized]      ,[AuthorizedBy]      ,[CurrentStatus]      ,[Comment]      ,[ReconDate]      ,[CtradeValue]        ,[ValueVariance]      ,[ValueStatus])  select [Account No]      ,[Names]      ,[Security]      ,[AssetManager]      ,[C-Trade Holding]      ,[Asset Manager Holding] ,    [Variance] , MarketValue       ,[Status]      ,[Reference]      ,[DateAuthorized]      ,[AuthorizedBy]      ,[CurrentStatus]      ,[Comment]      ,@date     ,[C-Trade Value]       ,[C-Trade Value]-MarketValue    , case when [C-Trade Value]-[MarketValue]>0 then 'Less Value at Asset Manager' when  [C-Trade Value]-[MarketValue]<0  then 'More Value at Asset Manager' when [C-Trade Value]-[MarketValue]=0 then 'Balancing' end as [Value Status]  from ( select *, [Account No]+' '+ [Security]+' '+Assetmanager+''+convert(nvarchar(50),@date) as [Reference], NULL AS [DateAuthorized], NULL as AuthorizedBy,'Pending' as [CurrentStatus], '" + comment + "' AS Comment, @date as recdate, custodial.dbo.moneymarketgain(IssueDate,Rate,DayCountBasis,[C-Trade Holding] , 0 , @date)+[C-Trade Holding] as [C-Trade Value]    from (select *, case when j.variance>0 then 'Less units at Asset Manager' when j.Variance<0 then 'More units at Asset Manager' else 'Balancing' end as [Status] from ( select t.cds_number as [Account No], a.Surname+' '+a.Forenames as [Names],   t.Company as [Security], AssetManager, isnull(sum(t.shares),0) as [C-Trade Holding], b.IssueDate, b.Rate,   b.DayCountBasis , (select isnull(sum(units),0) from Recon_AssetManager_Bond where company=t.Company and AssetManager=t.AssetManager and AccountNumber=t.CDS_Number and convert(date, ForDate)=@date  ) as [Asset Manager Holding] ,isnull(sum(t.shares),0)-  (select isnull(sum(units),0) from Recon_AssetManager_Bond where company=t.Company and AssetManager=t.AssetManager and AccountNumber=t.CDS_Number and convert(date, ForDate)=@date) as [Variance], (select isnull(sum(MarketValue),0) from Recon_AssetManager_Bond where company=t.Company and AssetManager=t.AssetManager and AccountNumber=t.CDS_Number and convert(date, ForDate)=@date) as [MarketValue]    from trans_recon t, Accounts_Clients a, Bond b  where a.CDS_Number = t.CDS_Number and b.code=t.company     and convert(date, t.date_created)<=@date  group by t.CDS_Number, a.Surname, a.Forenames, t.Company,t.AssetManager, b.Rate, b.IssueDate, b.DayCountBasis ) j) z) zz  where  [Account No]+' '+ [Security]+' '+Assetmanager+''+convert(nvarchar(50),@date)='" + id + "'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
