...
Partial Class TransferSec_AFT
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        Try
            ...
            If ASPxButton13.Text = "Mark AFT" Or ASPxButton13.Text = "Un-mark AFT" Then
                ...
                Else
                    ...
                    Try
                        ...
                        a.auditlog(Session("BrokerCode"), Session("Username"), "Submitted AFT request", txtewraccountno.Text, cmbparaCompany.Text)
                    Catch ex As Exception
            ...
            Else
                ...
                Try
                    Dim a As New passmanagement
                    a.auditlog(Session("BrokerCode"), Session("Username"), "Re-Submitted AFT request", txtewraccountno.Text, cmbparaCompany.Text)
                Catch ex As Exception
    ...
    Public Sub getpending()
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand("select w.id,case when w.OTPStatus IS NULL and w.ApprovedBy is NULL THEN 'Enter OTP' else 'N/A' end as [otp] , W.EWR_Holder as [Account No],wr.UnitMeasure, W.Company as [Details], W.ReceiptID AS [EWRNo], case when W.approvedby is NULL and w.rejected is NULL then 'Pending' when w.rejected='1' then 'Rejected' else 'Approved' end as  [Status],  W.[Date], FORMAT(W.AFT_Quantity,'#,###.##','en-us') as AFT_Quantity ,wr.Commodity, wr.Grade, wr.WarehousePhysical, wr.Date_Issued, wr.[Status] as wrstatus    from AFT W, WR wr where wr.ReceiptNo=w.ReceiptID and ParticipantCode='" + Session("BrokerCode") + "' and deleted is NULL", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub getrejected()
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand("select w.id, W.EWR_Holder as [Account No],wr.UnitMeasure, W.Company as [Details], W.ReceiptID AS [EWRNo], case when W.approvedby is NULL and w.rejected is NULL then 'Pending'  when w.rejected='1'  THEN 'Rejected' else 'Approved' end as  [Status],  W.[Date], FORMAT(W.AFT_Quantity,'#,###.##','en-us') as AFT_Quantity ,wr.Commodity, wr.Grade, wr.WarehousePhysical, wr.Date_Issued, wr.[Status] as wrstatus    from AFT W, WR wr where wr.ReceiptNo=w.ReceiptID and ParticipantCode='" + Session("BrokerCode") + "' and w.deleted is Null and w.rejected is NOT NULL and deleted is NULL", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub loadcomboforreceipts(holder As String)
        Dim dsport As New DataSet
        cmd = New SqlCommand("select * from WR where holder='" + holder + "' and Quantity='10' and [Status]='ISSUED' and quantity>0 and status='ISSUED' and warehouse='" + Session("BrokerCode") + "' and receiptno not in (select receiptid from pendingtrans)", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub loadcomboforreceipts_AFT(holder As String)
        Dim dsport As New DataSet
        cmd = New SqlCommand("select * from WR where holder='" + holder + "' and Quantity='10' and [Status]='AFT' and isnull(sentPmex,0)='0' and quantity>0 and status='AFT' and warehouse='" + Session("BrokerCode") + "' and receiptno not in (select receiptid from pendingtrans)", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
            cmbparaCompany.DataSource = dsport
