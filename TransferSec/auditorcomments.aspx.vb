Partial Class TransferSec_auditorcomments
    ...
    Public Sub getpendingauth()
        Try
            ...
            cmd = New SqlCommand("SELECT [id],(select Company from [SystemUsers] where id=convert(int,audit_comments.auditor)) as [Auditor Company] ,(select Surname+' '+forenames as [Names] from [SystemUsers] where id=convert(int, audit_comments.auditor)) as [Auditor Name],[report] as [Reports Group],[Comment],[date_posted] as [Date Posted] FROM [audit_comments] where date_posted between '" + txtDateFrom.Text + "' and '" + txtDateFrom0.Text + "' order by id desc", cn)
