...
Partial Class TransferSec_ApproveBondTrade
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("select *, a.surname+' '+a.forenames as [ClientName] from Bond_Trades c, Accounts_clients a   where c.ApprovedBy is NULL and c.rejected is NULL and c.clientNo=a.cds_number", cn)
    ...
    Public Sub getauthorized()
        Dim dsport As New DataSet
        cmd = New SqlCommand("select *, a.surname+' '+a.forenames as [ClientName] from Bond_Trades c, Accounts_clients a where c.ApprovedBy is NOT NULL and c.rejected is NULL and c.clientNo=a.cds_number", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub GETREJECTED()
        Dim dsport As New DataSet
        cmd = New SqlCommand("select *, a.surname+' '+a.forenames as [ClientName] from Bond_Trades c, Accounts_clients a   where c.ApprovedBy is NULL and c.rejected is NOT NULL and c.clientNo=a.cds_number", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub getdetails(id As String)
        Dim dsport As New DataSet
        cmd = New SqlCommand("select * from Bond_Trades c, accounts_clients a where c.id='" + id + "' and c.clientNo=a.cds_number", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub approvetrans(id As String)
        Try
            ...
            cmd = New SqlCommand("update Bond_Trades set ApprovedBy='" + Session("Username") + "', ApprovedDate=getdate(), TradeStatus='SETTLED' where id='" + id + "'", cn)
            ...
            cmd = New SqlCommand("insert into trans (Company, CDS_Number, Date_Created, Trans_Time, Shares  , Update_Type, Created_By, Batch_Ref, Source, Pledge_shares, Reference, Instrument, [Broker], Custodian, AssetManager) select Company, ClientNo , SettlementDate, getdate(), CASE TradeType when 'RVP'  then Units when 'RF' then Units when 'DVP' then Units*-1  when 'DF' then Units*-1 else 0  end as [Shares], 'BOND TRADE', '" + Session("Username") + "', id, 'D', '0', TradeRef , 'BOND', [Broker], 'CABS', AssetManager from Bond_Trades where id='" + id + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
            ...
            cmd = New SqlCommand("insert into cashtrans ([Description], TransType , Amount, DateCreated , TransStatus , CDS_Number, Reference,AssetManager, BankName, BankAccount, Ref2, PostedBy, Currency)  select TradeType+ ' - '+ convert(nvarchar(50),Units)+ ' '+ Company +' @ '+ convert(nvarchar(50),price) , TradeType  +' Bond Trade' , case TradeType when 'RVP' then SettlementAmount*-1 when 'DVP' then SettlementAmount end as [Amt], SettlementDate , '1' , c.ClientNo , c.id,c.AssetManager, BankName, BankAccount, TradeRef , '" + Session("Username") + "', SettlementCurrency from Bond_Trades  c, Client_AssetManagers ca where ca.clientno=c.ClientNo and c.AssetManager=ca.AssetManager AND c.SettlementCurrency=ca.Currency  and c.id='" + id + "' AND (TradeType='DVP' or TradeType='RVP')", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
    ...
    Public Sub rejections(id As String)
        cmd = New SqlCommand("update Bond_Trades set Rejected='1', TradeStatus='EDIT', RejectionReason='REJECTED BY ADMIN' where id='" + id + "'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
    ...
    Public Sub rejections_CDS(id As String)
        cmd = New SqlCommand("update Bond_Trades set Rejected='1', TradeStatus='FAILED', RejectionReason='REJECTED BY ADMIN' where id='" + id + "'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
