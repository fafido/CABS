...
Partial Class TransferSec_Enq_VaultAv
    ...
    Public Sub loaddata(mro As String)
        If cmbvalue.SelectedItem.Value.ToString = "ALL" Then
            Dim ds1 As New DataSet
            cmd = New SqlCommand("  select " + qry() + "  from vw_Vault  where  convert(date, " + cmbdatetype.SelectedItem.Text + ")<=convert(date,'" + dtto.Date + "') AND [Certificate/Ref] not in (select certificate_No  from Deposit_Cert where DepType<>'DEPOSIT')", cn)


            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds1, "Accounts_Clients1")

            ASPxGridView1.Caption = "Vault Transactions as at  " + cmbdatetype.SelectedItem.Text + " " + dtto.Date.ToString("dd MMM yyyy") + ""
            If (ds1.Tables("Accounts_Clients1").Rows.Count > 0) Then
        ...
        Else
            ...
            Dim ds1 As New DataSet
            cmd = New SqlCommand("  select " + qry() + "  from vw_Vault  where [" + mro + "]='" + cmbvalue.SelectedItem.Text.ToString + "' and convert(date, " + cmbdatetype.SelectedItem.Text + ")<=convert(date,'" + dtto.Date + "')  AND [Certificate/Ref] not in (select certificate_No  from Deposit_Cert where DepType<>'DEPOSIT')", cn)
