Partial Class TransferSec_BatchReceipting_Allocation
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Session("finish") = "True" Then
                Session("finish") = ""
                msgbox("Series Allocation successfully captured")
            End If
    ...
    Protected Sub ASPxButton10_Click(sender As Object, e As EventArgs) Handles ASPxButton10.Click
        ...
        End If
    End Sub
    Public Function certexist(certificate As String) As Boolean
        Dim ds As New DataSet
        cmd = New SqlCommand("select * from bond_allocation where certificate='" + certificate + "' and [status]<>'ARCHIVED'", cn)
        cmd.Parameters.AddWithValue("@cdsnum", txtclientnumber.Text)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Public Function totalallocated(company As String) As Decimal
        Dim ds As New DataSet
        cmd = New SqlCommand("select ISNULL(sum(quantity),0) as tot from Bond_Allocation where company='" + company + "' and [status]<>'ARCHIVED'", cn)
        cmd.Parameters.AddWithValue("@cdsnum", txtclientnumber.Text)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Public Function issuedtotal(company As String) As Decimal
        Dim ds As New DataSet
        cmd = New SqlCommand(" select Facevalue  from Bond where code='" + company + "' ", cn)
        cmd.Parameters.AddWithValue("@cdsnum", txtclientnumber.Text)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
            Return ds.Tables(0).Rows(0).Item("Facevalue").ToString
        Else
            Return 0
        End If
    End Function
    ...
    Protected Sub ASPxButton12_Click(sender As Object, e As EventArgs) Handles ASPxButton12.Click
        If ASPxButton12.Text = "Allocate" Then
            Try
                ...
                If certexist(txtcertificate.Text) = True Then
                    msgbox("Certificate already captured!")
                    Exit Sub
                End If

                If totalallocated(cmbparaCompany1.SelectedItem.Text) + redeem > issuedtotal(cmbparaCompany1.SelectedItem.Text) Then
                    msgbox("You cannot allocate more than the Facevalue!")
                    Exit Sub
                End If

                cmd = New SqlCommand("insert into bond_Allocation (quantity, cds_number, [date], [status],company,  capturedBy, CaptureDate, ApprovedBy, ApprovedDate, Rejected, RejectionReason, Certificate) values (@quantity, @cds_number, getdate(), @status,@company, @capturedBy, getdate(),NULL, NULL, NULL,NULL,@certificate)", cn)
        ...
        Else
            Try
                ...
                cmd = New SqlCommand("update bond_Allocation set quantity='" + txtredeemunits.Text + "', cds_number='" + ListBox1.SelectedValue.ToString + "', [date]=getdate(), [status]='PENDING',company='" + cmbparaCompany1.SelectedItem.Value.ToString + "',  capturedBy='" + Session("Username") + "', CaptureDate=getdate(), ApprovedBy=NULL, ApprovedDate=NULL, Rejected=NULL, RejectionReason=NULL, Certificate='" + txtcertificate.Text + "' WHERE id='" + lblid.Text + "'", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
    ...
    Public Sub loadpending()
        Dim ds As New DataSet
        cmd = New SqlCommand("  select a.surname+' '+a.forenames as Names, * from Bond_Allocation b, Accounts_clients a where a.CDS_Number=b.cds_number and b.[status]<>'ARCHIVED'", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub grdTranShareholder_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdTranShareholder.SelectedIndexChanged
        Dim ds As New DataSet
        cmd = New SqlCommand("select b.quantity,b.certificate, b.id, b.cds_number, a.surname,a.forenames, b.company from bond_allocation b, accounts_clients a where b.cds_number=a.cds_number and b.id='" + grdTranShareholder.SelectedValue.ToString + "' and (b.[Status]='PENDING' OR b.[status]='REJECTED')", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "sec_type")
        If (ds.Tables(0).Rows.Count > 0) Then
            ...
            txtcertificate.Text = ds.Tables(0).Rows(0).Item("Certificate").ToString
    ...
    Protected Sub ASPxButton15_Click(sender As Object, e As EventArgs) Handles ASPxButton15.Click
        ...
        End If
        Try
            cmd = New SqlCommand("update Bond_Allocation set [Status]='ARCHIVED' where id='" + lblid.Text + "' ", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
