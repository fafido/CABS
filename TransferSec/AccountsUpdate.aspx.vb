...
Partial Class AccountsCreations
            ...
            If (Not IsPostBack) Then
                ...
                If Session("finish") = "yes" Then
                    ...
                    If xyz.OTPenabled = True Then
                        ...
                        msgbox("Update has been sent for approval!")
                        Session("AccountNo") = ""
                        Session("updatemsg") = ""
                    Else
    ...
    Public Sub SaveNewAccount()
        Try
            ...
            Try
                ...
                ESS.auditlog(Session("BrokerCode"), Session("Username"), "Updated an account", TXTClientID.Text, lblRecordID.Text)
            Catch ex As Exception
            End Try
            Session("numb") = ""
            Session("finish") = "yes"
    ...
    Sub loadPendingOTP()
        Dim sql_str As String = ""
        sql_str = "SELECT *,CASE WHEN A.AccountType='I' THEN 'Single' when A.AccountType='J' THEN 'Joint' when A.AccountType='C' THEN 'Corporate' when A.AccountType='P' THEN 'Pension Fund' else A.AccountType end as myAccType FROM Accounts_Audit A WHERE A.AuthorizationState='O' and ISNULL(A.OTP,'0')<>'0' and ISNULL(A.OTPStatus,'')<>'APPROVED' order by ID"
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("conpath").ConnectionString)
            Using cmd As New SqlCommand(sql_str, con)
                Dim dss As New DataSet
                Dim adp = New SqlDataAdapter(cmd)
    ...
    Sub getAssetmanagersClients()
        ...
        strSQL = "SELECT *,CASE WHEN ISNULL(MarkedForDeletion,0)=1 THEN 'Marked for Deletion' WHEN ISNULL(MarkedForDeletion,0)=3 then 'New' else '' end as UpdateStatus, CASE when MarkedForStatUpdate is NULL THEN '' WHEN MarkedForStatUpdate=0 THEN 'De-Activate' WHEN MarkedForStatUpdate=1 THEN 'Activate' END AS UpdateStatus1,CASE WHEN [Status]=0 then 'INACTIVE' WHEN [Status]=1 then 'ACTIVE' END AS Status1 FROM Client_AssetManagers WHERE ClientNo='" & TXTClientID.Text & "'"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Protected Sub grdAsertManagersClients_RowCommand(sender As Object, e As DevExpress.Web.ASPxGridView.ASPxGridViewRowCommandEventArgs) Handles grdAsertManagersClients.RowCommand
        Try
            Dim myID As String = e.CommandArgs.CommandArgument.ToString
            If e.CommandArgs.CommandName = "Delete" Then
                If accountHasTransactions(myID) = True Then
                    msgbox("Cannot delete account as it has transactions")
                    Exit Sub
                End If
                Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
                    Using cmd As New SqlCommand("Update Client_AssetManagers set MarkedForDeletion=CASE WHEN ISNULL(MarkedForDeletion,0)=1 THEN 0 ELSE 1 END WHERE ID='" & myID & "' AND ISNULL(MarkedForDeletion,0) NOT IN (3,4) DELETE FROM Client_AssetManagers WHERE ID='" & myID & "' AND ISNULL(MarkedForDeletion,0) IN (3,4)", cn)
                        ...
                        Try
                            Dim ESS As New passmanagement
                            ESS.auditlog(Session("BrokerCode"), Session("Username"), "Marked Bank Account for deletion", TXTClientID.Text, myID)
                        Catch ex As Exception
                        End Try
                        getAssetmanagersClients()
                        msgbox("Record successfully Marked/UnMarked for deletion, pending authorization")
                    End Using
                End Using
            ElseIf e.CommandArgs.CommandName = "Activate" Then
                If AccountAlreadyMarkedForDeletion(myID) = True Then
                    msgbox("Account already marked for deletion")
                    Exit Sub
                End If
                Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
                    Using cmd As New SqlCommand("Update Client_AssetManagers set MarkedForStatUpdate=CASE WHEN MarkedForStatUpdate IS NULL THEN 1 WHEN MarkedForStatUpdate=1 THEN 0 WHEN MarkedForStatUpdate=0 THEN NULL END WHERE ID='" & myID & "' AND ISNULL(MarkedForStatUpdate,0) NOT IN (3,4)", cn)
                        If cn.State = ConnectionState.Open Then cn.Close()
                        cn.Open()
                        cmd.ExecuteNonQuery()
                        cn.Close()
                        Try
                            Dim ESS As New passmanagement
                            ESS.auditlog(Session("BrokerCode"), Session("Username"), "Marked Bank Account for Status Update", TXTClientID.Text, myID)
                        Catch ex As Exception
                        End Try
                        getAssetmanagersClients()
                        msgbox("Record successfully marked/unmarked as Active, pending authorization")
                    End Using
                End Using
            End If
        Catch ex As Exception
    ...
    Sub showHideMandatoryFieldsCORP(ByVal showHide As String)
        ...
        End If
    End Sub
    Sub updateAssetManagersBack()
        Try
            Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
                Dim strDocs As String = " UPDATE Accounts_Documents SET MarkedForDeletion=0 where isnull(MarkedForDeletion,0) in (1,2) and doc_generated=@CDS_Number "
                strDocs = strDocs & "  UPDATE Client_AssetManagers SET MarkedForDeletion=0 where isnull(MarkedForDeletion,0) in (1,2) and ClientNo=@CDS_Number "
                strDocs = strDocs & "  UPDATE Accounts_Joint SET MarkedForDeletion=0 where isnull(MarkedForDeletion,0) in (1,2) and CDS_Number=@CDS_Number "
                strDocs = strDocs & " DELETE FROM Accounts_Documents WHERE isnull(MarkedForDeletion,0) in (3,4) and doc_generated=@CDS_Number "
                strDocs = strDocs & " DELETE FROM Client_AssetManagers where isnull(MarkedForDeletion,0) in (3,4) and ClientNo=@CDS_Number "
                strDocs = strDocs & " DELETE FROM Accounts_Joint where isnull(MarkedForDeletion,0) in (3,4) and CDS_Number=@CDS_Number "
                strDocs = strDocs & "  UPDATE Client_AssetManagers SET MarkedForStatUpdate=NULL where MarkedForDeletion in (0,1) and ClientNo=@CDS_Number "
                Using cmd As New SqlCommand(strDocs, cn)
                    cmd.Parameters.AddWithValue("@CDS_Number", TXTClientID.Text)
                    If cn.State = ConnectionState.Open Then cn.Close()
                    cn.Open()
                    cmd.ExecuteNonQuery()
    ...
    Protected Sub btnPrintCORP_Click(sender As Object, e As EventArgs) Handles btnPrintCORP.Click
        ...
        End Try
    End Sub
    Function getAssetmanagersClientsDset() As DataSet
        Dim strSQL As String = ""
        strSQL = "SELECT *,CASE WHEN ISNULL(MarkedForDeletion,0)=1 THEN 'Marked for Deletion' WHEN ISNULL(MarkedForDeletion,0)=3 then 'New' else '' end as UpdateStatus, CASE when MarkedForStatUpdate is NULL THEN '' WHEN MarkedForStatUpdate=0 THEN 'De-Activate' WHEN MarkedForStatUpdate=1 THEN 'Activate' END AS UpdateStatus1,CASE WHEN [Status]=0 then 'INACTIVE' WHEN [Status]=1 then 'ACTIVE' END AS Status1 FROM Client_AssetManagers WHERE ClientNo='" & TXTClientID.Text & "'"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
        ...
        End Using
    End Function
    Protected Sub grdAsertManagersClients_DataBinding(sender As Object, e As EventArgs) Handles grdAsertManagersClients.DataBinding
        grdAsertManagersClients.DataSource = getAssetmanagersClientsDset()
    End Sub
    Protected Sub grdAsertManagersClientsCORP_DataBinding(sender As Object, e As EventArgs) Handles grdAsertManagersClientsCORP.DataBinding
        grdAsertManagersClientsCORP.DataSource = getAssetmanagersClientsDset()
    End Sub
    Function accountHasTransactions(ByVal RefID As String) As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT A.* FROM CashTrans A JOIN Client_AssetManagers B ON A.CDS_Number=B.ClientNo AND A.AssetManager=B.AssetManager AND A.BankAccount=B.BankAccount AND A.Currency=B.Currency WHERE B.ID='" & RefID & "'"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            Dim adp = New SqlDataAdapter(cmd)
        ...
        End Using
    End Function
    Function AccountAlreadyMarkedForDeletion(ByVal RefID As String) As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT B.* FROM Client_AssetManagers B WHERE B.ID='" & RefID & "' AND ISNULL(MarkedForDeletion,0)=1 "
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            Dim adp = New SqlDataAdapter(cmd)
