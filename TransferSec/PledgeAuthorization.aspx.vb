Partial Class TransferSec_PledgeAuthorization
    ...
    Public Sub getlst()
        Try
            ...
            cmd = New SqlCommand("select CONVERT(varchar(10), id)+' '+ company+' '+ securitytype+' '+ CONVERT(varchar(10), Quantity ) as pleadgeinf, id from pledges_recording where Pledge_Status='A1'", cn)
            adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub ASPxButton10_Click(sender As Object, e As EventArgs) Handles ASPxButton10.Click
        Try
            ...
            cmd = New SqlCommand("update pledges_recording set Pledge_Status='A' WHERE CONVERT(varchar(10), id)+' '+ company+' '+ securitytype+' '+ CONVERT(varchar(10), Quantity ) = '" & lstpledgeSearch.SelectedItem.Text & "'", cn)
            ...
            '    cmd = New SqlCommand("update pledges_recording set Pledge_Status='X' WHERE convert (varchar,id)+' '+cds_number+' '+surname+' '+forenames = '" & cmbPendingPledges.SelectedItem.Text & "'", cn)
            '    If (cn.State = ConnectionState.Open) Then
            '        cn.Close()
            '    End If
            '    cn.Open()
