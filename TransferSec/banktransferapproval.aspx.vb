...
Partial Class Parameters_banktransferapproval
    ...
    Public Sub approve(id As String)
        cmd = New SqlCommand("  declare @id nvarchar(50)='" + id + "' insert into Bank_Transfers ([DebitAccountNumber]      ,[CreditAccountNumber]      ,[Currency]      ,[Amount]      ,[TransactionDate]      ,[Response]      ,[Processed]      ,[DrCDSNumber]      ,[CrCDSNumber]      ,[BankFrom]      ,[BankTo]      ,[Description]) select [DebitAccountNumber]      ,[CreditAccountNumber]      ,[Currency]      ,[Amount]      ,[TransactionDate]      ,[Response]      ,[Processed]      ,[DrCDSNumber]      ,[CrCDSNumber]      ,[BankFrom]      ,[BankTo]      ,[Description]	  from Bank_Transfers_aUDIT WHERE id=@id	  update Bank_Transfers_Audit set Response='POSTED' where id=@id", cn)
    ...
    Public Sub decline(id As String)
        cmd = New SqlCommand("declare @id nvarchar(50)='" + id + "' update Bank_Transfers_Audit set Response='POSTED' where id=@id", cn)
        If cn.State = ConnectionState.Open Then
            cn.Close()
        End If
        cn.Open()
    ...
    Protected Sub getSecurities_Categories()
        Try
            cmd = New SqlCommand("select * from Bank_Transfers_Audit where response is NULL", cn)
