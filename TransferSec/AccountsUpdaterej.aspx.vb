...
Partial Class AccountsCreations
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If (Not IsPostBack) Then
                ...
                Catch ex As Exception
                End Try
                ASPxButton11_Click(DBNull.Value, New EventArgs)
    ...
    Protected Sub ASPxButton11_Click(sender As Object, e As EventArgs) Handles ASPxButton11.Click
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select a.ID,a.cds_number+' '+case when a.AccountType IN ('J','C') THEN isnull(a.Surname,'') else isnull(a.forenames,'')+' '+isnull(a.surname,'') end as namess from accounts_Audit a where isnull(a.surname,'')+' '+isnull(a.middlename,'')+' '+isnull(a.forenames,'') +' '+cds_number+' '+ISNULL(a.CNIC,'')+' '+ISNULL(a.IDNoPP,'') like '%" & txtnamesearch.Text & "%' and a.AuthorizationState='X' order by cds_number", cn)
            adp = New SqlDataAdapter(cmd)
    ...
    Public Sub getAccountDetails()
        ClearAllFields()
        Dim ds As New DataSet
        cmd = New SqlCommand("select case when ISNULL(BillCashAccount,0)=0 then 0 else 1 end AS BillCashAccount1,FORMAT(AgreementDate,'dd MMMM yyyy') AS AgreementDate1,FORMAT(CNIC_expiry_date,'dd MMMM yyyy','en-us') as CNICExpiry1,FORMAT(Password_expiry_date,'dd MMMM yyyy','en-us') as PassportExpiry1,FORMAT(DOB,'dd MMMM yyyy') AS DOB1,FORMAT(BusCommenceDate,'dd MMMM yyyy') AS BusCommenceDate1,*,CASE WHEN AccountType='I' THEN 'Single' when AccountType='J' THEN 'Joint' when AccountType='C' THEN 'Corporate' else AccountType end as myAccType from Accounts_Audit where ID='" & lstNameSearch.SelectedItem.Value & "'", cn)
        ...
        If (ds.Tables(0).Rows.Count > 0) Then
            ...
            txtReasons.Text = ds.Tables(0).Rows(0).Item("Comments").ToString
    ...
    Sub singleTab(ByVal actionNeeded As String)
        If actionNeeded = "Show" Then
            ...
            Panel3bbbb_by.Visible = True
            Panel3bbbb_by2.Visible = True
        Else
            ...
            Panel3bbbb_by.Visible = False
            Panel3bbbb_by2.Visible = False
        End If
    End Sub
    Sub CoorporateTab(ByVal actionNeeded As String)
        If actionNeeded = "Show" Then
            ...
            CORP8c.Visible = True
            CORP8D.Visible = True
            CORP8D1.Visible = True
            CORP8D2.Visible = True
            CORP8E.Visible = True
            CORP9.Visible = True
            CORP10.Visible = True
            CORP11.Visible = True
        ...
        Else
            ...
            CORP8D1.Visible = False
            CORP8D2.Visible = False
    ...
    Public Sub SaveNewAccount()
        Try
            ...
            cmd.Parameters.AddWithValue("@Update_Type", UPDATETYPE)
    ...
    Function AccountHasPendingUpdate(ByVal cdsAccount As String) As Boolean
        ...
        sql_str = "select B.* from Accounts_Audit B WHERE (B.AuthorizationState='A' or B.AuthorizationState='O') and B.CDS_Number=@CDS_Number"
        ...
        End Using
    End Function
    Function GetUpdateType() As String
        Dim sql_str As String = ""
        sql_str = "SELECT CASE WHEN EXISTS(SELECT TOP 1 * FROM Accounts_Clients B WHERE B.CDS_Number=@CDS_Number) THEN 'UPDATE' ELSE 'NEW' END AS UpdateType"
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("conpath").ConnectionString)
            Using cmd As New SqlCommand(sql_str, con)
                cmd.Parameters.AddWithValue("@CDS_Number", TXTClientID.Text)
                Dim dss As New DataSet
                Dim adp = New SqlDataAdapter(cmd)
                adp.Fill(dss, "Accounts_Clients")
                If dss.Tables(0).Rows.Count > 0 Then
                    Return dss.Tables(0).Rows(0).Item("UpdateType").ToString
                Else
                    Return "UPDATE"
                End If
            End Using
