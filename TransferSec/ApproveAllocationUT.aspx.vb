...
Partial Class TransferSec_ApproveAllocationUT
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("  select *,'Fund Allocation' as [Type], a.Surname+' '+a.forenames as [Names] from UT_Allocation b, Accounts_clients a where b.[status]='PENDING' and a.cds_number=b.cds_number", cn)
    ...
    Public Sub rejecttrans(transid As String)
        Try
            cmd = New SqlCommand("update UT_Allocation set [status]='REJECTED' where id='" + transid + "'", cn)
    ...
    Public Sub approvetrans(transid As String)
        Try
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number, date_created, trans_time, shares, Update_Type, Created_By, Batch_Ref, source, Pledge_shares, reference) select company, cds_number, getdate(), getdate(), quantity, 'Fund Allocation', '" + Session("Username") + "', '" + transid + "', 'D', 0, '0' from UT_Allocation where id='" + transid + "' update UT_Allocation set [status]='Approved' where id='" + transid + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
    ...
    Private Sub grddocuments_DataBinding(sender As Object, e As EventArgs) Handles grddocuments.DataBinding
        grddocuments.DataSource = pendingdata()
