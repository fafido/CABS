Partial Class TransferSec_UT_Withdrawal
    ...
    Public Sub loadpending()
        Dim ds As New DataSet
        cmd = New SqlCommand("  select a.surname+' '+a.forenames as Names, * from UT_Redemption b, Accounts_clients a where a.CDS_Number=b.cds_number and b.[status]<>'ARCHIVED'", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub ASPxButton12_Click(sender As Object, e As EventArgs) Handles ASPxButton12.Click
        ...
        If ASPxButton12.Text = "Redeem" Then
            ...
            Try
                ...
                cmd = New SqlCommand("insert into UT_Redemption (quantity, cds_number, [date], [status],company,  capturedBy, CaptureDate, ApprovedBy, ApprovedDate, Rejected, RejectionReason, Bank, BankAccount, Amount, Price) values (@quantity, @cds_number, getdate(), @status,@company, @capturedBy, getdate(),NULL, NULL, NULL,NULL,@Bank,@BankAccount, @Amount, @Price)", cn)
                ...
                cmd.Parameters.AddWithValue("@Amount", txtwithdrawamt.Text)
    ...
    Public Sub loadsecurities()
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                ...
                cmbparaCompany1.TextField = "company"
    ...
    Public Function pendingunits(cdsnumber As String, series As String) As Decimal
        Dim sel As String = ""
        Dim ds As New DataSet
        cmd = New SqlCommand("select isnull(sum(quantity),0) as amt from UT_Redemption where company='" + series + "' and cds_number='" + cdsnumber + "' and [status]='PENDING'", cn)
        adp = New SqlDataAdapter(cmd)
    ...
    Protected Sub grdTranShareholder_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdTranShareholder.SelectedIndexChanged
        Dim ds As New DataSet
        cmd = New SqlCommand("select b.quantity,b.bank, b.bankaccount,b.Price, b.Amount, b.id, b.cds_number, a.surname,a.forenames, b.company from UT_Redemption b, accounts_clients a where b.cds_number=a.cds_number and b.id='" + grdTranShareholder.SelectedValue.ToString + "' and (b.[Status]='PENDING' OR b.[status]='REJECTED')", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "sec_type")
        If (ds.Tables(0).Rows.Count > 0) Then
            ...
            txtwithdrawamt.Text = ds.Tables(0).Rows(0).Item("Amount").ToString
    ...
    Public Sub recalc()
        Try
            ...
            If txtredeemunits.Text <> "" Then
                Dim redeem As Decimal = txtredeemunits.Text
                Dim val As Decimal = redeem * curr
                txtwithdrawamt.Text = val.ToString
            End If
    ...
    Protected Sub ASPxButton15_Click(sender As Object, e As EventArgs) Handles ASPxButton15.Click
        Response.Redirect(Request.RawUrl)
    End Sub
    Protected Sub txtredeemunits_TextChanged(sender As Object, e As EventArgs) Handles txtredeemunits.TextChanged
        recalc()
