Partial Class TransferSec_BatchReceipting_allot
    ...
    Protected Sub ASPxButton10_Click(sender As Object, e As EventArgs) Handles ASPxButton10.Click
        Dim ds As New DataSet
        cmd = New SqlCommand("select cds_number+' '+surname+' '+middlename+' '+forenames as names, cds_number from accounts_clients where cds_number+' '+surname+' '+forenames+' '+tel like '%' + @cdsnum + '%' ", cn)
    ...
    Protected Sub ASPxButton12_Click(sender As Object, e As EventArgs) Handles ASPxButton12.Click
        If ASPxButton12.Text = "Update" Then
            cmd = New SqlCommand("delete from batch_certs where cds_number='' update batch_certs set cds_number='" + ListBox1.SelectedValue.ToString + "', shares='" + txtshares.Text + "' where id='" + lblid.Text + "'", cn)
            cmd.Parameters.AddWithValue("@clonn", txtClientNo.Text)
            ...
            ASPxButton12.Text = "Add"
        Else
            cmd = New SqlCommand("delete from batch_certs where cds_number='' insert into batch_certs values (@clonn,'" + ListBox1.SelectedValue.ToString + "',@shares,'0')", cn)
            cmd.Parameters.AddWithValue("@clonn", txtClientNo.Text)
            cmd.Parameters.AddWithValue("@shares", txtshares.Text)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand("select b.id,a.surname+' '+a.middlename+' '+a.forenames as name, b.cds_number, b.batch_no, b.certificate_no, b.shares  from accounts_clients a, batch_certs b where b.cds_number=a.CDS_Number and batch_no=@clon2", cn)
        cmd.Parameters.AddWithValue("@clon2", txtClientNo.Text)
        adp = New SqlDataAdapter(cmd)
    ...
    Public Sub loadsecurities()
        Try
            ...
            adp.Fill(ds, "para_broker")
            If (ds.Tables(0).Rows.Count > 0) Then
                cmbparaCompany0.DataSource = ds
                cmbparaCompany0.TextField = "security_name"
    ...
    Protected Sub grdTranShareholder_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdTranShareholder.SelectedIndexChanged
        Dim ds As New DataSet
        cmd = New SqlCommand("select id, cds_number, shares from batch_certs where id='" + grdTranShareholder.SelectedValue.ToString + "'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "sec_type")
        If (ds.Tables(0).Rows.Count > 0) Then
            txtclientnumber.Text = ds.Tables(0).Rows(0).Item("cds_number")
            txtshares.Text = ds.Tables(0).Rows(0).Item("shares")
            ListBox1.DataSource = ds
            ListBox1.DataTextField = "cds_number"
            ListBox1.DataValueField = "cds_number"
            ListBox1.DataBind()
            ListBox1.SelectedIndex = 0
            ASPxButton12.Text = "Update"
            lblid.Text = ds.Tables(0).Rows(0).Item("id")
