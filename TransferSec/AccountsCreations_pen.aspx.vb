...
Partial Class TransferSec_AccountsCreations_pen
    Inherits System.Web.UI.Page 'll
    ...
    Public Sub SaveNewAccount()
        Try
            ...
            cmd.Parameters.AddWithValue("@AccountType", "P")
    ...
    Public Sub loadallWIP()
        ...
        Dim ds As New DataSet
        'cmd = New SqlCommand("  Select ID,Forenames,Surname,IDNoPP,Add_1,JointName,IDNOPP  from Accounts_Audit_temp where AccountType in ('P') AND Created_By=@Created_By and IDNoPP NOT IN (SELECT DISTINCT r.IDNoPP FROM Accounts_Audit r UNION SELECT DISTINCT p.IDNoPP FROM Accounts_Clients p) order by id desc ", cn)
        cmd = New SqlCommand("  SELECT E.* FROM ( Select ID,Forenames,Surname,IDNoPP,Add_1,JointName,'Work in Progress' as [Source]  from Accounts_Audit_temp where AccountType in ('P') AND Created_By=@Created_By and IDNoPP NOT IN (SELECT DISTINCT r.IDNoPP FROM Accounts_Audit r UNION SELECT DISTINCT p.IDNoPP FROM Accounts_Clients p) UNION Select ID,Forenames,Surname,IDNoPP,Add_1,JointName,'Work in Progress' as [Source]  from Accounts_Audit_temp where AccountType in ('P')  AND AuthorizationState='Account Created on Web Portal' and IDNoPP NOT IN (SELECT DISTINCT r.IDNoPP FROM Accounts_Audit r UNION SELECT DISTINCT p.IDNoPP FROM Accounts_Clients p) )E order by E.ID desc ", cn)
        adp = New SqlDataAdapter(cmd)
        cmd.Parameters.AddWithValue("@Created_By", Session("Username"))
    ...
    Public Sub saveWIPCOOP()
        ...
        strCMD = strCMD & " if not exists(select top 1 * from Accounts_Audit_temp where IDNoPP=@IDNoPP AND CreatedBy=@CreatedBy AND AccountType='P') begin "
    ...
    Protected Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Try
            ...
            Dim queryString = "AccountPrintReport.aspx?Username=" & Session("Username") & "&AccType=P&IDD=" & outputString & ""
