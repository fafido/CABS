...
Partial Class UploadApprovalBond
    ...
    Public Function pending(assetmanager As String, dat As String) As DataSet
        ...
        If assetmanager = "" And dat = "" Then
            cmd = New SqlCommand("select * from Recon_AssetManager_Approvals_Bond where [CurrentStatus]='Pending' AND CurrentStatus='Pending'", cn)
        Else
            cmd = New SqlCommand("select * from Recon_AssetManager_Approvals_Bond where [CurrentStatus]='Pending' and convert(date, recondate)='" + dat + "' and AssetManager='" + assetmanager + "' and CurrentStatus='Pending'", cn)
        End If
        adp = New SqlDataAdapter(cmd)
    ...
    Public Sub approvetransaction(transkey As String)
        Try
            ...
            cmd = New SqlCommand("UPDATE Recon_AssetManager_Approvals_Bond set [Currentstatus]='APPROVED', AuthorizedBy='" + Session("Username") + "', DateAuthorized= getdate() where   id='" + transkey + "' AND currentStatus='Pending'", cn)
    ...
    Public Sub DECLINE(transkey As String)
        Try
            ...
            cmd = New SqlCommand("UPDATE Recon_AssetManager_Approvals_Bond set [Currentstatus]='REJECTED', AuthorizedBy='" + Session("Username") + "', DateAuthorized= getdate()  where  id='" + transkey + "' AND currentStatus='Pending'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
