...
Partial Class TransferSec_TransferApproval
    ...
    Public Function isOTPsent(ref As String) As Boolean
        ...
        cmd = New SqlCommand("select * from share_transfer where id='" + ref + "' and  OTP is NOT NULL AND OTPStatus<>'Approved'", cn)
        ...
        Try
            ...
            Else
                If charge > 0 Then
                    ...
                    Exit Sub
                End If
                Dim oth As Decimal = txtothercharges.Text.Replace(",", "")
                If oth > 0 Then
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number, Date_Created, trans_time, shares, update_type, Created_By, Batch_Ref, [source], Pledge_shares, reference, warehouse, WarehouseOperator) select Commodity+'/'+grade, '" + txttransfereeAccountNo.Text + "', getdate(), getdate(), " + txtshares.Text + "*1, 'TRANSFERRED', '" + Session("Username") + "', t.id,'D','0','" + cmbparaCompany.Text + "',Warehouse, WarehousePhysical    from wr w, share_transfer t WHERE w.ReceiptNo='" + cmbparaCompany.Text + "' and t.receiptid=w.receiptno and t.id='" + txtid.Text + "'  insert into trans (company, cds_number, Date_Created, trans_time, shares, update_type, Created_By, Batch_Ref, [source], Pledge_shares, reference, warehouse, WarehouseOperator) select Commodity+'/'+grade, holder, getdate(), getdate(), " + txtshares.Text + "*-1, 'TRANSFERRED', '" + Session("Username") + "', t.id,'D','0','" + cmbparaCompany.Text + "',Warehouse, WarehousePhysical    from wr w, share_transfer t WHERE w.ReceiptNo='" + cmbparaCompany.Text + "' and w.receiptNo=t.receiptID AND  t.id='" + txtid.Text + "' update wr set holder='" + txttransfereeAccountNo.Text + "' where receiptno='" + cmbparaCompany.Text + "'  Update share_transfer set status='1', approvedby='" + Session("Username") + "',ApprovedDate=getdate() where id='" + txtid.Text + "'", cn)
            ...
            Try
                ...
                a.auditlog(Session("BrokerCode"), Session("Username"), "Approved a transfer Request", txtAccountNo.Text, cmbparaCompany.Text)
            Catch ex As Exception
            ...
            Try
                ...
                em.sendmail(getemail(txtewraccountno.Text), "EWR Transfer", Session("msg"))
            Catch ex As Exception
    ...
    Public Sub loadreceiptdetails()
        ...
        If xyz.OTPenabled = True Then
            cmd = New SqlCommand("select *,FORMAT(QUANTITY,'#,###.##','en-us') as Quant from WR where receiptno in (select receiptid from share_transfer  where status='0' and ParticipantCode='" + Session("Brokercode") + "' and rejected is NULL and  approvedby is NULL AND deleted IS NULL and OTPStatus='Approved')", cn)
        Else
            cmd = New SqlCommand("select *,FORMAT(QUANTITY,'#,###.##','en-us') as Quant from WR where receiptno in (select receiptid from share_transfer  where status='0' and ParticipantCode='" + Session("Brokercode") + "' and rejected is NULL AND deleted IS NULL and approvedby is NULL)", cn)
        End If
    ...
    Private Sub ASPxGridView5_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles ASPxGridView5.RowCommand
        ...
        If e.CommandArgs.CommandName.ToString = "Select" Then
            ...
            Try
                ...
                txtcharge.Text = transcharge.ToString("N")
        ...
        End If
    End Sub
    Public Function getname(cds_number As String) As String
    ...
    Public Sub getinfortoedit(receipt As String)
        cmd = New SqlCommand("select wr.holder, w.transferee ,wr.receiptno, w.amount_to_transfer, w.capturedBy, w.capturedate, w.id    from wr , share_transfer w  where receiptno='" + receipt + "' and wr.receiptno=w.receiptid and w.ApprovedBy is NULL and w.deleted is NULL AND rejectionreason is NULL", cn)
        Dim ds As New DataSet
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "pc")
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            txtavailableshares.Text = ds.Tables(0).Rows(0).Item("amount_to_transfer").ToString
            ...
            txttransfereeAccountNo.Text = ds.Tables(0).Rows(0).Item("transferee").ToString
        ...
        Else
            If txtrejectreason.Text <> "" Then
                Try
                    ...
                    Try
                        Dim m As New NaymatBilling
                        Dim charge As Double = m.transfercharges_REVERSAL("BILL", "DEPOSITOR", txtshares.Text, txtewraccountno.Text.ToString, cmbparaCompany.Text)

                    Catch ex As Exception
                        msgbox("Error Charging: " + ex.Message)
                    End Try

                    cmd = New SqlCommand(" update share_transfer set rejected='1', approveddate=getdate(), rejectionreason='" + txtrejectreason.Text + "' where id='" + txtid.Text + "'", cn)
                    If (cn.State = ConnectionState.Open) Then
                        cn.Close()
                    End If
                    cn.Open()
                    ...
                    Try
                        Dim a As New passmanagement
                        a.auditlog(Session("BrokerCode"), Session("Username"), "Rejected a transfer Request", txtAccountNo.Text, cmbparaCompany.Text)
                    Catch ex As Exception
                    ...
                    Session("msg") = "Transfer request for EWR No. " + cmbparaCompany.Text + " has been rejected successfully. "
                    Response.Redirect(Request.RawUrl)
                Catch ex As Exception
                    msgbox(ex.Message)
                End Try
    ...
    Private Sub ASPxGridView3_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles ASPxGridView3.RowCommand
        ...
        If e.CommandArgs.CommandName.ToString = "Resend" Then
            ...
            Else
                Dim OTP As String = CreateRandomPassword(4)
                cmd = New SqlCommand(" update share_transfer set  OTP='" + OTP + "',OTPSent='1' ,OTPCreateTime=getdate() where id='" + id.ToString + "'", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
