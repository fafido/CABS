Partial Class TransferSec_UT_AllocationIncome
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Session("finish") = "True" Then
                Session("finish") = ""
                msgbox("Income Allocation successfully captured")
            End If
    ...
    Protected Sub ASPxButton12_Click(sender As Object, e As EventArgs) Handles ASPxButton12.Click
        If ASPxButton12.Text = "Allocate" Then
            Try
                ...
                cmd = New SqlCommand(" insert into UT_Allocation_Income (Fund, IncomeDescription, Amount, Price, IssuedUnits, CutOffDate, Status, CreatedBy, DateCreated)  values (@Fund, @IncomeDescription, @Amount, @Price, @IssuedUnits, @CutOffDate, @Status, @CreatedBy, getdate())", cn)
                cmd.Parameters.AddWithValue("@Fund", cmbparaCompany1.SelectedItem.Value.ToString)
                cmd.Parameters.AddWithValue("@status", "PENDING")
                cmd.Parameters.AddWithValue("@IncomeDescription", txtdescription.Text)
                cmd.Parameters.AddWithValue("@Amount", txtamountreceived.Text)

                cmd.Parameters.AddWithValue("@Price", txtcurrentprice.Text)
                cmd.Parameters.AddWithValue("@IssuedUnits", txtshares.Text)
                cmd.Parameters.AddWithValue("@CutOffDate", txtcutoff.Date)
    ...
    Public Sub loadpending()
        Dim ds As New DataSet
        cmd = New SqlCommand("select * from UT_Allocation_Income", cn)
        adp = New SqlDataAdapter(cmd)
