Partial Class Enquiries_PledgeTransfer
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Session("end") = "Yes" Then
                Session("end") = ""
                msgbox("Pledge Transfer Sent For Approval")
            Else
    ...
    Protected Sub ASPxButton10_Click(sender As Object, e As EventArgs) Handles ASPxButton10.Click
        Try
            cmd = New SqlCommand("Update Pledges_recording set transferred='Yes' where id='" + lstpledgeSearch.Value.ToString + "'", cn)
            ...
            cmd = New SqlCommand("insert into pledges_transfer (pledge_id, trans_date,transferred_by, transfer_reason, transfer_approval) values ('" + lstpledgeSearch.Value.ToString + "', '" + Now() + "','" + Session("username") + "','" + txtPledgeReason.Text + "','No')", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
