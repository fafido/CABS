Partial Class TransferSec_settledrecords2
    ...
    Public Sub getpendingauth()
        Try
            ...
            cmd = New SqlCommand("select trans.company as Company, trans.CDS_Number, Accounts_Clients.Surname+' '+Accounts_Clients .Forenames as [Client],CONVERT(VARCHAR(11),tbl_SettlementSummary.trade_date,106) as [Date],tbl_SettlementSummary.vat as [VAT],tbl_SettlementSummary.Capital_Gain_Tax as [Capital Gain Tax],case left(tbl_settlementsummary.deal_reference,1) when 'S' THEN tbl_SettlementSummary.amount_due WHEN 'P'  THEN (tbl_SettlementSummary.amount_due*-1)  END  as [Amount($)],replace(CAST(CONVERT(varchar, CAST(sum(shares) AS money), 1) AS varchar),'.00','') as [Shares]   from trans, Accounts_Clients, tbl_SettlementSummary  where trans.Reference=tbl_SettlementSummary.deal_reference and trans.Created_by ='SETTLEMENT' and trans.CDS_Number = Accounts_Clients.CDS_Number and trans.Date_Created between '" + txtDateFrom.Text + "' and '" + txtDateFrom0.Text + "' group by Accounts_Clients.Surname , Accounts_Clients .Forenames , trans.CDS_Number, trans.company,trans.date_created,trans.trans_time,tbl_SettlementSummary.trade_date,tbl_SettlementSummary.vat,tbl_SettlementSummary.Capital_Gain_Tax,tbl_SettlementSummary.Amount_due, tbl_SettlementSummary.deal_reference   order by trans.date_created desc, trans.trans_time desc", cn)
