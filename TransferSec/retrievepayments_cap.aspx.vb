Partial Class TransferSec_retrievepayments_cap
    ...
    Public Sub getpendingauth()
        Try
            ...
            cmd = New SqlCommand("SELECT (SELECT MOBILE FROM CDS.DBO.ACCOUNTS_CLIENTS WHERE CDS_Number=convert(nvarchar(50),m.bondholder))  as mobile , ACTUAL_GROSS as amount, '0' as quantity FROM Kbond.dbo.capital_repayment  m where issuer='GVT'", cn)
    ...
    Public Sub Csv1()
        Try
            ...
            Else
                grdsellers.DataSource = Nothing
                grdsellers.DataBind()
                msgbox("No Pending Capital Redemption Records")
            End If
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" & Now.Date & "_capital_red_Payments_")
            Response.Charset = ""
