Partial Class TransferSec_interdepository_authorization2_outgoing
    ...
    Public Sub saveupdate()
        ...
        cmd = New SqlCommand("insert into trans (company, cds_number, date_created, trans_time, shares, update_type, created_by,batch_ref, source, pledge_shares) values ('" + dtlBankDetail.Rows(7).Cells(1).Text + "','" + dtlBankDetail.Rows(1).Cells(1).Text + "',getdate(),cast(getdate() as time),'" + dtlBankDetail.Rows(11).Cells(1).Text + "','Cross Border','" + Session("Username").ToString + "','0','S','0')", cn)
        ...
        cmd = New SqlCommand("update mast set shares=(select shares from mast where cds_number='" + dtlBankDetail.Rows(1).Cells(1).Text + "')+" + dtlBankDetail.Rows(11).Cells(1).Text + " where CDS_Number='" + dtlBankDetail.Rows(1).Cells(1).Text + "'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
