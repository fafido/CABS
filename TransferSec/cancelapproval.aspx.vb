...
Partial Class TransferSec_cancelApproval
    ...
    Public Sub msgbox(ByVal strMessage As String)
        Dim strScript As String = "<scrihttp://localhost:1730/TransferSec/cancelapproval.aspx.vbpt language=JavaScript>"
    ...
    Public Function isOTPsent(ref As String) As Boolean
        ...
        cmd = New SqlCommand("select * from ReceiptCancel where id='" + ref + "' and  OTP is NOT NULL AND OTPStatus<>'Approved'", cn)
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        Try
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number, Date_Created, trans_time, shares, update_type, Created_By, Batch_Ref, [source], Pledge_shares, reference, warehouse, WarehouseOperator) select Commodity+'/'+grade, holder, getdate(), getdate(), " + txtshares.Text + "*-1, 'CANCELLED', '" + Session("Username") + "', id,'D','0','" + cmbparaCompany.Text + "',Warehouse, WarehousePhysical    from wr WHERE ReceiptNo='" + cmbparaCompany.Text + "' UPDATE wr SET [Status]='CANCELLED' WHERE ReceiptNo='" + cmbparaCompany.Text + "' update ReceiptCancel set Approvedby='" + Session("Username") + "', status='CANCELLED', approveddate=getdate(), OTP='" + OTP + "',OTPSent='1' ,OTPCreateTime=getdate() where id='" + txtid.Text + "'       insert into cashtrans (Description, TransType, Amount, DateCreated ,TransStatus, CDS_Number, Paid, Reference, ChargeCode )  select 'EWR Cancellation Reversal', 'EWR Cancellation Reversal', sum(amount)*-1, getdate(), '1','" + txtewraccountno.Text + "', NULL, '" + cmbparaCompany.Text + "','REVERSAL' from CashTransComb where cds_Number='" + txtewraccountno.Text + "' and reference='" + cmbparaCompany.Text + "'", cn)
            ...
            If txtcanceltype.Text = "Cancel EWR Only" Then
                cmd = New SqlCommand("  update WarehourseDeliveries set quantity='" + txtavailableshares.Text + "' where id=(select top 1 DeliveryID  from wr where receiptno='" + cmbparaCompany.Text + "') ", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
            ...
            Session("finish") = "yes"
            Session("msg") = "EWR Cancellation request for EWR No. " + cmbparaCompany.Text + " has been successfully authorized. Transaction ID is " + txtid.Text + ""
            Try
                Dim em As New sendmail
                em.sendmail(getemail(txtewraccountno.Text), "EWR Cancellation", Session("msg"))
            Catch ex As Exception
            ...
            Try
                ...
                a.auditlog(Session("BrokerCode"), Session("Username"), "Approved an EWR Cancellation Request", txtAccountNo.Text, cmbparaCompany.Text)
            Catch ex As Exception
    ...
    Public Sub loadreceiptdetails()
        ...
        If xyz.OTPenabled = True Then
            cmd = New SqlCommand("select *,FORMAT(QUANTITY,'#,###.##','en-us') AS Quant from WR where receiptno in (select receiptid from ReceiptCancel  where status='0' and ParticipantCode='" + Session("Brokercode") + "' and rejected is NULL and approvedby is NULL and OTPStatus='Approved' and deleted is NULL)", cn)
        Else
            cmd = New SqlCommand("select *,FORMAT(QUANTITY,'#,###.##','en-us') AS Quant from WR where receiptno in (select receiptid from ReceiptCancel  where status='0' and ParticipantCode='" + Session("Brokercode") + "' and rejected is NULL and approvedby is NULL and deleted is NULL) ", cn)
        End If
    ...
    Public Sub getinfortoedit(receipt As String)
        cmd = New SqlCommand("select wr.holder, wr.receiptno, w.amount_to_withdraw, w.cancellationType ,w.capturedBy, w.capturedate, w.id    from wr , ReceiptCancel w  where receiptno='" + receipt + "' and wr.receiptno=w.receiptid", cn)
        Dim ds As New DataSet
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "pc")
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            txtcanceltype.Text = ds.Tables(0).Rows(0).Item("CancellationType").ToString
        ...
        Else
            If txtrejectreason.Text <> "" Then
                Try
                    ...
                    cmd = New SqlCommand(" update ReceiptCancel set rejected='1', approveddate=getdate(), rejectionreason='" + txtrejectreason.Text + "' where id='" + txtid.Text + "'", cn)
                    If (cn.State = ConnectionState.Open) Then
                        cn.Close()
                    End If
                    cn.Open()
                    ...
                    Try
                        Dim a As New passmanagement
                        a.auditlog(Session("BrokerCode"), Session("Username"), "Rejected an EWR Cancellation Request", txtAccountNo.Text, cmbparaCompany.Text)
                    Catch ex As Exception
                    ...
                    Session("msg") = "Receipt Cancellation request for EWR No. " + cmbparaCompany.Text + " has been rejected successfully. "
                    Response.Redirect(Request.RawUrl)
                Catch ex As Exception
                    msgbox(ex.Message)
                End Try
    ...
    Private Sub ASPxGridView3_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles ASPxGridView3.RowCommand
        ...
        If e.CommandArgs.CommandName.ToString = "Resend" Then
            ...
            Else
                Dim OTP As String = CreateRandomPassword(4)
                cmd = New SqlCommand(" update ReceiptCancel set  OTP='" + OTP + "',OTPSent='1' ,OTPCreateTime=getdate() where id='" + id.ToString + "'", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
    ...
    Protected Sub ASPxButton15_Click(sender As Object, e As EventArgs) Handles ASPxButton15.Click
        ...
        Try
            pd(id, "EWR Cancellation")
        Catch ex As Exception
        End Try
        Try
            word(id, "EWR Cancellation")
        Catch ex As Exception
        End Try
        Try
            xls(id, "EWR Cancellation")
        Catch ex As Exception
        End Try
        Try
            Img(id, "EWR Cancellation")
        Catch ex As Exception
        End Try
    End Sub
End Class
