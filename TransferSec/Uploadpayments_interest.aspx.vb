...
Partial Class TransferSec_uploadpayments_interest
    ...
    Public Sub getpendingauth()
        Try
            ...
            cmd = New SqlCommand("select min(id) as min, max(id) as max from Payment_Interest_Payment where Payment_Status='0'", cn)
            ...
            If (ds.Tables("ids").Rows.Count > 0) Then
                ...
                For value As Integer = ds.Tables("ids").Rows(0).Item("min") To ds.Tables("ids").Rows(0).Item("max")
                    ...
                    cmd = New SqlCommand("select * from Payment_Interest_Payment where Payment_Status='0' and id='" + value.ToString + "'", cn)
                    adp = New SqlDataAdapter(cmd)
                    adp.Fill(ds1, "mobile")
    ...
    Public Sub pay2()
        ...
        cmd = New SqlCommand("select * from Payment_Interest_Payment where Payment_Status='0'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds2, "tbl_SettlementSummary2")
        If (ds2.Tables(0).Rows.Count > 0) Then
            Dim i As Integer = 0
            For i = 0 To ds2.Tables(0).Rows.Count - 1
                Dim mystr As String = sendsoapfile("" + ds2.Tables("tbl_SettlementSummary2").Rows(i).Item("mobile").ToString + "", ds2.Tables("tbl_SettlementSummary2").Rows(i).Item("amount"), "" + ds2.Tables("tbl_SettlementSummary2").Rows(i).Item("id").ToString + "1999999564399", " from Bond Interest Payment . Thank You for using our services.")
                ' msgbox(i)
            Next
            cmd = New SqlCommand("update Payment_Interest_Payment set payment_status='1'", cn)
    ...
    Public Sub insertAccounts()
        Try
            ...
            For i = 0 To rowcount - 1
                ...
                cmd = New SqlCommand("insert into Payment_Interest_Payment (Mobile, Amount, Quantity, payment_status) values ('" + mobile + "','" + amount + "','" + quantity + "','0')", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
