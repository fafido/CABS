...
Partial Class TransferSec_Enq_Vault
    ...
    Public Function qry() As String
        ...
        cmd = New SqlCommand("select * from custodian_trades_report where ReportName='Vault'", cn)
    ...
    Public Sub loaddata(mro As String)
        If cmbvalue.SelectedItem.Value.ToString = "ALL" Then
            Dim ds1 As New DataSet
            cmd = New SqlCommand("  select " + qry() + "  from vw_Vault  where  convert(date, " + cmbdatetype.SelectedItem.Text + ")<=convert(date,'" + dtto.Date + "')  and  convert(date, " + cmbdatetype.SelectedItem.Text + ")>=convert(date,'" + dtfrom.Date + "')", cn)


            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds1, "Accounts_Clients1")

            ASPxGridView1.Caption = "Vault Transactions from " + cmbdatetype.SelectedItem.Text + " " + dtfrom.Date.ToString("dd MMM yyyy") + " to " + cmbdatetype.SelectedItem.Text + " " + dtto.Date.ToString("dd MMM yyyy") + ""
            If (ds1.Tables("Accounts_Clients1").Rows.Count > 0) Then
        ...
        Else
            ...
            Dim ds1 As New DataSet
            cmd = New SqlCommand("  select " + qry() + "  from vw_Vault  where [" + mro + "]='" + cmbvalue.SelectedItem.Text.ToString + "' and convert(date, " + cmbdatetype.SelectedItem.Text + ")<=convert(date,'" + dtto.Date + "')  and  convert(date, " + cmbdatetype.SelectedItem.Text + ")>=convert(date,'" + dtfrom.Date + "')", cn)
    ...
    Public Sub addcols(col As String)
        cmd = New SqlCommand("insert into Custodian_Trades_Report (report_column,ReportName ) values ('" + col + "','Vault')", cn)
    ...
    Public Sub deletecol(col As String)
        cmd = New SqlCommand("delete Custodian_Trades_Report where reportName='Vault'", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
    ...
    Public Sub filtersload()
        Dim ds1 As New DataSet
        cmd = New SqlCommand("SELECT * FROM (select 1 AS NR, 'ALL' as COLUMN_NAME UNION SELECT 2 AS NR, COLUMN_NAME   FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'vw_Vault' AND DATA_TYPE not like '%date%') j order by j.NR", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds1, "Accounts_Clients1")
        If (ds1.Tables("Accounts_Clients1").Rows.Count > 0) Then
    ...
    Public Sub filtersloadDate()
        Dim ds1 As New DataSet
        cmd = New SqlCommand("SELECT COLUMN_NAME   FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'vw_Vault' AND DATA_TYPE like '%date%'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds1, "Accounts_Clients1")
        If (ds1.Tables("Accounts_Clients1").Rows.Count > 0) Then
    ...
    Public Sub getfiltervalues(columnname As String)
        Dim ds1 As New DataSet
        cmd = New SqlCommand("select distinct [" + columnname + "] as val from vw_Vault", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds1, "Accounts_Clients1")
        If (ds1.Tables("Accounts_Clients1").Rows.Count > 0) Then
    ...
    Public Sub getcolumns()
        Dim ds1 As New DataSet
        cmd = New SqlCommand("  SELECT s.COLUMN_NAME as report_column, case when (select count(*) from custodian_trades_report where reportName='Vault' and report_column=s.COLUMN_NAME)=0 then 'False' else 'True' end as chk   FROM INFORMATION_SCHEMA.COLUMNS s WHERE s.TABLE_NAME = N'vw_Vault'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds1, "Accounts_Clients1")
        If (ds1.Tables("Accounts_Clients1").Rows.Count > 0) Then
