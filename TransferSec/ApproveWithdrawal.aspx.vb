...
Partial Class TransferSec_ApproveWithdrawal
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("  select *,'Redemption' as [Type] from bond_redemption where [status]='PENDING'", cn)
    ...
    Public Sub approvetrans(id As String)
        Try
            ...
            cmd = New SqlCommand("insert into trans (company, cds_number, date_created, trans_time, shares, Update_Type, Created_By, Batch_Ref, source, Pledge_shares, reference) select company, cds_number, getdate(), getdate(), quantity*-1, 'Trustee Redemption', '" + Session("Username") + "', '" + id + "', 'D', 0, '0' from bond_redemption where id='" + id + "' insert into CashTrans_Audit ([Description],TransType, Amount, DateCreated, TransStatus , cds_Number, PAID, Reference, ChargeCode, [Type], [CapturedBy],BeneficiaryBank, BeneficiaryBranch, BeneficiaryAccount, BeneficiaryAccountName)  select 'Series Redemption','Series Redemption', quantity  , getdate(), '0' , cds_Number, NULL, ID, '', 'PAYMENT', '" + Session("Username") + "',Bank , '', BankAccount , CDS_Number  from Bond_Redemption where id='" + id + "' update Bond_Redemption set [status]='Approved' where id='" + id + "'", cn)
    ...
    Public Sub rejecttrans(id As String)
        Try
            cmd = New SqlCommand("update Bond_Redemption set [status]='REJECTED' where id='" + id + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
