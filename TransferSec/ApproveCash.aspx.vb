...
Partial Class TransferSec_ApproveCash
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("select [ID]      ,[Description]      ,[TransType]      ,format([Amount] ,'#,0.00')  as Amount    ,[DateCreated]      ,[TransStatus]      ,[CDS_Number]      ,[Paid]      ,[Reference]      ,[ChargeCode]      ,[Type]      ,[Data]      ,[Extension], BeneficiaryBank, BeneficiaryBranch, BeneficiaryAccount,BeneficiaryAccountName from cashtrans_Audit where Transstatus='0'", cn)
    ...
    Private Sub grddocuments_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles grddocuments.RowCommand
        ...
        ElseIf e.CommandArgs.CommandName.ToString = "Approve Transaction" Then
            Try
                ...
                cmd = New SqlCommand("INSERT INTO CashTrans ([Description]      ,[TransType]      ,[Amount]      ,[DateCreated]      ,[TransStatus]      ,[CDS_Number]      ,[Paid]      ,[Reference]      ,[ChargeCode])SELECT [Description]     ,[TransType]      ,[Amount]      ,[DateCreated]      ,[TransStatus]      ,[CDS_Number]      ,[Paid]      ,[Reference]      ,[ChargeCode]     FROM [CashTrans_Audit] where ID='" + id + "' update cashtrans_audit set Transstatus='1' where ID='" + id + "'", cn)
        ...
        ElseIf e.CommandArgs.CommandName.ToString = "Decline Transaction" Then
            ...
            'Try
            '    m.sendmail(emaill, "Deposit Declined", "Your deposit of " + amt.ToString() + " for " + transtyp + " has been declined, kindly re-submit your deposit and attach proof of payment!")
            ...
            cmd = New SqlCommand("update cashtrans_audit set Transstatus='2' where ID='" + id + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
