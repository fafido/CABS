...
Partial Class Orders
    ...
    Protected Sub btnupload_Click(sender As Object, e As EventArgs)
        ...
        If fileupload1.HasFile Then
            ...
            For x = 1 To dtExcelRecords.Rows.Count - 1                                                                                                                                                                                      '                                                                                                                                                                                                                                                                                                                               ImportID,Company,Date_trade,CDS_Ref,Date_Settlement,Client_Id,Other_Names,Surname,Buy_Sell,Quantity,Price,UploadDate)
                ...
                Dim cmdStr As String = "insert into bankstatement ([Date], [Accountnumber], [Description], [Withdrawals], [Deposits],Balance)"
                cmdStr = cmdStr & "values(@Date, @Accountnumber, @Description, @Withdrawals, @Deposits, @Balance)"
                cmd2 = New SqlCommand(cmdStr, cn)
                cmd2.Parameters.AddWithValue("Date", dr.Item(0).ToString)
                cmd2.Parameters.AddWithValue("@Accountnumber", dr.Item(1).ToString)
                cmd2.Parameters.AddWithValue("@Description", dr.Item(2).ToString)
                cmd2.Parameters.AddWithValue("@Withdrawals", dr.Item(3).ToString)
                cmd2.Parameters.AddWithValue("@Deposits", dr.Item(4).ToString)
                cmd2.Parameters.AddWithValue("@Balance", dr.Item(5).ToString)
    ...
    Public Sub loadModules()
        Try
            cmd = New SqlCommand("SELECT * FROM bankstatement WHERE Accountnumber NOT IN (SELECT Accountnumber FROM [CDS_Router].[dbo].[bankstatement])", cn)
            adp = New SqlDataAdapter(cmd)
    ...
    Public Sub loadModules2()
        Try
            cmd = New SqlCommand("SELECT * FROM [CDS_Router].[dbo].[bankstatement] WHERE Accountnumber NOT IN (SELECT Accountnumber FROM  bankstatement )", cn)
            adp = New SqlDataAdapter(cmd)
            Dim ds1 As New DataSet
            ds1.Clear()
            adp.Fill(ds1, "Date")
    ...
    Public Sub loadModules3()
        Try
            cmd = New SqlCommand("Select sum(Balance) as [Balance], Balance from bankstatement group by Balance having sum(Balance)>0", cn)
            adp = New SqlDataAdapter(cmd)
            Dim ds1 As New DataSet
            ds1.Clear()
            adp.Fill(ds1, "Date")
