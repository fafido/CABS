...
Partial Class TransferSec_AFTApproval
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        ...
        Try
            ...
            em.sendmail(getemail(txtewraccountno.Text), "EWR Marked Available for Trading", Session("msg"))
        Catch ex As Exception
        ...
        Try
            ...
            a.auditlog(Session("BrokerCode"), Session("Username"), "Approved an AFT Request", txtAccountNo.Text, cmbparaCompany.Text)
        Catch ex As Exception
    ...
    Public Sub loadreceiptdetails()
        ...
        If xyz.OTPenabled = True Then
            cmd = New SqlCommand("select *,FORMAT(QUANTITY,'#,###.##','en-us') AS Quant from WR where receiptno in (select receiptid from AFT  where status='0' and ParticipantCode='" + Session("Brokercode") + "' and rejected is NULL and approvedby is NULL and OTPStatus='Approved' and deleted is NULL)", cn)
        Else
            cmd = New SqlCommand("select *,FORMAT(QUANTITY,'#,###.##','en-us') AS Quant from WR where receiptno in (select receiptid from AFT  where status='0' and ParticipantCode='" + Session("Brokercode") + "' and rejected is NULL and approvedby is NULL and deleted is NULL) ", cn)
        End If
        ...
        Else
            If txtrejectreason.Text <> "" Then
                Try
                    ...
                    Try
                        Dim a As New passmanagement
                        a.auditlog(Session("BrokerCode"), Session("Username"), "Rejected an AFT Request", txtAccountNo.Text, cmbparaCompany.Text)
                    Catch ex As Exception
