...
Partial Class cdcapproval
    ...
    Public Function pending(assetmanager As String, dat As String) As DataSet
        ...
        If assetmanager = "" And dat = "" Then
            cmd = New SqlCommand("select * from Recon_CDC_Approvals where [CurrentStatus]='Pending'", cn)
        Else
            cmd = New SqlCommand("select * from Recon_CDC_Approvals where convert(date, recondate)='" + dat + "' and AssetManager='" + assetmanager + "' and CurrentStatus='Pending'", cn)
        End If
        adp = New SqlDataAdapter(cmd)
    ...
    Public Sub approvetransaction(transkey As String)
        Try
            ...
            cmd = New SqlCommand("UPDATE Recon_cdc_Approvals set [Currentstatus]='APPROVED', AuthorizedBy='" + Session("Username") + "', DateAuthorized= getdate() where   id='" + transkey + "' AND currentStatus='Pending'", cn)
    ...
    Public Sub DECLINE(transkey As String)
        Try
            ...
            cmd = New SqlCommand("UPDATE Recon_cdc_Approvals set [Currentstatus]='REJECTED', AuthorizedBy='" + Session("Username") + "', DateAuthorized= getdate()  where  id='" + transkey + "' AND currentStatus='Pending'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
    ...
    Private Sub grddocuments_DataBinding(sender As Object, e As EventArgs) Handles grddocuments.DataBinding
        If cmbassetmanager0.SelectedIndex = -1 And dtdateview.Text = "" Then
            pending("", "")
        Else
            pending(cmbassetmanager0.SelectedItem.Value.ToString, dtdateview.Date.ToString)
        End If
    End Sub
