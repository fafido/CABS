...
Partial Class Corp_BonusInstr
    ...
    Sub getNextDivnumber()
        ...
        strSQL = "SELECT ISNULL(MAX(div_no),0)+1 as NextDivNo FROM Bonus WHERE company=@Company"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Protected Sub btnDivsearch_Click(sender As Object, e As EventArgs) Handles btnDivsearch.Click
        Dim strSQL As String = ""
        strSQL = "SELECT *,format(date_declared,'dd MMMM yyyy','en-us') as date_declared1,format(date_closed,'dd MMMM yyyy','en-us') as date_closed1,format(date_payment,'dd MMMM yyyy','en-us') as date_payment1,format(date_Yearending,'dd MMMM yyyy','en-us') as date_Yearending1 FROM Bonus WHERE company=@Company and div_no=@div_no"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                ElseIf Auth = 1 Then
                    msgbox("Bonus Issue " & txtIssueNo.Text & " at Authorization stage,proceed to reject it or contact Admin for reflagging to be able to update it")
                    clearFields()
                ElseIf Auth = 2 Then
                    msgbox("Bonus Issue " & txtIssueNo.Text & " at Computation stage, contact Admin for reflagging")
                    clearFields()
                ElseIf Auth = 3 Then
                    msgbox("Bonus Issue " & txtIssueNo.Text & " already Computed, contact Admin for reflagging")
                    clearFields()
                ElseIf Auth = 4 Then
                    msgbox("Bonus Issue " & txtIssueNo.Text & " already Computed/Posted, contact Admin for reflagging")
                    clearFields()
                End If
            Else
                msgbox("Invalid Bonus Issue " & txtIssueNo.Text)
                clearFields()
            End If
        End Using
    End Sub
    ...
    Sub saveInstruction()
        ...
        If DivNoExists() = True Then
            msgbox("Bonus Issue already exists")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = " INSERT INTO Bonus(Authorize,company,div_no,div_type,date_declared,date_closed,date_payment,date_Yearending,rate,TaxRate,bank_accNo,TaxBankAccount,mess_1,scrip_round,SpecieOfferShares,SpecieforEvery,SpecieNewCompany,scrip_price,createdBy)values(1,@company,@div_no,@div_type,GETDATE(),@date_closed,@date_payment,GETDATE(),@rate,@TaxRate,@bank_accNo,@TaxBankAccount,@mess_1,@scrip_round,@SpecieOfferShares,@SpecieforEvery,@SpecieNewCompany,@scrip_price,@createdBy) "
            Dim stmntAUDIT As String = " INSERT INTO BonusAuth(Authorize,company,div_no,div_type,date_declared,date_closed,date_payment,date_Yearending,rate,TaxRate,bank_accNo,TaxBankAccount,mess_1,scrip_round,SpecieOfferShares,SpecieforEvery,SpecieNewCompany,scrip_price,createdBy)values(1,@company,@div_no,@div_type,GETDATE(),@date_closed,@date_payment,GETDATE(),@rate,@TaxRate,@bank_accNo,@TaxBankAccount,@mess_1,@scrip_round,@SpecieOfferShares,@SpecieforEvery,@SpecieNewCompany,@scrip_price,@createdBy) "
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction saved, pending authorization') ; location.href='BonusInstr.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Bonus where Company=@Company and div_no=@div_no"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Sub UpdateInstruction()
        ...
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Instruction updated, pending authorization') ; location.href='BonusInstr.aspx'</script>")
    End Sub
End Class
