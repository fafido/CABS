...
Partial Class TransferSec_ApproveBondTrade
    ...
    Public Sub approvetrans(id As String)
        Try
            ...
            cmd = New SqlCommand("insert into trans (Company, CDS_Number, Date_Created, Trans_Time, Shares  , Update_Type, Created_By, Batch_Ref, Source, Pledge_shares, Reference, Instrument, [Broker], Custodian, AssetManager) select Company, ClientNo , getdate(), getdate(), CASE TradeType when 'RVP'  then Units when 'RF' then Units when 'DVP' then Units*-1  when 'DF' then Units*-1 else 0  end as [Shares], 'BOND TRADE', '" + Session("Username") + "', id, 'D', '0', TradeRef , 'EQUITY', [Broker], 'CABS', AssetManager from Bond_Trades where id='" + id + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
            ...
            cmd = New SqlCommand("insert into cashtrans ([Description], TransType , Amount, DateCreated , TransStatus , CDS_Number, Reference,AssetManager, BankName, BankAccount, Ref2, PostedBy, Currency)  select TradeType+ ' - '+ convert(nvarchar(50),Units)+ ' '+ Company +' @ '+ convert(nvarchar(50),price) , 'DVP Bond Trade' , SettlementAmount, getdate() , '1' , c.ClientNo , c.id,c.AssetManager, BankName, BankAccount, TradeRef , '" + Session("Username") + "', SettlementCurrency from Custodian_Trades c, Client_AssetManagers ca where ca.clientno=c.ClientNo and c.AssetManager=ca.AssetManager and c.id='" + id + "' AND TradeType='DVP'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
