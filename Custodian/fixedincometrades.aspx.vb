...
Partial Class TransferSec_fixedincometrades
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        Try
            ...
            If ASPxButton13.Text = "Withdraw" Then
                ...
                Else
                    ...
                    If xyz.OTPenabled = True Then
                        ...
                        Try
                            z.sendmail(getemail(txtewraccountno.Text), "Withdrawal Authorization Request", "A withdrawal on EWR No. " + cmbparaCompany.Value + " for " + txtshares.Text + " has been initiated in your account. Please authorize using OTP: " + OTP + "")
                        Catch ex As Exception
                    ...
                    Else
                        cmd = New SqlCommand("insert into withdrawals (amount_to_withdraw,EWR_holder, [date], [status], query, company, ReceiptID, CapturedBy, CaptureDate, ParticipantCode) values ('" + txtshares.Text + "','" + txtewraccountno.Text + "',getdate(),'0','0','" + cmbparaCompany.Text + "','" + cmbparaCompany.Value.ToString + "','" + Session("Username") + "',getdate(),'" + Session("BrokerCode") + "')", cn)
                    ...
                    Session("msg") = "Withdrawal request on EWR No. " + cmbparaCompany.Value.ToString + " captured successfully.Transaction ID is " + transid.ToString + ". Please enter OTP."
            ...
            Else
                ...
                If xyz.OTPenabled = True Then
                    ...
                    Dim z As New sendmail
                    Try
                        z.sendmail(getemail(txtewraccountno.Text), "Withdrawal Authorization Request", "A withdrawal on EWR No. " + cmbparaCompany.Value.ToString + " for " + txtshares.Text + " has been initiated in your account. Please authorize using OTP: " + OTP + "")
                    Catch ex As Exception
                        msgbox("Error Sending Email:" + ex.Message + "")
                    End Try

                    cmd = New SqlCommand("delete from withdrawals where id='" + lblid.Text + "' insert into withdrawals (amount_to_withdraw,EWR_holder, [date], [status], query, company, ReceiptID, CapturedBy, CaptureDate, ParticipantCode, OTP, OTPSent,OTPCreateTime ) values ('" + txtshares.Text + "','" + txtewraccountno.Text + "',getdate(),'0','0','" + cmbparaCompany.Text + "','" + cmbparaCompany.Value.ToString + "','" + Session("Username") + "',getdate(),'" + Session("BrokerCode") + "','" + OTP + "','1',getdate())", cn)
                ...
                Session("msg") = "Withdrawal request on EWR No. " + cmbparaCompany.Value.ToString + " has been re-submitted successfully.Transaction ID is " + transid.ToString + ". Please enter OTP."
                Session("mymy") = transid.ToString
                Response.Redirect(Request.RawUrl)
    ...
    Public Sub getpending()
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand("select w.id,case when w.OTPStatus IS NULL and w.ApprovedBy is NULL THEN 'Enter OTP' else 'N/A' end as [otp] , W.EWR_Holder as [Account No],wr.UnitMeasure, W.Company as [Details], W.ReceiptID AS [EWRNo], case when W.approvedby is NULL and w.rejected is NULL then 'Pending' when w.rejected='1' then 'Rejected' else 'Approved' end as  [Status],  W.[Date], FORMAT(W.amount_to_withdraw,'#,###.##','en-us') AS  amount_to_withdraw ,wr.Commodity, wr.Grade, wr.WarehousePhysical, wr.Date_Issued, wr.[Status] as wrstatus    from withdrawals W, WR wr where wr.ReceiptNo=w.ReceiptID and ParticipantCode='" + Session("BrokerCode") + "' and deleted is NULL", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Protected Sub ASPxButton14_Click(sender As Object, e As EventArgs) Handles ASPxButton14.Click
        ...
        If m.Document_Upload(fs, filePath, txtdocumentname.Text, ext, filename, "Withdrawal", txtewraccountno.Text, Session("autogen"), bytes).ToString <> "Upload Successful" Then
    ...
    Private Sub grddocuments_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles grddocuments.RowCommand
        ...
        If e.CommandArgs.CommandName.ToString = "View Document" Then
            ...
            Try
                pd(Session("autogen"), "Withdrawal")
            Catch ex As Exception
            End Try
            Try
                word(Session("autogen"), "Withdrawal")
            Catch ex As Exception
            End Try
            Try
                xls(Session("autogen"), "Withdrawal")
            Catch ex As Exception
            End Try
            Try
                Img(Session("autogen"), "Withdrawal")
            Catch ex As Exception
            End Try
