...
Partial Class TransferSec_moneymarketredemption
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Session("finish") = "resubmit" Then
                ...
                Session("msg") = ""
            End If
            If Session("finish") = "Redemption" Then
                Session("finish") = ""
            ...
            If (Not IsPostBack) Then
                ...
                dtredemptiondate.MaxDate = Date.UtcNow
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        Try
            ...
            ElseIf ASPxButton13.Text = "Update & Re-Submit" Then
                ...
                Dim currentamount As Decimal = txtamount.Text
                Dim interestamt As Decimal = txtinterestamt.Text
                ...
                cmd = New SqlCommand("update MoneyMarketRedemption set  Rejected=NULL, RejectionReason=NULL, RejectedDate=NULL, RedeemedAmount='" + txtcurrentvalue.Text + "', Amount='" + txtcurrentvalue.Text + "', RedeemedInterest='" + txtinterestamt.Text + "', RedemptionDate='" + dtredemptiondate.Date.ToString + "' where traderef='" + lblid.Text + "'", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
            ...
            ElseIf ASPxButton13.Text = "Submit Redemption" Then
                ...
                If redamount <= 0 Then
                    ...
                    Exit Sub
                End If
                cmd = New SqlCommand("  insert into MoneyMarketRedemption (TradeRef, Amount, DateCreated,CreatedBy, ClientNo, OriginalAmount, CurrentAmount, RedeemedInterest, RedeemedAmount, RedemptionDate) values ('" + lblid.Text + "','" + txtcurrentvalue.Text + "',getdate(), '" + Session("Username") + "','" + txtewraccountno.Text + "','" + txtamount.Text + "','" + lblcurrentamount.Text + "','" + txtinterestamt.Text + "','" + txtcurrentvalue.Text + "','" + dtredemptiondate.Date + "')", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
                cmd.ExecuteNonQuery()
                cn.Close()
                Session("finish") = "Redemption"
                Session("msg") = "Money Market Redemption Transaction has been successfully sent for approval!"
                Response.Redirect(Request.RawUrl)
    ...
    Public Sub getcallbackmoneymarket(holder As String)
        Dim dsport As New DataSet
        cmd = New SqlCommand("select id , [Description]+' '+convert(nvarchar(50),Amount)+' '+AssetManager+' '+isnull(Currency,'')+' '+interest_Type as descr from moneymarket where  TradeStatus='ON-GOING' AND ClientNo='" + holder + "' and id not in (  select TradeRef from MoneyMarketRedemption where rejected is NULL and Approved is NULL)", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
    ...
    Protected Sub cmbmoneymarket_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbmoneymarket.SelectedIndexChanged
        getdetails(cmbmoneymarket.SelectedItem.Value.ToString, "Submit Redemption")
    ...
    Public Sub calculateint()
        ...
        Dim numberofdays As Decimal = DateDiff(DateInterval.Day, dttradedate.Date, dtredemptiondate.Date)
        Dim dailyrate, totalgain, maturityamount As Decimal
        If cmbdaycount.SelectedItem.Text = "actual/360" Then
            dailyrate = rate / 360
            totalgain = dailyrate * numberofdays
            maturityamount = totalgain / 100 * amount
            txtinterestamt.Text = maturityamount.ToString("N")
        ElseIf cmbdaycount.SelectedItem.Text = "actual/365" Then
            dailyrate = rate / 365
            totalgain = dailyrate * numberofdays
            maturityamount = totalgain / 100 * amount
    ...
    Protected Sub dtredemptiondate_DateChanged(sender As Object, e As EventArgs) Handles dtredemptiondate.DateChanged
        Dim rate As Decimal = txtfixedrate.Text
        Dim amount As Decimal = txtamount.Text
        Dim numberofdays As Decimal = DateDiff(DateInterval.Day, dttradedate.Date, dtredemptiondate.Date)
        Dim dailyrate, totalgain, maturityamount As Decimal
        ...
        txtcurrentvalue.Text = amount.ToString
        lblcurrentamount.Text = maturityamount.ToString
    ...
    End Sub
    Protected Sub txtinterestamt_TextChanged(sender As Object, e As EventArgs) Handles txtinterestamt.TextChanged
