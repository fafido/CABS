Partial Class Bond_commit
    ...
    Protected Sub ASPxButton7_Click(sender As Object, e As EventArgs) Handles ASPxButton7.Click
        ...
        Try
            cmd = New SqlCommand("declare @isin nvarchar(50)='" + ASPxComboBox1.SelectedItem.Value.ToString + "' insert into trans select isin, cds_number, getdate(), getdate(), value/(select top 1 multiples from bond where isin=@isin), 'Auction', 'ATS', '0','S','0', NULL, 'BOND' from bids_bond where isin=@isin  and status_flag is null update bids_bond set status_flag='1' where isin=@isin and status_flag is null", cn)
            ...
            ' gebill_types()
            clearall()
            msgbox("Bond Register Successfully Captured!")
        Catch ex As Exception
            msgbox(ex.Message)
        End Try
    ...
    Protected Sub gebill_types()
        Try
            cmd = New SqlCommand("select fnam+' '+company as [fnm], COMPANY  from para_company where company in (select isin from bids_bond where status_flag='0' or status_flag is null)", cn)
    ...
    Protected Sub ASPxButton8_Click(sender As Object, e As EventArgs) Handles ASPxButton8.Click
        ...
        strscript += "window.open('bond_schedule.aspx?Company=" & ASPxComboBox1.SelectedItem.Value.ToString & "');"
        strscript += "</script>"
