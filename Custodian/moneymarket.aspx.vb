...
Partial Class TransferSec_moneymarket
    ...
    End Function
    Public Sub updatewithRef(ByVal autog As String, ByVal newref As String, ByVal typetrans As String)
    ...
    Protected Sub ASPxButton13_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ASPxButton13.Click
        Try
            ...
            If ASPxButton13.Text = "Save" Then
                ...
                If cmbordertype.SelectedItem.Text = "RVP" Then
                    Dim settleamt As Decimal = txtsettlementamount.Text
            ...
            ElseIf ASPxButton13.Text = "Update & Re-Submit" Then
                ...
                cmd = New SqlCommand("update MoneyMarket set ExchangeRate='" + txtexchangerate.Text + "', TradeRef='" + txtereference.Text + "', TradeDate='" + dttradedate.Text + "', SettlementDate='" + dtsettlementdate.Text + "', [Description]='" + txtdescription.Text + "', Amount='" + txtamount.Text + "', SettlementAmount='" + txtsettlementamount.Text + "', ClientNo='" + txtewraccountno.Text + "',  AssetManager='" + cmbassetmanager.SelectedItem.Value.ToString + "', TradeType='" + cmbordertype.SelectedItem.Text + "', Currency='" + cmbcurrency.SelectedItem.Value + "',[interest_Type]='" + cmbinteresttype.SelectedItem.Text + "' ,[FixedRate]='" + txtfixedrate.Text + "' ,[DayCountBasis]='" + cmbdaycount.SelectedItem.Text + "',[PaymentFrequency]='" + cmbpaymentfrequency.SelectedItem.Text + "',[Tax]='" + txttax.Text + "', [MaturityDate]='" + dtmaturitydate.Text + "', CounterPartBank='" + cmbbank.SelectedItem.Value.ToString + "', AccountNo='" + txtAccountNo.Text + "', AccountName='" + txtAccountName.Text + "', TradeCharge='" + txttranscharge.Text + "', SettlementCurrency='" + cmbcurrencysett.SelectedItem.Value + "', MaturityValue='" + txtmaturityvalue.Text + "', Rejected=NULL, RejectionReason=NULL, TradeStatus='OUTSTANDING' where id='" + lblid.Text + "'", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
    ...
    Public Sub getcommitted()
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand(" select *, a.surname+' '+a.forenames as ClientName from MoneyMarket m, Accounts_clients a where m.ApprovedBy is NOT NULL and m.rejected is NULL and a.cds_number=m.clientNo", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub getrejectedmoneymarket()
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand(" select *, a.surname+' '+a.forenames as ClientName from MoneyMarket m, Accounts_clients a where m.ApprovedBy is NULL and m.rejected='1' and deleted is NULL and a.cds_number=m.clientNo", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Protected Sub cmbparaCompany_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbinteresttype.SelectedIndexChanged
        calculatematurity()
    ...
    Private Sub ASPxGridView4_RowCommand(ByVal sender As Object, ByVal e As ASPxGridViewRowCommandEventArgs) Handles ASPxGridView4.RowCommand
    ...
    End Sub
    Private Sub ASPxGridView6_RowCommand(ByVal sender As Object, ByVal e As ASPxGridViewRowCommandEventArgs) Handles AspxGridview6.RowCommand
        Dim id As String = e.KeyValue.ToString
        If e.CommandArgs.CommandName.ToString = "Edit" Then
    ...
    End Sub
    Public Function deletable(ByVal idn As String) As Boolean
        Dim dsport As New DataSet
    ...
    Private Sub grddocuments_RowCommand(ByVal sender As Object, ByVal e As ASPxGridViewRowCommandEventArgs) Handles grddocuments.RowCommand
        ...
        ElseIf e.CommandArgs.CommandName.ToString = "Remove Document" Then
            If ASPxButton13.Text = "Update & Re-Submit" Or ASPxButton13.Text = "Update" Then
                deletedocument(lblid.Text, "MONEY MARKET")
                getdocuments(lblid.Text, "MONEY MARKET")
            Else
        ...
        End If
    End Sub
    Protected Sub cmbbank_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbbank.SelectedIndexChanged
        calculatematurity()

    End Sub
    Public Sub calculatematurity()
        Try
            Try
                If cmbinteresttype.SelectedItem.Text = "Fixed" Then
                    txtdescription.Text = cmbbank.SelectedItem.Value.ToString + " @ " + txtfixedrate.Text + "% P.A " + dtmaturitydate.Date + ""
                Else
                    txtdescription.Text = cmbbank.SelectedItem.Value.ToString + " @ " + txtfixedrate.Text + "% P.A"
                End If
            ...
            If cmbinteresttype.SelectedItem.Text = "Fixed" Then
                ...
                Dim tax As Decimal = txttax.Text
                ...
                If cmbdaycount.SelectedItem.Text = "actual/360" Then
                    ...
                    totalgain = totalgain - (tax / 100 * totalgain)
                ...
                ElseIf cmbdaycount.SelectedItem.Text = "actual/365" Then
                    ...
                    maturityamount = (totalgain / 100 * amount) + amount
                End If
                txtmaturityvalue.Text = maturityamount.ToString("N")
                dtmaturitydate.ReadOnly = True
                dtmaturitydate.ValidationSettings.RequiredField.IsRequired = True
                txtnumberofdays.ReadOnly = False


            Else
                txtmaturityvalue.Text = "0"
                dtmaturitydate.ReadOnly = True
                dtmaturitydate.ValidationSettings.RequiredField.IsRequired = False
                txtnumberofdays.Text = "0"
                txtnumberofdays.ReadOnly = True
                ' dtmaturitydate.Date = ""
                dtmaturitydate.Text = ""
            End If
    ...
    Protected Sub txtnumberofdays_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtnumberofdays.TextChanged
    ...
    Protected Sub dttradedate_DateChanged(ByVal sender As Object, ByVal e As EventArgs) Handles dttradedate.DateChanged

    End Sub
    Protected Sub ASPxButton15_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ASPxButton15.Click
        Response.Redirect(Request.RawUrl)
    End Sub
    Protected Sub cmbdaycount_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbdaycount.SelectedIndexChanged
        calculatematurity()
    ...
    Protected Sub cmbcurrency_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbcurrency.SelectedIndexChanged
        ...
        End Try
    End Sub
    Protected Sub cmbcurrencysett_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbcurrencysett.SelectedIndexChanged
        Try
            If cmbcurrency.SelectedItem.Text <> cmbcurrencysett.SelectedItem.Text Then
                txtexchangerate.ReadOnly = False
                txtexchangerate.Text = ""
    ...
    Protected Sub txtamount_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtamount.TextChanged
        calculatematurity()
        txtsettlementamount.Text = txtamount.Text
        calculatesettlement()
    ...
    End Sub
    Protected Sub txtfixedrate_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtfixedrate.TextChanged
        calculatematurity()

    End Sub
    Protected Sub calculatesettlement()
        Try
            If cmbcurrency.SelectedItem.Text <> cmbcurrencysett.SelectedItem.Text Then
                Dim currentrate As Decimal = txtexchangerate.Text
                Dim tradeamt As Decimal = txtamount.Text
    ...
    Protected Sub cmbordertype_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbordertype.SelectedIndexChanged
        If cmbordertype.SelectedItem.Text = "RF" Then
            ...
            '    dtsettlementdate.MinDate = Date.UtcNow.AddDays(-9999)
        Else
