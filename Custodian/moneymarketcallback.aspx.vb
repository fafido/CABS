...
Partial Class TransferSec_moneymarketcallback
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Session("finish") = "resubmit" Then
                ...
                Session("msg") = ""
            End If
            If Session("finish") = "callback" Then
                Session("finish") = ""
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        Try
            ...
            ElseIf ASPxButton13.Text = "Update & Re-Submit" Then
                ...
                cmd = New SqlCommand("update MoneyMarketCallback set  Rejected=NULL, RejectionReason=NULL, RejectedDate=NULL where traderef='" + lblid.Text + "'", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
            ...
            ElseIf ASPxButton13.Text = "Callback" Then


                cmd = New SqlCommand("  insert into MoneyMarketCallback (TradeRef, Amount, DateCreated,CreatedBy, ClientNo, OriginalAmount) values ('" + lblid.Text + "','" + txtcurrentvalue.Text + "',getdate(), '" + Session("Username") + "','" + txtewraccountno.Text + "','" + txtamount.Text + "')", cn)
                If (cn.State = ConnectionState.Open) Then
                    cn.Close()
                End If
                cn.Open()
                ...
                Session("msg") = "Money Market Callback Transaction has been successfully sent for approval!"
                Response.Redirect(Request.RawUrl)
    ...
    Public Sub getcallbackmoneymarket(holder As String)
        Dim dsport As New DataSet
        cmd = New SqlCommand("select id , [Description]+' '+convert(nvarchar(50),Amount)+' '+AssetManager+' '+isnull(Currency,'')+' '+interest_Type as descr from moneymarket where interest_Type='On Call' and TradeStatus='ON-GOING' AND ClientNo='" + holder + "' and id not in (  select TradeRef from MoneyMarketCallback where rejected is NULL and Approved is NULL)", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(dsport, "trans")
        If (dsport.Tables(0).Rows.Count > 0) Then
    ...
    Public Sub getdetails(id As String, transtype As String)
        ...
        If (dsport.Tables(0).Rows.Count > 0) Then
            ...
            txtcurrentvalue.Text = maturityamount.ToString("N")
        End If
    ...
    Protected Sub cmbmoneymarket_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbmoneymarket.SelectedIndexChanged
        getdetails(cmbmoneymarket.SelectedItem.Value.ToString, "Callback")
