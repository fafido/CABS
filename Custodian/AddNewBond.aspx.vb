...
Partial Class AddNewBond
    ...
    Protected Sub loadBonds()
        cmd = New SqlCommand("  select ID,  Issuer, Code AS Series,[Type], DayCountBasis, Currency, FaceValue, Rate, IssueDate, MaturityDate,[Status] FROM Bond where Category='BOND'", cn)
        Dim ds As New DataSet
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "Bonds")
        If ds.Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub btnSaveContract_Click(sender As Object, e As EventArgs) Handles btnSaveTB.Click
        ...
        ElseIf btnSaveTB.Text = "Update" Then
            ...
            Dim query As String = " update  Bond set Code=@code, Issuer=@Issuer, Tenor=@Tenor,FaceValue=@FaceValue, Rate=@CouponRate, IssueDate=@IssueDate, MaturityDate=@MaturityDate,  [Type]=@Type, ISIN=@ISIN, CouponPayments=@CouponPayments, DayCountBasis=@DayCountBasis, CreatedBy=@CreatedBy, [Status]=@Status, Currency=@Currency, Bank=@Bank, AccountNo=@AccountNo, AccountName=@AccountName, ApprovedBy=NULL, ApprovedDate=NULL where id='" + lbltransid.Text + "'"
            Using conny As New SqlConnection(constr)
                Using comm As New SqlCommand()
                    With comm
                        .Connection = conny
