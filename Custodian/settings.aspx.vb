Partial Class Settings
    ...
    End Sub
    Public Sub getcustodianAccountAuth()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select top 1 on_off from para_settings_custodians where control='Accounts Authorization' and custodian='" + Session("Brokercode") + "' order by id desc", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "para_country")

            If ds.Tables(0).Rows(0).Item("on_off") = "On" Then
                rdauthorizationon.Checked = True
            Else
                rdauthorizationoff.Checked = True
            End If
        Catch ex As Exception
            msgbox(ex.Message)
        End Try
    ...
    Protected Sub ASPxButton6_Click(sender As Object, e As EventArgs) Handles ASPxButton6.Click
        Dim on_off As String
        If rdauthorizationon.Checked = True Then
            on_off = "On"
        Else
            on_off = "Off"
        End If

        cmd = New SqlCommand("insert into para_settings_custodians  values ('Accounts Authorization','" + on_off + "','" + Session("Brokercode") + "')", cn)
        If (cn.State = ConnectionState.Open) Then
            cn.Close()
        End If
        cn.Open()
