...
Partial Class TransferSec_ApproveNewStat
    ...
    Public Sub getpending()
        ...
        cmd = New SqlCommand("select * from Custodian_Trades_Comm where ApprovedBy is NULL and rejected is NULL", cn)
    ...
    Private Sub grddocuments_RowCommand(sender As Object, e As ASPxGridViewRowCommandEventArgs) Handles grddocuments.RowCommand
        ...
        ElseIf e.CommandArgs.CommandName.ToString = "Approve Transaction" Then
            Try
                ...
                cmd = New SqlCommand("update Custodian_Trades_Comm set ApprovedBy='" + Session("Username") + "', ApprovedDate=getdate() where id='" + id + "'", cn)
        ...
        ElseIf e.CommandArgs.CommandName.ToString = "Decline Transaction" Then
            ...
            cmd = New SqlCommand("update Custodian_Trades_Comm set Rejected='1', RejectionReason='REJECTED BY ADMIN' where id='" + id + "'", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
