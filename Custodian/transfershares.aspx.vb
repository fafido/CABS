Partial Class TransferSec_transfershares
    ...
    Public Sub getgrid()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select s.*, a1.Surname+' '+a1.Forenames as [FromName],a2.Surname+' '+a2.Forenames as [ToName] from share_transfer s, Accounts_Clients a1, Accounts_Clients a2 where s.transferor=a1.CDS_Number and a2.CDS_Number=s.transferee", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "para_batch_type")
            If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Protected Sub ASPxButton12_Click(sender As Object, e As EventArgs) Handles ASPxButton12.Click
        If ASPxButton12.Text = "Transfer" Then
            Try
                ...
                If av < totrans Then
                    msgbox("Please enter shares less or equal to available shares!")
                Else
            ...
            Try
                ...
                Else
                    ...
                    If n = "" Then
                        ...
                        Exit Sub
                    End If
                    cmd = New SqlCommand("update share_transfer SET amount_to_transfer= '" + txtshares.Text + "', transferor='" + ListBox1.SelectedValue.ToString + "', transferee='" + ListBox2.SelectedValue.ToString + "', [date]='" + dtreallocation.Date.ToString + "', [status]='PENDING', company='" + cmbparaCompany.SelectedItem.Value.ToString + "',  CapturedBy='" + Session("Username") + "', CaptureDate=getdate() where id='" + lblid.Text + "'", cn)
                    If (cn.State = ConnectionState.Open) Then
                        cn.Close()
                    End If
                    cn.Open()
    ...
    Protected Sub ASPxButton15_Click(sender As Object, e As EventArgs) Handles ASPxButton15.Click
        ...
        Dim ds As New DataSet
        cmd = New SqlCommand("select cds_number+' '+isnull(surname,'')+' '+isnull(middlename,'')+' '+isnull(forenames,'') as names, cds_number from accounts_clients where  BankAccount_Type='Trustee'  and cds_number+' '+isnull(surname,'')+' '+isnull(middlename,'')+' '+isnull(forenames,'') like '%" + txtclientnumber0.Text + "%'", cn)
        adp = New SqlDataAdapter(cmd)
        adp.Fill(ds, "names")
        If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Private Sub grdpending_SelectedIndexChanged(sender As Object, e As EventArgs) Handles grdpending.SelectedIndexChanged
        Dim ds As New DataSet
        cmd = New SqlCommand("select s.*, a1.Surname+' '+a1.Forenames as [FromName],a2.Surname+' '+a2.Forenames as [ToName] from share_transfer s, Accounts_Clients a1, Accounts_Clients a2 where s.transferor=a1.CDS_Number and a2.CDS_Number=s.transferee and s.id='" + grdpending.SelectedValue.ToString + "' AND s.[status] in ('PENDING','REJECTED')", cn)
        adp = New SqlDataAdapter(cmd)
        ...
        If (ds.Tables(0).Rows.Count > 0) Then
            ...
            dtreallocation.Date = ds.Tables(0).Rows(0).Item("date").ToString
