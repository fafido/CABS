...
Partial Class TransferSec_vaultinother
    ...
    Protected Sub ASPxButton13_Click(sender As Object, e As EventArgs) Handles ASPxButton13.Click
        ...
        If ASPxButton13.Text = "Save" Then
            ...
            If xyz.OTPenabled = True Then
                ...
                Try
                    z.sendmail(getemail(txtewraccountno.Text), "New Deposit", "A Certificate Deposit on Cert No. " + txtserialno.Text + " for " + cmbparaCompany.SelectedItem.Text + " has been initiated in your account. ")
                Catch ex As Exception
                    msgbox("Error Sending Email:" + ex.Message + "")
                End Try

                cmd = New SqlCommand("insert into Deposit_Cert (quantity,holder, [date], [status], query, company, Certificate_No, CapturedBy, CaptureDate, ParticipantCode, OTP, OTPSent,OTPCreateTime,Price, IssueDate, MaturityDate, Charge, DepType ) values ('" + txtavailableshares.Text + "','" + txtewraccountno.Text + "',getdate(),'0','0','" + cmbparaCompany.Value.ToString + "','" + txtserialno.Text + "','" + Session("Username") + "',getdate(),'" + Session("BrokerCode") + "','" + OTP + "','1',getdate(),'" + txtprice.Text + "',getdate(),getdate(),'" + txttranscharge.Text + "','DEPOSIT')", cn)

            Else
                cmd = New SqlCommand("insert into Deposit_Cert (quantity,holder, [date], [status], query, company, Certificate_No, CapturedBy, CaptureDate, ParticipantCode, Price, IssueDate, MaturityDate, Charge, DepType) values ('" + txtavailableshares.Text + "','" + txtewraccountno.Text + "',getdate(),'0','0','" + cmbparaCompany.Value.ToString + "','" + txtserialno.Text + "','" + Session("Username") + "',getdate(),'" + Session("BrokerCode") + "','" + txtprice.Text + "',getdate(),getdate(),'" + txttranscharge.Text + "','DEPOSIT')", cn)
            ...
            Try
                ...
                a.auditlog(Session("BrokerCode"), Session("Username"), "Submitted a Certificate. Cert No." + txtserialno.Text + ", Company: " + cmbparaCompany.SelectedItem.Value.ToString + "", txtewraccountno.Text, cmbparaCompany.Text)
            Catch ex As Exception
    ...
    Public Function pendingbalance(receipt As String) As String
        ...
        End If
    End Function
    Public Function unitofmeasure(product As String) As String
        Dim ds1 As New DataSet
