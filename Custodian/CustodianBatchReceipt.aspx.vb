...
Partial Class Custodian_CustodianBatchReceipt
    ...
    Protected Sub btnAddCerts_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddCerts.Click
        Try
            ...
            cmd = New SqlCommand("select * from CertificatesRegister where company='" & cmbParaCompany.Text & "' and certificate=" & txtCert.Text & "", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(dscert, "CertificatesRegister")

            If (dscert.Tables(0).Rows.Count > 0) Then
                If (dscert.Tables(0).Rows(0).Item("status").ToString > 0) Then
                    msgbox("Entered certificate has already been immobilized")
                    Exit Sub
                End If
            End If
        ...
        Try
            ...
            If (CmbBatchType.Text = "XFER") Then
                ...
                If (ros.Tables(0).Rows.Count > 0) Then
                    ...
                    If (dsshares.Tables(0).Rows(0).Item("shares").ToString < CInt(txtShares.Text)) Then
                        msgbox("Cannot Batch transfer shares less than shares held: " & dsshares.Tables(0).Rows(0).Item("shares").ToString)
                        Exit Sub
                    End If
                Else
