      ...
      '  Else
       '     msgbox("Please enter a 12 character ISIN!")
        ...
        If rdbType.SelectedValue = "Save Equity" Then
            ...
            End If
        Else
            Dim stmnt As String = "insert into para_company([Company],[Fnam],[Date_created],[Issued_shares],[registrar],[Add_1],[Add_2],[Add_3],[Add_4],[City],[Country],[Contact_Person],[Telephone],[Cellphone],[Fax],[Comments],[sec_type],[board],[index_type],[ticker],[ISIN],[year_listed],[comp_secretary],[email],[website],[issued_capital],[nominal_value]) values (''" & removeSpecialCharacter(txtISIN.Text) & "'',''" & removeSpecialCharacter(txtSecName.Text) & "'',''" & dtYear.Text & "'',''" & txtIssuedCapital.Text & "'',(select company_code from client_companies where company_name=''" & removeSpecialCharacter(txtcompsecretary.Text) & "''),''" & removeSpecialCharacter(txtRegdOffice1.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice2.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice3.Text) & "'',''" & removeSpecialCharacter(txtRegdOffice4.Text) & "'','''',''" & cmbCountryListed.SelectedItem.Value & "'',''" & removeSpecialCharacter(txtcompsecretary.Text) & "'',''" & txtPhoneNo.Text & "'','''',''" & txtFaxNo.Text & "'','''',''GDR'','''','''','''',''" & txtISIN.Text & "'',''" & dtYear.Text & "'',(select company_code from client_companies where company_name=''" & removeSpecialCharacter(txtcompsecretary.Text) & "''),''" & txtEmail.Text & "'',''" & txtWebsite.Text & "'',''" & txtIssuedCapital.Text & "'',''" & txtNominalValue.Text & "'')      insert into testcds.dbo.para_company (company, sector, Issued_shares, [status], Date_created, Contact_Person, Cellphone, email, isin_no, Market_Segment, Instrument, Index_Type, fhl, fel, swl, InitialPrice, fnam, Board) values (''" + txtISIN.Text + "'',''0'',''" + txtIssuedCapital.Text + "'',''0'',getdate(),''ADMIN'',''0'',''0'', ''" + txtISIN.Text + "'',''0'',''GDR'',''20 SHARE INDEX'',''0'',''0'',''0'',''" + txtprice.Text + "'',''" + txtsearchsecname.SelectedItem.Text + "'',''Equities Board'')  declare @price numeric(18,4)=''" + txtprice.Text + "'' insert into testcds.dbo.companyprices (company, SHARESINISSUE, LASTPRICE, CHANGEPercent, CHANGEValue, CurrentPrice, [ShareVOL],sharevalue, highestprice, LowestPrice, UpdatedDate, OpeningPrice, bestprice) select company, issued_shares, @price,''0'',''0'',@price,''0'',''0'',@price, @price, getdate(), @price, @price from para_company where company=''" + txtISIN.Text + "''"
            Dim descr As String = "Added a new Instrument with the following details. Company=" + txtSecName.Text + "   Full Company Name=" + txtSecName.Text + " Issued Shares:" + txtIssuedCapital.Text + "  Address:" + txtRegdOffice1.Text + " " + txtRegdOffice2.Text + " " + txtRegdOffice3.Text + " " + txtRegdOffice4.Text + " Country:" + cmbCountryListed.SelectedItem.Text + " Security Type:GDR   Type:" + txtTicker.Text + " ISIN" + txtISIN.Text + "   Nominal Value:" + txtNominalValue.Text + " Status:" + cmbIndexType0.SelectedItem.Text + " Term:" + txtNominalValue0.Text + ""
