...
Partial Class Corp_SplitFinalPosting
    ...
    Sub PostRights()
        If DivNoExists() = False Then
            msgbox("Split instruction no longer at Final Authorization stage")
            Exit Sub
        End If
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            End Using
        End Using
        Response.Write("<script>alert('Split Final Authorization successful') ; location.href='SplitFinalPosting.aspx'</script>")
    End Sub
    ...
    Function DivNoExists() As Boolean
        Dim strSQL As String = ""
        strSQL = "SELECT * from Split_instr where Company=@Company and div_no=@div_no and Authorize=3"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
