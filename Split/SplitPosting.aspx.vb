...
Partial Class Corp_SplitFinalPosting
    ...
    Sub getCompanies()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            ...
            Dim cmd = New SqlCommand("SELECT DISTINCT company FROM Split_instr where Authorize=3 order by company", cn)
    ...
    Sub getDivnumber()
        ...
        strSQL = "SELECT DISTINCT div_no FROM Split_instr where Authorize=3 and company=@company order by div_no desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
    ...
    Private Function ISSelectedData() As Boolean
        ...
        End If
    End Function
    Private Function SomeSelectedDataMisMatchAmountToPaySplit() As Boolean
        Dim myGridView As New ASPxGridView
        myGridView = grdPaymentsSplit
    ...
    Sub PostSplit()
        If ISSelectedData() = False Then
            ...
            Exit Sub
        End If
        If SomeSelectedDataMisMatchAmountToPaySplit() = True Then
            msgbox("Split Shares to Post + Write-off should not exceed remaining Split Shares")
            Exit Sub
        End If
        PostTempData()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "Proc_PostSplit"
            ...
            End Using
        End Using
        Response.Write("<script>alert('Split Shares Posted successfully, awaiting authorization') ; location.href='SplitPosting.aspx'</script>")
    End Sub
    ...
    Sub getSplitData()
        Dim strSQL As String = " "
        strSQL = "SELECT a.*,a.Scaled_Shares-ISNULL(c.PaidShares,0) as RemainingShares FROM Split_Issue a left outer join (SELECT b.company,b.Div_No,b.Shareholder,b.AssetManager,sum(b.Shares) as PaidShares FROM SplitPayments b WHERE ISNULL(b.Authorized,0) in (0,1) group by b.company,b.Div_No,b.Shareholder,b.AssetManager) c On a.company=c.Company and a.Split_No=c.Div_No and a.Shareholder=c.Shareholder and a.AssetManager=c.AssetManager where a.company=@Company and a.Split_No=@IssueNo and isnull(a.Scaled_Shares,0)>0 AND (a.Scaled_Shares-isnull(c.PaidShares,0)>0) Order by RemainingShares desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
            Dim cmd = New SqlCommand(strSQL, cn)
            cmd.Parameters.AddWithValue("@Company", cmbCompany.Value)
    ...
    Protected Sub btnCalc_Click(sender As Object, e As EventArgs) Handles btnCalc.Click
        ...
        Dim totalwriteoffSplitShares As Long = 0
        Dim myGridView As New ASPxGridView
        myGridView = grdPaymentsSplit
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"SEQ"})
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                Dim PostShares As Long = 0, writeoffSplitShares As Long = 0
                Try
                ...
                End Try
                Try
                    writeoffSplitShares = CLng(TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("SharesWriteoffc"), GridViewDataColumn), "txtSharesWriteoff"), ASPxTextBox).Text)
                Catch ex As Exception
                End Try
                totalPostShares += PostShares
                totalwriteoffSplitShares += writeoffSplitShares
            End If
        Next
        lblTotalCashSelected.Text = "Split Shares to Post: " & totalPostShares.ToString("#,#") & " . Write-off Split Shares: " & totalwriteoffSplitShares.ToString("#,#") & ""
    End Sub
    Sub PostTempData()
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim stmnt As String = "DELETE FROM SplitPayments_temp WHERE PaidBy=@PaidBy"
            Using cmd As New SqlCommand(stmnt, cn)
        ...
        sharesPostDesc = "Split Shares"
        sharesWriteoffDesc = "Write-off Split Shares"
        Dim keys As List(Of Object) = myGridView.GetCurrentPageRowValues(New String() {"SEQ"})
        For Each key As Object In keys
            If TryCast(myGridView.FindRowCellTemplateControlByKey(key, TryCast(myGridView.Columns("Selec"), GridViewDataColumn), "chk1"), ASPxCheckBox).Checked = True Then
                ...
                If PostShares > 0 Then
                    stmnt = " INSERT INTO SplitPayments_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [PaidBy], [PaymentType],[Shares])VALUES(@Company,@IssueNo,@Shareholder,@AssetManager,0,@PostedBy,'" & sharesPostDesc & "','" & PostShares & "') "
                End If
                If writeoffSplitShares > 0 Then
                    stmnt = stmnt & " INSERT INTO SplitPayments_temp([Company], [Div_No], [Shareholder], [AssetManager], [AmountPaid], [PaidBy], [PaymentType],[Shares])VALUES(@Company,@IssueNo,@Shareholder,@AssetManager,0,@PostedBy,'" & sharesWriteoffDesc & "','" & writeoffSplitShares & "') "
                End If
                Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
                    Using cmd As New SqlCommand(stmnt, cn)
                        cmd.CommandType = CommandType.Text
    ...
    Protected Sub grdPaymentsSplit_DataBinding(sender As Object, e As EventArgs) Handles grdPaymentsSplit.DataBinding
        grdPaymentsSplit.DataSource = getSpliDataDSET()
    End Sub
    Function getSpliDataDSET() As DataSet
        Dim strSQL As String = " "
        strSQL = "SELECT a.*,a.Scaled_Shares-ISNULL(c.PaidShares,0) as RemainingShares FROM Split_Issue a left outer join (SELECT b.company,b.Div_No,b.Shareholder,b.AssetManager,sum(b.Shares) as PaidShares FROM SplitPayments b WHERE ISNULL(b.Authorized,0) in (0,1) group by b.company,b.Div_No,b.Shareholder,b.AssetManager) c On a.company=c.Company and a.Split_No=c.Div_No and a.Shareholder=c.Shareholder and a.AssetManager=c.AssetManager where a.company=@Company and a.Split_No=@IssueNo and isnull(a.Scaled_Shares,0)>0 AND (a.Scaled_Shares-isnull(c.PaidShares,0)>0) Order by RemainingShares desc"
        Using cn = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("connpath"))
            Dim ds As New DataSet
