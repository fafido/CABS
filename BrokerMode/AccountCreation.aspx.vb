...
Partial Class BrokerMode_AccountCreation
    ...
    Public Sub SecurityDocuments()
        ...
        End Try
    End Sub
    Public Sub GetTaxCode()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select distinct (rate) from para_tax ", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "para_tax")
            If (ds.Tables(0).Rows.Count > 0) Then
                cmbTax.DataSource = ds.Tables(0)
                cmbTax.DataValueField = "rate"
                cmbTax.DataBind()
            Else
                cmbTax.Items.Clear()
            End If
        ...
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                ...
                If (cmbIndustry.Items.Count > 0) Then
                    ...
                    cmd = New SqlCommand("select rate from para_tax where code='" & cmbIndustry.SelectedItem.Text & "'", cn)
                    adp = New SqlDataAdapter(cmd)
                    adp.Fill(ri, "para_tax")
                    If (ri.Tables(0).Rows.Count > 0) Then
                        cmbTax.SelectedItem.Text = ri.Tables(0).Rows(0).Item("rate").ToString
                    End If
                End If
            End If
        Catch ex As Exception
