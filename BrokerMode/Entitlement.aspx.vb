...
Partial Class BrokerMode_Entitlement
        ...
        Try
            ...
            If (Not IsPostBack) Then
                Try
                    ...
                    End If
                    txtFileName.Text = "" & Now.Date & "Entitlements.csv"
                Catch ex As Exception
                    msgbox(ex.Message)
                End Try
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        cmd = New SqlCommand("declare @company nvarchar(50)='" + ASPxComboBox1.SelectedItem.Text + "' DECLARE @date_as_at date='" + ASPxDateEdit1.Text + "' select convert(numeric(18,0),cds_number) as [Investor Code], Surname as [LASTNAME], ISNULL(FORENAMES,'')+' '+ISNULL(MIDDLEname,'') AS [FIRSTNAME], CUSTODIAN,CASE Accounttype  WHEN 'I' THEN 'IND' WHEN 'C' THEN 'CORP' WHEN 'J' THEN 'IND' WHEN 'B' THEN 'CORP' END  AS   INVESTORTYPE, 'ZWE' AS [COUNTRY], 'N' AS [TAX EXEMPT], IDNOPP AS [REGISTRATION ID], replace(ADD_1,',','') AS [ADDRESS 1], replace(ADD_2,',','') AS [ADDRESS 2], TEL AS [BUSINESS PHONE], '' AS [HOME PHONE], EMAIL AS [EMAIL],mobile as [MOBILE PHONE], '' AS [SWIFT ADDRESS], DIV_BANK AS [BANK NAME], (  select top 1 branch from para_branch where branch_name=Accounts_clients.Div_Branch and bank=(select bank from para_bank where bank_name=div_bank)) AS [BRANCH NUMBER],DIV_BRANCH AS [BRANCH NAME], Div_AccountNo as [BANK ACCOUNT],divpayee as [BANK ACCOUNT NAME],  (select fnam from para_company where company=@company) AS [COUNTER NAME], (select isin from para_company where company=@company) AS [ISIN], Isnull((SELECT SUM(SHARES) FROM  TRANS WHERE CDS_NUMBER=ACCOUNTS_CLIENTS.CDS_nUMBER and company=@company and Date_Created<=@date_as_at HAVING sum(shares)>0),0) as [QUANTITY]   from Accounts_Clients where cds_number in (SELECT cds_Number FROM  TRANS where company=@company and Date_Created<=@date_as_at group by CDS_Number HAVING sum(shares)>0)", cn)
    ...
    Public Sub LDGRD()
        ...
        cmd = New SqlCommand("select getdate() as [Date], (select  trading_branch from client_companies where company_code=(select  BrokerCode from Accounts_Clients where cds_number=mo.buyercdsno)) as [From Branch], (select  trading_account from client_companies where company_code=(select  BrokerCode from Accounts_Clients where cds_number=mo.buyercdsno)) as [From Account No],(select  trading_account_name  from client_companies where company_code=(select  BrokerCode from Accounts_Clients where cds_number=mo.buyercdsno)) as [From Account Name], (select  trading_branch from client_companies where company_code=(select  BrokerCode from Accounts_Clients where cds_number=mo.Sellercdsno)) as [To Branch], (select  trading_account from client_companies where company_code=(select  BrokerCode from Accounts_Clients where cds_number=mo.Sellercdsno)) as [To Account No],(select  trading_account_name  from client_companies where company_code=(select  BrokerCode from Accounts_Clients where cds_number=mo.Sellercdsno)) as [To Account Name],case when (select top 1 isnull(currency,'Ksh') from testcds.dbo.broker_curr where seller_cds=mo.Sellercdsno order by id desc)='Ksh' then ss.Amount_due when (select top 1 isnull(currency,'Ksh') from testcds.dbo.broker_curr where seller_cds=mo.Sellercdsno order by id desc)<>'Ksh' then ss.Amount_due*(select ratetobase from para_currencyrates where currencycode=(select currencycode from para_Currencies  where currencysymbol=(select top 1 currency from testcds.dbo.broker_curr where seller_cds=mo.Sellercdsno order by id desc))) else ss.Amount_due   end as [Amount], isnull((select top 1 currency from testcds.dbo.broker_curr where seller_cds =mo.Sellercdsno order by id desc),'Ksh') as [USD] from tbl_matchedorders mo, tbl_SettlementSummary ss where right(ss.deal_reference, LEN(SS.deal_reference)-2)=convert(varchar(30),mo.deal) and ss.OrderType ='B' AND ss.Eft_File_Status='O'", cn)
    ...
    Public Sub Csv1()
        Try
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" & Now.Date & "Entitlement_File.csv")
            Response.Charset = ""
            Response.ContentType = "application/text"
    ...
    Protected Sub txtFileName_TextChanged(sender As Object, e As EventArgs) Handles txtFileName.TextChanged
