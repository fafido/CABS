...
Partial Class BrokerMode_DormantAccountsApproval
    ...
    Public Sub GetHolderData()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("Select * from names where dormantState=3 ", cn)
    ...
    Public Sub getHolderDetails()
        Try
            Dim ds As New DataSet
            cmd = New SqlCommand("select * from names where cds_number='" & cmbCdsNumber.Text & "' and dormantState=3 ", cn)
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "names")
            If (ds.Tables(0).Rows.Count > 0) Then
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            If (ds.Tables(0).Rows.Count > 0) Then
                ...
                cmd = New SqlCommand("update names set dormantState=4 where cds_number='" & cmbCdsNumber.Text & "'", cn)
    ...
    Protected Sub btnSave0_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave0.Click
        Try
            cmd = New SqlCommand("Update pre_names_creation set Approved=3, Approved_By='" & Session("Username").ToString & "', Approved_On='" & Date.Now & "' where cds_number='" & cmbCdsNumber.Text & "' and approved=0", cn)
            If (cn.State = ConnectionState.Open) Then
                cn.Close()
            End If
            cn.Open()
